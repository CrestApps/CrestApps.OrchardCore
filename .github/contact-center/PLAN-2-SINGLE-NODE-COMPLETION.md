...
> - **Verify Omnichannel against current branch source, not the `orchardcore-omnichannel` skill or older docs** — that module has changed substantially in this branch, so treat the skill and pre-existing documentation as unreliable for it. (Confirmed in-branch while writing this plan: the DB-1 enum-vs-string bug is still live at `OmnichannelActivityStore.cs:210`, which casts `(int)ActivityStatus.*` into raw SQL while the index compares the enum by name.)
