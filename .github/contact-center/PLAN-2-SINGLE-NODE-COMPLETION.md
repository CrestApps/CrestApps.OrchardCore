...
> - **Verify Omnichannel against current branch source, not the `orchardcore-omnichannel` skill or older docs** — that module has changed substantially in this branch, so treat the skill and pre-existing documentation as unreliable for it. (DB-1 **resolved at root cause**: the enum index columns were declared `.Column<string>(...WithLength(50))`/`.Column<int>` even though the index properties are enums. YesSql persists the enum's underlying **integer** regardless, so on SQLite the `(int)ActivityStatus.*` raw SQL at `OmnichannelActivityStore.cs:210` matched via type affinity, but `varchar IN (<int literals>)` does **not** coerce under Npgsql — a latent Postgres defect. Fix: declare every enum index column with its **enum type** (`.Column<ActivityStatus>`, `.Column<CallbackRequestStatus>`, etc.) so YesSql creates an **integer** column on **all** providers, making the raw SQL and the type-safe LINQ predicates consistent everywhere. Applied to the Omnichannel activity/batch `CreateAsync` and every new Contact Center index migration; Telephony already followed the convention. All query consumers use LINQ (enum→int) or the existing `(int)` raw SQL — no name-based SQL in services — so the change is ORM-transparent. `OmnichannelActivityStoreTests` now reads the raw column back as integer `0` and stays 2/2 green.)
...
1. **Single-node is the immediate delivery and verification target — but the platform is distributed by construction.** Every service is built on Orchard Core's distributed primitives (`IDistributedCache`, `IDistributedLock`, `OrchardCore.Redis`, the tenant-qualified SignalR backplane), so it is *distributable* by design and is not rewritten later. For this phase we deliver and **fully test on a single node** — reliable, distributable, and secure — using Docker containers orchestrated by the **`CrestApps.Aspire.AppHost`** with **`OrchardCore.Redis` enabled**, so the distributed code paths (distributed cache, distributed locks, Redis SignalR backplane) are actually exercised on that one node rather than bypassed. **Multi-node testing and high-availability hardening are a later phase** (Part 8, which feeds `PLAN.md` R8).
...
**What "single-node" means here.** One application node, but the distributed stack is **switched on**: the Aspire host runs Redis and the reference tenant enables **both `OrchardCore.Redis` and `CrestApps.OrchardCore.SignalR.Redis`**, so distributed cache, distributed locks, and the tenant-qualified Redis SignalR backplane are exercised and their Redis-backed implementations are **asserted active** — only *multiple application nodes* are out of scope for this phase. **One-node Redis validates configuration/client compatibility and single-writer behavior, not cross-node lock/backplane semantics; Part 8 remains the only proof of multi-node correctness.** Two mandatory single-node CI lanes are therefore required (Part 0): (1) **Redis-on** — proves the distributed implementations load and the call/monitor/record paths work with the backplane; (2) **Redis-off** — proves the same DB-CAS locks, cache, SignalR, call lifecycle, and tenant isolation are correct with the Redis features **disabled at startup** (a valid single-node production configuration — this is *feature-off*, **not** automatic failover when a running Redis dies). Nothing in this plan removes or weakens the distributed design; it defers only the multi-node *tests*.
...
**Do not break the existing architecture.** Every change is an **additive extension of existing Orchard Core and existing module patterns** — it must not regress the current multi-tenancy architecture, the CRM → Contact Center → Telephony layer boundary, or existing Omnichannel/Telephony behavior and public contracts. Extend existing seams rather than replace them; regressions are caught by keeping existing contracts stable and by the two-tenant tests (Part 6). Because the Omnichannel module has changed heavily in this branch, its integration points are verified against current source, not the `orchardcore-omnichannel` skill.
...
Post-GA:            Part 8 (multi-node testing + distributed hardening, feeds PLAN.md R8) · Part 9 (future architecture)
```
...
- **Extend the existing `CrestApps.Aspire.AppHost`** (the single-node reference/test host) instead of building a parallel stack. It already orchestrates **Redis**, an **Asterisk** container (with `ari.conf`/`http.conf`/`extensions.conf` bind mounts), the CMS (`OrchardCore__OrchardCore_Redis__Configuration`), and an **`AsteriskWeb`** inbound/two-party call simulator (`/api/contact-center/voice/inbound`). Add to it: **wire the Aspire-injected Redis endpoint** — today `.WithReference(redis)`/`.WaitFor(redis)` are commented out and the Redis configuration is hardcoded to `localhost` (`Program.cs:27-35`); use the injected endpoint, not a literal host — and **enable both `OrchardCore.Redis` and `CrestApps.OrchardCore.SignalR.Redis`**, asserting at startup that the Redis-backed `IDistributedCache`/`IDistributedLock`/backplane implementations are the active ones. Replace the dev image **`andrius/asterisk:latest` (`Program.cs:20`) with a digest-pinned Asterisk 22.x image — build an owned/controlled image if any required module is missing** — carrying `res_http_websocket`, `res_pjsip_transport_websocket`, `res_pjsip`, `res_crypto`, `codec_opus`, and add **startup smoke checks** that assert WSS, the PJSIP WebSocket transport, crypto/DTLS, and **Opus (build/architecture-dependent — verify, do not assume)** are actually present, with a real registration/audio check. Add **coturn**; commit WebRTC config templates (`pjsip.conf` WSS/DTLS-SRTP, `http.conf`, `ari.conf`, Stasis `extensions.conf`) with **tenant-namespaced** contexts/app names (CC-1 — replacing the current global `crestapps-telephony`/`crestapps-dashboard` names). A plain `docker-compose` variant is kept for headless CI where Aspire is unavailable.
...
- **Two-tenant adversarial harness (from Model A) — enforced by explicit assertions, not prose.** Provision two tenants that share one Asterisk + one coturn, each with **distinct generated application/dialplan-context/endpoint namespaces** and **`subscribeAll=false`**, and seed identical *logical* IDs. Prove isolation through the **real ARI Stasis path**, not the `AsteriskWeb` inbound simulator (which posts pre-normalized events to `/api/contact-center/voice/inbound` and therefore cannot prove listener attribution or the persisted channel→tenant binding). Assertions must include a **negative cross-application/cross-tenant event test** — tenant B's ARI events must never be accepted, routed, bridged, monitored, or recorded by tenant A — and must **fail if the global dev defaults are in effect** (today `Program.cs:74,108` use global app names and `AsteriskSettingsUtilities.cs:172,199` set `subscribeAll=true`; the harness must reject these). Built here in Part 0 so it can be used as a **Part 3 acceptance gate**, then enforced in CI in Part 6.
...
**Exit:** `dotnet run` on the Aspire AppHost (or the `docker-compose` CI variant) yields Asterisk+coturn+Redis+CMS on one node with the distributed primitives **active and asserted**; **both** the Redis-on and Redis-off single-node CI lanes run; the tone-based E2E scenario and the two-tenant harness exist (red); baseline pinned.
**Node scope:** single-node (distributed stack enabled). **Depends on:** none.
...
- **DB-1 enum column-type correctness (resolved at root cause):** the enum index columns were declared `.Column<string>(...WithLength(50))` (and `CallbackRequestIndex.Status` as `.Column<int>`) even though every backing index property is an enum. YesSql persists the enum's underlying **integer** regardless of the declared column type, so on SQLite the `(int)` raw-SQL literals matched via type affinity — but `varchar IN (<int literals>)` does **not** auto-coerce under Npgsql, so DB-1 was a latent Postgres defect, not just a lane item. **Fix (per the YesSql convention): declare each enum index column with its enum type** — `.Column<ActivityStatus>`, `.Column<ActivityKind>`, `.Column<ActivityAssignmentStatus>`, `.Column<ActivityUrgencyLevel>`, `.Column<ActivityInteractionType>`, `.Column<CallbackRequestStatus>`, `.Column<ContactCenterCallState>`, `.Column<InteractionChannel/Direction/Status>`, `.Column<QueueItemStatus>`, `.Column<InteractionPriority>`, `.Column<ReservationStatus>`, `.Column<AgentPresenceStatus>`, `.Column<OutboxMessageStatus>`, `.Column<ProviderCommandStatus>`, `.Column<ProviderWebhookInboxStatus>`, `.Column<OmnichannelActivityBatchStatus>` — which makes YesSql create an **integer** column on every provider. This aligns the `(int)ActivityStatus.*` raw SQL at `OmnichannelActivityStore.cs:210` and the type-safe LINQ predicates (`BuildBulkManageableContextAsync`, `BulkManageActivityFilterHandler`, `DefaultContactActivityBatchLoader`) so they are correct on SQLite **and** Postgres. Scope was limited to `CreateAsync` for the **two released** Omnichannel migrations (`OmnichannelActivityIndexMigrations` and `OmnichannelActivityBatchIndexMigrations`, both shipped in `v2.0.0-rc.1`): new installs get correct integer columns on all providers; their historical `UpdateFrom` `AddColumn<string>` steps are left immutable because SQLite cannot `ALTER COLUMN` type, upgraded SQLite installs keep working via affinity, and rare Postgres rc.1 installs can reset pre-GA — no new data migration was added (honors the no-migration directive). **Existing PostgreSQL preview installs keep their `varchar` enum columns and must either reset (acceptable pre-GA) or run a provider-specific `ALTER TABLE … ALTER COLUMN … TYPE integer USING …` in the Part 8 multi-provider lane; this is called out explicitly there rather than shipped as an unportable in-place migration.** All new Contact Center index migrations were unreleased, so their `CreateAsync` was corrected directly. The two unreleased intra-branch v1→v2 upgrade paths (`ActivityReservationIndexMigrations.UpdateFrom1Async` and `QueueItemIndexMigrations.UpdateFrom1Async`) had their raw-SQL status comparisons switched from enum **names** to the invariant **integer** strings YesSql actually stores, so the legacy-row preflight and claim-key backfill match real rows on SQLite and Postgres alike; `ContactCenterClaimMigrationTests` was updated to seed legacy rows as integer-as-text (the faithful YesSql representation) instead of enum names, and its four migration tests stay green. Verified: full build **0/0**; `OmnichannelActivityStoreTests` seeds via the real `session.SaveAsync` path, asserts the raw `Status` column's SQLite storage class is `integer`, and reads back `(int)ActivityStatus.NotStated` (2/2 green); Omnichannel/ContactCenter/Telephony suite **1125/1125**. Both this fix and the earlier CreateAsync pass were independently confirmed by two gpt-5.6 reviews.
...
## Part 8 — Multi-node testing & distributed hardening (SECONDARY; feeds `PLAN.md` R8)

**Findings:** DIST-5, DIST-6, DB-2, DB-3 (High); DB-4 (Medium). **All built on Orchard Core primitives only (CC-1).** The distributed *code* (distributed cache, distributed locks, Redis SignalR backplane) already ships and is **exercised on the single node** (Part 0 runs with `OrchardCore.Redis` enabled). What is deferred to this phase is **testing and hardening across more than one application node** — not the distributed implementation itself.
...
- **DB-1 residual (Postgres/SQL Server upgrade of pre-GA previews):** new installs already get integer enum columns on every provider (fixed in Part A). Any pre-GA **PostgreSQL** preview that installed the two released Omnichannel migrations under the former `varchar` declaration keeps `varchar` enum columns (SQLite cannot `ALTER COLUMN` type, so no portable in-place migration was shipped). This lane must either (a) document reset as the supported path for pre-GA previews, or (b) add a **provider-guarded** `ALTER TABLE … ALTER COLUMN "Status"/"Kind"/… TYPE integer USING "<col>"::integer` step (Postgres/SQL Server only; SQLite is left as-is because affinity already makes it correct) and cover it with a Testcontainers upgrade assertion alongside TEST-4.
...
- 2026-07-16 — Product-owner confirmation folded in (no scope change): the plan keeps using Orchard Core's distributed cache/lock primitives but is **fully tested single-node only**; multi-node testing is deferred to Part 8. Part 0 now **extends the existing `CrestApps.Aspire.AppHost`** (Redis + Asterisk + `AsteriskWeb` simulator already wired) and runs with **`OrchardCore.Redis` enabled**, so the distributed paths are exercised on one node via Aspire Host and a `docker-compose` CI variant. Reaffirmed CC-1: changes are additive and must not break existing multi-tenancy/Orchard Core architecture. Recorded that the `orchardcore-omnichannel` skill is unreliable for this branch (DB-1 re-verified in-branch at `OmnichannelActivityStore.cs:210`).
- 2026-07-16 — Aspire/single-node refinement re-ratified. Both `gpt-5.6-terra` and `gpt-5.6-sol` returned **RATIFY-WITH-CHANGES** and independently converged on the same four must-fixes, now incorporated: (1) two mandatory single-node CI lanes — **Redis-on and Redis-off** — with "Redis-absent" clarified as *features-off at startup*, not auto-failover, and **`CrestApps.OrchardCore.SignalR.Redis` enabled/asserted** alongside `OrchardCore.Redis`; (2) **actually wire the Aspire-injected Redis endpoint** (`.WithReference`/`.WaitFor` are commented out; config hardcodes `localhost` at `Program.cs:27-35`); (3) Part 0 tenant isolation is an **explicit assertion via the real ARI Stasis path** (distinct per-tenant app/context/endpoint namespaces, `subscribeAll=false`, a negative cross-tenant event test, and rejection of the global dev defaults — the `AsteriskWeb` inbound simulator is not accepted as attribution proof); (4) **digest-pin/own the Asterisk 22.x image** (replace `andrius/asterisk:latest`) with startup smoke checks for WSS/PJSIP-WS/crypto/Opus. **Both models RATIFY with these applied.** Still no production code changed.
- 2026-07-17 — **Wave 1 complete and pushed** (`02c4140a..9edc4efd` on `ma/add-contact-center`), delivered as five scoped commits: **Part 0 media-runner infra** (Aspire Redis wiring + digest-pinned Asterisk/coturn + `docker-compose` CI variant); **Part A typed CAS call-state authority** (single-writer state, YesSql document-version concurrency, tenant-scoped unique agent-session claim via `AgentSessionIndexMigrations` v2, atomic enqueue, retained webhook tombstones) with store-backed StateAuthority + concurrency tests and reconciled superseded unit tests; **Part 1a fail-closed soft-phone call-ownership authorization (SEC-1/BOLA)** + `MergeRequest.GetCallIds` null-guard + shell-scoped hub authz tests; **DB-1 SQLite regression guard**; **Part 0 two-tenant adversarial isolation harness** + `[Skip]`-gated WebRTC audio-proof scaffold (shared logical IDs namespaced per scenario to respect Part A's one-session-per-user-per-tenant claim). Verification (single-node): full solution build **0 warnings / 0 errors**; main suite **1125/1125**; DistributedTests StateAuthority **8/8**; FeatureActivationTests **12 passed / 2 skipped** (the 2 ARI tests stay skipped until Wave 2 Asterisk tenant isolation). CC-1 upheld — all additions extend Orchard Core's own primitives with tenant-qualified keys and no cross-tenant leakage. **Progress: ~1 of 6 waves (~17%).**
- 2026-07-17 — **DB-1 re-verified and re-scoped (correction to earlier framing).** Independent re-review found the original DB-1 statement ("enum columns store `.Column<string>` names but filters cast `(int)`") is **incorrect for SQLite**: YesSql persists the `ActivityStatus` index column as its underlying **integer**, so the raw column reads back as `"0"` and the `(int)` literals at `OmnichannelActivityStore.cs:210` match correctly and stay consistent with the type-safe LINQ predicates (proven by `OmnichannelActivityStoreTests`, 2/2 green). **No production code changed and no data migration was added** (honors the no-code-migration directive); only the SQLite regression guard remains. **DB-1 is therefore not a live SQLite bug.** It stays open solely as a **Postgres/multi-provider lane** item because `varchar IN (<int literals>)` is not guaranteed to coerce under Npgsql the way SQLite does — that lane must confirm parity or switch the predicate to `CAST(status AS INTEGER)`. Re-scoped under Part 8; plan body (intro note + Part A DB-1 bullet) updated to match.
- 2026-07-18 — **Wave 2 complete and approved for commit** (Parts 1b–1e + Part 2 — the browser-media centerpiece; 118 files, +8437/−56). **Part 1b–1e — shared call-control authorization boundary:** object authorization is now a single fail-closed boundary invoked by every entry point and verb (Telephony hub, Contact Center hub, supervisor endpoints, and each `IContactCenterCallCommandService`/executor path), resolving ownership from Part A's typed `CallSession`; clients submit a **logical/interaction id** and the server resolves the provider call/leg id (1b); **typed, server-resolved transfer destinations** with a premium/emergency deny-list, an RBAC-gated external allow-list, and per-transfer audit — no client-supplied raw E.164 (1c); **per-queue supervisor entitlement** enforced at both subscription and each operation (1d); **first-command authorization** for new manual dials — the server resolves caller identity + allowed originating endpoint + destination policy and creates an owned interaction/`CallSession` **before** provider execution (1e). **Part 2 — provider-agnostic browser media (WebRTC) + Asterisk adapter:** client-side `IBrowserMediaAdapter` SIP.js soft-phone (WSS SIP UA, DTLS-SRTP, ICE via coturn) wiring `getUserMedia`→`RTCPeerConnection`→remote `<audio>` sink with mute/hold/hangup bound to call state; server-side **PJSIP Realtime** endpoint/auth/AOR provisioning through a **durable, tenant-scoped credential lease store** (`AsteriskPjsipCredentialLeaseStore` over `IStore` with isolated committed transactions per mutation — the lease is committed BEFORE realtime provisioning and BEFORE the tenant lock releases, so there are no orphan realtime rows and the per-user cap is never bypassed), an `AsteriskPjsipCredentialIssuer` (issue/rotate/revoke/cleanup, per-user cap, `IDistributedLock`-guarded, best-effort tenant-keyed `IDistributedCache` projection with the **durable lease as the sole authority** — revocation and cleanup are fully resilient to a cache outage), sign-out/logout credential revocation wired into the ContactCenter availability middleware, a background cleanup task for expired/revoked leases, and ARI dialog termination on revoke; coturn + `pjsip.conf` WebRTC config templates; per-tenant availability (unconfigured provider is unavailable, never a host-startup crash). **Audit:** a single gpt-5.6-terra code-review agent red-teamed Wave 2 across **five rounds** — Gate 1 (lease durability/orphan-prevention/cap-integrity), Gate 2 (cache-independent revocation), Gate 3 (dial `ProviderCallId` conflict) — each round driving a concrete remediation, ending in **APPROVE** with no remaining Blocker/High findings. Verified single-node: full build **0 warnings / 0 errors**; full suite **2175 passed / 1 skipped / 0 failed** (the 1 skip is the `[Skip]`-gated WebRTC audio-proof E2E, unblocked in Wave 3). CC-1 upheld — every cache/lock key is tenant-qualified, the durable lease store is tenant-scoped via `IStore`, there is no static mutable per-tenant state, and all distributed paths use Orchard Core's own primitives. **Progress: ~2 of 6 waves (~33%).**
- 2026-07-18 — **DB-1 fixed at root cause via the YesSql enum column-type convention (supersedes the 2026-07-17 Postgres-lane deferral).** Product-owner guidance clarified that enum index columns must be declared with their **enum type** (`.Column<MyStatus>`), which YesSql maps to an **integer** column on every provider — the earlier `.Column<string>(...WithLength)`/`.Column<int>` declarations were the actual defect behind DB-1. Converted every enum index column to its enum type across the Omnichannel activity/batch migrations and all new Contact Center index migrations (`ActivityReservation`, `AgentProfile`, `AgentQueueMembership`, `CallbackRequest`, `CallSession`, `ContactCenterOutboxMessage`, `Interaction`, `ProviderCommand`, `ProviderWebhookInboxMessage`, `QueueItem`); `TelephonyInteraction` already followed the convention. This makes the `(int)` raw SQL and the type-safe LINQ predicates consistent on **SQLite and Postgres** — closing DB-1 for all new installs on every provider rather than deferring it. Only `CreateAsync` of the **two released** Omnichannel migrations (`OmnichannelActivityIndexMigrations`, `OmnichannelActivityBatchIndexMigrations`) was touched; their historical `UpdateFrom AddColumn<string>` steps are immutable (SQLite can't `ALTER COLUMN` type; upgraded SQLite installs work via affinity; existing Postgres rc.1 previews either reset or get a Postgres-only `ALTER COLUMN … TYPE integer USING …` in the Part 8 multi-provider lane) and **no new data migration was added**. The two unreleased intra-branch upgrade paths (`ActivityReservationIndexMigrations.UpdateFrom1Async` and `QueueItemIndexMigrations.UpdateFrom1Async`) had their raw-SQL preflight/backfill comparisons corrected from enum **names** to the invariant **integer** strings YesSql stores, and `ContactCenterClaimMigrationTests` was updated to seed legacy rows as integer-as-text (the faithful YesSql representation) — fixing a latent defect where the name-based comparisons would never match real YesSql-written legacy rows (terminal queue items treated as active; active reservation claims backfilled as inactive). `OmnichannelActivityStoreTests` now seeds via the real `session.SaveAsync` path and asserts the SQLite storage class of the raw `Status` column is `integer` (not `text`) and reads back `(int)ActivityStatus.NotStated`. Two independent gpt-5.6 reviews confirmed the CreateAsync pass and drove the migration/test correction (the first review's suggestion was initially deferred, then a second review reproduced YesSql's integer persistence and flagged the same latent bug in `QueueItem`, so both paths were fixed together). Verified single-node: full build **0 warnings / 0 errors**; Omnichannel/ContactCenter/Telephony suite **1125/1125**. CC-1 upheld (additive, tenant-scoped, no cross-tenant leakage).
- 2026-07-18 — **Wave 3 (Part 3 — executable connect + idempotent inbound routing) implementation complete, audited, and green.** **3a real server-side ACD:** `AsteriskContactCenterVoiceProvider.ConnectToAgentAsync` replaces the fail-closed placeholder with a real ARI originate/bridge — the agent leg originates into the tenant's own Stasis application with a **deterministic** channel/bridge id derived from the durable command id, waits on a **tenant-isolated readiness signal** (`AsteriskAgentChannelReadySignal`, not a static field) for the agent channel's confirmed `StasisStart` before bridging, detaches the caller from its holding bridge, and joins both legs into a mixing bridge; the connect is finalized by a single durable compare-and-set (`MarkConnectedAsync`) — **state never advances on the ARI HTTP accept** (CC-2). On no-answer/failure the attempt **compensates** (hang up the ringing agent leg, destroy the mixing bridge, leave the caller parked) and returns `agent_no_answer`. **3b idempotent inbound routing:** a raw inbound handler runs before the generic interaction lookup; the durable `Offering` channel→tenant binding created under the striped-lock atomic `CreateAsync` is the **exactly-once claim**, so a duplicate/reconnect `StasisStart` loses the create and performs no second answer/park/route (`AsteriskInboundCallOfferBridge`). **Teardown/reconciliation:** terminal `StasisEnd`/`ChannelDestroyed` unconditionally tear down mixing bridge + holding bridge + peer leg + both bindings in a `finally` (`AsteriskCallTeardownService`); `AsteriskInboundReconciler` (background task) recovers `Terminating`, aged `Pending`, and aged `Offering` legs, promoting still-routed callers via `IInboundVoiceInteractionProbe` rather than tearing down a live call; `AsteriskAriOutcomeClassifier` retains the durable record on ambiguous (null/5xx/timeout/cancel) ARI outcomes for age-gated re-probe. **CC-1 tenant isolation:** the shared `crestapps-telephony` default is removed — the Stasis application name is now **explicitly required and fails closed** (unset ⇒ provider unavailable, no listener, origination rejected), closing the two-blank-tenants cross-talk path; per-tenant app + `subscribeAll=false`, process-wide `AsteriskAriApplicationOwnershipRegistry` keyed on `(baseUrl, app)` with per-shell-generation reference-counted ownership (`AsteriskAriApplicationGate`), host-default collision guard, persisted per-tenant binding via isolated YesSql sessions, `StasisStart` origination-marker validation (also carried as a Stasis **app argument**, not only a channel var, so an owned outbound leg is never misclassified as inbound), and an ownership check before every mutation. **This session:** repaired the build left broken by the prior session — the six `IAsteriskChannelTenantBindingStore` test doubles were updated to the evolved interface (`CreateAsync` now returns `Task<bool>` for the exactly-once claim; added `HasAnyAsync`), and `AsteriskChannelTenantBindingIsolationTests` was updated for the store's new `(IStore, ShellSettings)` ctor via a `CreateBindingStore` helper. **Audit:** an independent gpt-5.6-class code-review agent red-teamed the full Part 3 diff against Part 3/CC-1/CC-2/CC-4 and returned **no Blocker/High findings** (one non-blocking informational note: ingestion is a single bounded serial channel rather than partitioned per-call, but it fails closed via disconnect+reconcile and never drops events). Verified single-node: full build **0 warnings / 0 errors**; main suite **2315 passed / 1 skipped** (the `[Skip]`-gated WebRTC audio-proof E2E); FeatureActivationTests **12 passed / 2 skipped**; DistributedTests **10 passed / 1 failed only for the missing `CONTACT_CENTER_REDIS_CONFIGURATION` env (Part 8 multi-node lane, not single-node)**. The two-tenant **state** adversarial gate passes (`TenantIsolationTests.TenantState_*`/`TenantOperation_*`). The two remaining `[Skip]`-gated ARI-attribution scaffolds (`AsteriskAriAttribution_*`, `AsteriskAriEvent_*`) assert a **local-mirror auto-namespace** model (`{tenant}-crestapps-telephony`) that does not match the shipped design — production isolates tenants by **operator-configured distinct app names + ownership registry + persisted binding + marker validation**, which are covered by concrete passing suites (`AsteriskAriApplicationOwnershipRegistryTests`, `AsteriskAriApplicationGateTests`, `AsteriskChannelTenantBindingIsolationTests`, `AsteriskRealtimeVoiceEventMapperTests`); these two scaffolds should be **retired or rewritten to the real registry/binding seams** in a follow-up rather than satisfied by inventing a contradicting auto-namespace. CC-1 upheld — every ownership/lock key is tenant-qualified, bindings are tenant-scoped via `IStore`, and the only process-wide static holds ownership tokens, not tenant data. **Progress: ~3 of 6 waves (~50%).**
- 2026-07-18 — **Wave 4 (Part 4) — recording sub-feature implemented.** Delivered the Asterisk **ARI bridge-recording** primitive and closed **DIST-8**. Added four tenant-scoped ARI client operations (`StartBridgeRecordingAsync` → `POST /bridges/{id}/record` with `ifExists=overwrite`/`terminateOn=none`, `PauseBridgeRecordingAsync` → `POST /recordings/live/{name}/pause`, `UnpauseBridgeRecordingAsync` → `DELETE /recordings/live/{name}/pause`, `StopBridgeRecordingAsync` → `POST /recordings/live/{name}/stop` with a guarded `GET /recordings/stored/{name}` for format/duration), reusing the existing `SendAsync`/`EnsureSuccessAsync`/`ReadJsonAsync`/`ResolveSettings` pattern and null-status→`OutcomeUnknown` ambiguity semantics; NotFound on stop is an idempotent no-op and a 409 on start is read back as the already-running live recording. `AsteriskContactCenterVoiceProvider` now implements `IContactCenterVoiceRecordingProvider` and advertises `Recording`: it resolves the **canonical conversation (mixing) bridge** from the tenant-owned channel binding (fail-closed per CC-1 when no owning binding exists), maps Recording→start-or-resume (unpausing when the reused recording was paused), Paused→pause, Stopped/None→stop, returns `OutcomeUnknown` only on ambiguous ARI outcomes (CC-2), and returns `recordingName`/`storageReference`/`format`/`retrievalPath` (+`durationSeconds` when ARI exposes it) metadata. **CC-2 outcome classification (post-audit fix):** recording no longer reuses the deliberately-conservative provisioning/record-retention classifier — a dedicated `IsRecordingOutcomeUnknown` reports `OutcomeUnknown` **only** for a genuine transport-ambiguous outcome (a null-status `AsteriskAriException` that wraps an inner transport failure — timeout/unreachable). A received 4xx/5xx (non-null status) and a definite local pre-flight rejection (null status with no inner: unconfigured provider or tenant not owning the ARI application, thrown by `ResolveSettings`/gate/`EnsureConfigured` before any request is sent) are now **confirmed failures** (`recording_failed`), matching the plan's CC-2 contract; the shared provisioning classifier used by the connect path is untouched. **Recording continuity = Model A** (stable canonical bridge); the recording name is derived deterministically from the globally-unique interaction id, so pause/resume/stop address the same live recording without extra state and it is inherently tenant-distinct. **DIST-8 closed:** `ContactCenterRecordingService` now persists the provider metadata onto the interaction's `TechnicalMetadata` and sets `RecordingReference` from the storage reference/recording name (previously discarded); no `Interaction` model or migration change was needed (existing `RecordingReference` + `TechnicalMetadata` fields suffice, neither is indexed). Governance unchanged: the support-matrix `recording` capability stays **prohibited** until the Part 6 audio + two-tenant proofs land (CC-5), so `ContactCenterSupportMatrixTests` is untouched. Deviation/omission: ARI's stored-recording resource does not expose a **checksum** and generally not a duration, so no checksum metadata is fabricated and `durationSeconds` is emitted only when present. Tests: new `AsteriskContactCenterVoiceProviderRecordingTests` (start/pause/resume/stop mapping on the resolved bridge, fail-closed with no owning binding, `OutcomeUnknown` on a transport-ambiguous ARI timeout, **confirmed-failure on a definite local pre-flight rejection**, confirmed-failure on a 4xx, idempotent stop on NotFound) + a Core `ContactCenterRecordingService` metadata-persistence fact; the six existing ARI test doubles were extended for the new interface methods. **Audit:** an independent gpt-5.6-terra code-review agent red-teamed the full recording diff against Part 4/CC-1/CC-2/DIST-8; it raised one High finding (the recording catch mislabeled received 5xx and local pre-flight failures as `OutcomeUnknown` by reusing the provisioning classifier), which was fixed as described above and **re-reviewed to APPROVE** with no remaining Blocker/High findings. Verified single-node: full build **0 warnings / 0 errors**; main suite **2325 passed / 1 skipped** (the `[Skip]`-gated WebRTC audio-proof E2E). Still open in Part 4: recording governance (consent/retention/erasure/failed-upload recovery + secure ARI file ingest), monitoring (snoop + supervisor audio leg), transfer, and conference. **Progress: Wave 4 recording primitive landed; ~3.5 of 6 waves.**
