...
- **DB-1 residual (reservation `UpdateFrom1` name-vs-int):** `ActivityReservationIndexMigrations.UpdateFrom1Async` still compares its raw-SQL `Status` against enum **names** (`ReservationStatus.Pending/Accepted.ToString()`), but `OmnichannelActivityStoreTests` proves YesSql persists the **integer** — so on a real (non-hand-seeded) legacy install the preflight/backfill would not match. Its existing test (`ContactCenterClaimMigrationTests`) hand-seeds name-valued rows, masking the mismatch. In this lane, switch the reservation `UpdateFrom1` comparisons to invariant integer strings **and** update that test's legacy seed to integer-as-text, validating both on the real-Postgres Testcontainers matrix (TEST-4) rather than on SQLite affinity alone.
