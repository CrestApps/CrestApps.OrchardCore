...
> - **Verify Omnichannel against current branch source, not the `orchardcore-omnichannel` skill or older docs** — that module has changed substantially in this branch, so treat the skill and pre-existing documentation as unreliable for it. (DB-1 correction after re-verification: YesSql persists the `ActivityStatus` index column as its underlying **integer** even though the migration declares it `Column<string>` varchar(50), so the `(int)ActivityStatus.*` raw SQL at `OmnichannelActivityStore.cs:210` is **correct and self-consistent on SQLite** — `OmnichannelActivityStoreTests` reads the raw column back as `"0"` and `ListBulkManageableAsync` returns exactly the editable set, 2/2 green. The original "compares the enum by name" premise was wrong for SQLite; production was left at HEAD with no data migration. **Postgres stays unverified** because `varchar IN (<int literals>)` does not auto-coerce under Npgsql the way SQLite does, so DB-1 survives only as a Postgres/multi-provider lane validation item, not a live SQLite bug.)
...
- **DB-1 enum-SQL consistency (re-scoped after verification):** the original premise was partly wrong. YesSql stores these enum index columns as the underlying **integer** (not the `.Column<string>` name), so the `(int)` raw-SQL literals are **verified correct and self-consistent on SQLite** — `OmnichannelActivityStoreTests` reads the raw `Status` column back as `"0"` and `ListBulkManageableAsync` returns exactly the editable set (2/2 green), matching the type-safe LINQ predicates elsewhere in the store. Production was left at HEAD (no code migration, honoring the no-migration directive). **Remaining work is Postgres-only:** confirm `varchar IN (<int literals>)` behaves identically under Npgsql (it does **not** auto-coerce like SQLite), and if not, make the predicate provider-agnostic (e.g. `CAST(status AS INTEGER)`) and add a Postgres-lane assertion for `BuildBulkManageableContextAsync`, `BulkManageActivityFilterHandler`, and `DefaultContactActivityBatchLoader`. Moved to the Postgres/multi-provider lane (Part 8); a SQLite regression guard is already in place.
