...
- 2026-07-13: Refined multi-call soft-phone conference and transfer behavior. The conference control appears only after exactly two active interactions are selected, active-call number/state share one compact row, the Keypad scrolls within a bounded height, and dial input clears after submission and when held-call dialing becomes available. Asterisk now clears persisted hold markers when channels enter a conference and identifies two-channel mixing bridges as conferences, preventing reconciliation from returning merged calls to On hold. Conference transfer remains hidden until one interaction is explicitly selected. Added the provider-neutral `Directory` capability and `ITelephonyDirectoryProvider`; Asterisk lists ARI endpoints and DialPad lists paginated company users for transfer destination selection.
