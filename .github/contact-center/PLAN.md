...
> - **Release plan (read this first):** [`PLAN-3-PRODUCTION-SINGLE-NODE-DISTRIBUTED.md`](PLAN-3-PRODUCTION-SINGLE-NODE-DISTRIBUTED.md) is the authoritative execution plan and progress ledger from 2026-07-26 onward. It supersedes the open items of this document (**R8**, **R9**) and of Plan 2 (Waves 5–6) for release scoping, and targets a production-supported `single-node-distributed` topology. This document remains the architecture reference and the record of completed work. Note that **R7 is re-opened** by Plan 3: it was marked complete against a declared-but-unused `ActivitySource`, and no distributed tracing exists.
...
- [x] Expand the R1 single-host backplane fixture into the minimal two-node Orchard/Redis-backplane/provider-listener harness needed by distributed R0b scenarios.
- [x] Define per-feature quiesce/drain/re-enable contracts and test inactive/idle enable-disable behavior before moving registrations. Defer active-call, pending-command, and in-flight-outbox quiesce proof to R3 after durable command state exists.
- [x] Run every finite GA-Core support-matrix profile through a fresh SQLite Orchard tenant, verify the exact manifest dependency closure, migrations and key services, prove idle provider disable/re-enable, and prove two tenants with different providers remain isolated in one host.
...
- [x] Add row versions, unique active constraints, and compare-and-set queue/reservation transitions.
- Add canonical `AgentAvailability` and fix disconnect/heartbeat/after-call recovery.
- [x] Persist accepted reservation and provider command intent before execution; add idempotent command ids and compensation.
- [x] Add the durable provider-command state machine (`Pending`, `Claimed/Fenced`, `Sent`, `OutcomeUnknown`, `Confirmed`, `Compensating`, terminal), query/reconcile before retry, and pause rather than redial when the outcome cannot be proven.
- [x] Add canonical provider identity, provider inbox uniqueness, provider-call uniqueness, sequence/high-water handling, alias migration/repair, and stable outbox handler ids.
- [x] Isolate poison messages and make every handler's idempotency contract explicit.
- [x] Define atomic domain-state-plus-outbox commits and idempotent cross-context sagas; eliminate inline provider/network dispatch from mutating requests.
- [x] Implement and test active-work quiesce/drain/re-enable for Voice, Dialer, RealTime, provider adapters, and outbox processing using the durable command/inbox state.
...
- [x] Add one listener owner/lease per tenant/provider stream or prove duplicate ingestion harmless; preserve monotonic provider truth.
- [x] Decouple PBX mutations from SignalR connection cancellation and use bounded server-owned command timeouts.
- [x] For GA-Core, fail closed and hide BidirectionalMedia unless it is in the approved support matrix. If media is included, define the secure RTP deployment boundary and implement/test sequence-aware reorder/jitter handling; otherwise defer transport certification to R9.

Exit: Asterisk and DialPad provider contract suites pass; two-node duplicate-stream tests, plus listener-lease-loss tests for any provider that chooses exclusive listener ownership, preserve one monotonic call state; unsupported capabilities cannot be invoked or displayed; any GA media capability has its separate certification.
...
- [x] Keep automated dialing modes disabled until their specific compliance and pacing tests pass.
...
- [x] Add query-aligned indexes and normalized queue membership/capacity data.
- [x] Replace broad loads/N+1 counts with bounded top-N queries and aggregate SQL.
- [x] Claim callbacks, reconciliation work, outbox items, and retention batches with bounded leases/checkpoints.
- [x] Add projection processed-event keys, atomic upserts, replay versions/checkpoints, rebuild, drift detection, and poison isolation.
- [x] Resolve enum/raw-SQL portability and validate every supported database.
...
- When non-voice channel adapters (SMS, EventGrid, and future channels) reach GA, bring their inbound webhooks to full ingress-control parity with the voice surface (body/header limits, rate/concurrency limiting, freshness/replay policy, durable at-least-once inbox, and 2xx only after inbox commit). Today those endpoints are authenticated (Twilio HMAC signature, Azure EventGrid auth + body cap) but do not use the durable inbox. **Target design to minimize future breaking changes:** promote the reusable ingress primitives to a channel-neutral shared home at or below Omnichannel (the limiter/freshness contract and options to `Omnichannel.Core`; the durable-inbox contracts to shared abstractions) so both the Contact Center voice path and the non-voice channels consume one implementation. The current voice inbox is intentionally coupled to Contact Center (`IContactCenterScopeExecutor`, `IProviderIdentityResolver`, the `ProviderWebhookInboxMessage` YesSql index + module migration), so the promotion is a deliberate expand-migrate-contract refactor — not a GA-Core change — sequenced only when the second (non-voice) consumer is actually built, per "prove a second channel before generalizing."
...
- [x] **R0 — Support contract, SLOs, ownership, and failure reproductions:** publish the finite GA matrix and PR-to-test ownership, pin baselines, complete R0a tests, and track R0b harness dependencies.
  ...
  - [x] PR-to-test control matrix for every P0/P1 gate published in `.github/contact-center/pr-test-control-matrix.v1.json` (42 gates: C001-C008, D001-D009, F001-F007, O001-O006, S001-S005, T001-T003, V001-V004), each resolving a DRI role, approver roles, test id, CI job, provider/database/topology context, falsifiable invariant, and retained evidence location, with contract tests. Most CI jobs remain planned; only the currently enforced/partially enforced gates are marked implemented/partial.
  - [x] R0a in-process/shared-database failure reproductions and feature-dependency architecture test.
    ...
    - [x] Rolling-version outbox-handler characterizations: registration reorder replays an already completed handler because the checkpoint includes its prior index, and one poison due item blocks later messages in the batch. These characterizations were inverted by the stable-handler and poison-isolation R3 bullets.
    - [x] Ambiguous contact attribution characterization inverted: inbound lookup now persists deterministic unresolved, resolved, or ambiguous attribution state without first-match fallback; ambiguous calls remain routable, the source-neutral disposition boundary rejects unresolved ambiguity, activity completion requires an explicit candidate selection and resource authorization, and unresolved, ambiguous, or legacy unknown inbound activities cannot execute contact-bound subject actions.
...
- [~] **R1 — Security and tenant isolation:** tenant-qualified SignalR, queue/campaign entitlements, secret/PII redaction, simulator containment, XSS correction.
  ...
  - [x] Soft-phone queue/campaign selectors register Orchard's named `bootstrap-select` and Contact Center resources from the display driver before footer rendering, then initialize only explicitly marked Work-tab controls so the scripts and picker markup are emitted once and list-management filters are not reinitialized.
- [x] **R2 — Orchard feature and package graph:** headless base and Availability, explicit optional/provider features, stable abstraction references, finite activation matrix, quiesce/re-enable behavior, and two-node harness.
  ...
  - [x] `CrestApps.OrchardCore.SignalR.Redis` now provides an explicit Orchard Redis-dependent, per-tenant channel backplane. A dedicated Linux CI transport gate boots independent Node A/Node B tenant-scoped hosts through the production backplane registration and proves a provider-listener event crosses nodes for the intended shell without reaching an equal user id in another shell. This is the minimal distributed fixture required by later scenarios, not the fresh-tenant activation matrix or full authenticated Contact Center R0b certification, which remain separate R2 and R8 gates.
  - [x] Feature-owned tasks, hubs, provider listeners, adapters, media providers, and Voice reconciliation now have versioned inactive/idle quiesce, drain, and re-enable contracts in `feature-lifecycle-contracts.v1.json`. A base pre-disable coordinator lazily resolves and invokes matching `IContactCenterFeatureLifecycleParticipant` implementations before Orchard mutates the feature descriptor, while fresh shells register bounded reconciliation tasks that execute in normal tenant scopes instead of performing provider or database work inside activation; tests pin feature filtering, quiesce-before-drain ordering, registration, fresh-scope reconciliation, and complete background-task ledger coverage. Active calls, connected clients, pending provider commands, claimed inbox/outbox batches, and open media sessions remain explicitly deferred to R3.
  - [x] The dedicated feature-activation project now boots isolated Orchard hosts with disposable App_Data, sets up fresh SQLite tenants from the Blank recipe, enables both finite GA-Core profiles with dependency expansion, asserts the exact CrestApps feature closure and key service/background-task resolution, proves clean idle provider disable/re-enable, and proves Asterisk and DialPad tenants coexist without provider leakage. Linux CI retains TRX evidence.
- [x] **R3 — Atomic consistency and agent lifecycle:** database CAS/constraints, canonical availability, after-call recovery, provider command outbox/compensation, provider inbox, stable handler ids.
  - [x] Queue-item, reservation, agent-profile, and CRM-activity stores now use YesSql document-version optimistic concurrency; queue and reservation lifecycle transitions commit through one compare-and-set path, and a losing writer aborts the scope rather than reusing YesSql's canceled session. Reservation creation revalidates current presence, pending ownership, and interaction capacity under activity-then-agent locks. Portable non-null claim keys enforce one active queue item per activity, one pending/accepted reservation per activity, and one pending reservation per agent while allowing terminal history. Upgrade migrations preflight missing identifiers and duplicate legacy claims with actionable repair failures before adding the unique indexes. Shared-SQLite tests force overlapping lock holders and synchronized stale reads, prove exactly one persisted reservation, verify capacity/presence revalidation, exercise conflict scope abortion, test valid and invalid v1-to-v2 migrations, and verify each unique-active constraint plus terminal-history insertion. This bullet closes relational state authority only; fencing voicemail/reject/provider side effects behind durable command intent remains in the following R3 command bullets. PostgreSQL/SQL Server certification remains in the R6 database matrix rather than being inferred from SQLite.
  - [x] Availability now owns the canonical routing projection instead of treating `AgentProfile.PresenceStatus` as sufficient authority. Candidate selection joins administrative `Available` presence and queue entitlement with session queue opt-in, online connection state, heartbeat freshness, and active-interaction capacity through batched YesSql reads; reservation creation repeats the single-agent canonical check under the activity-then-agent transition locks. The last disconnect and stale heartbeat therefore stop new offers immediately while preserving the administrative presence during transient reconnects until stale-session cleanup performs durable sign-out. Tenant-scoped `HeartbeatTimeout` and `MaximumWrapUpDuration` policy is validated when Availability starts, and an idempotent minute task completes expired/orphaned interaction wrap-up timing and restores pending/default presence without completing CRM disposition. Focused tests cover missing/offline/disconnected/stale/wrong-queue sessions, heartbeat refresh, capacity exhaustion, the selection-to-reservation disconnect race, deadline recovery, and orphan recovery.
  - [x] Automated outbound attempts now accept and commit the reservation before creating a provider command, then persist the interaction with a stable command id before provider execution. The command id is carried explicitly and in provider metadata. Failed reservation acceptance never creates an interaction or invokes a provider; failed command-intent persistence compensates through a fresh Orchard child scope instead of reusing the canceled YesSql session; definitive provider failures compensate reservation, queue, activity, and agent ownership in one reservation-and-agent-fenced commit without disturbing newer work. Focused tests pin accept-before-dispatch order, no-dial behavior after lost reservation ownership, persisted-command order and identity, fresh-scope commit-failure compensation, pending and accepted ownership release, newer-owner isolation, and provider-failure cleanup. Durable command claiming, `OutcomeUnknown`, reconciliation-before-retry, and provider-side fenced compensation remain in the next R3 bullet.
  - [x] Durable provider-command state machine: adds durable per-tenant command identity, distributed registration serialization, optimistic concurrency, fenced dispatch, reconciliation, and compensation leases, stable idempotency metadata, and stale-owner handoff without reporting a safe-to-redial failure. Commands transition through `Pending`, `Claimed/Fenced`, `Sent`, `OutcomeUnknown`, `Confirmed`, `Compensating`, and terminal states. Provider transport failures and ambiguous HTTP 408, 429, and 5xx responses are classified as `OutcomeUnknown` and require reconciliation before retry. Unknown and terminal confirmation settlement commit atomically with interaction and activity projections. Pending crash recovery revalidates the current dialer profile and outbound compliance policy, failing closed without provider execution when the policy feature, profile, activity, or eligibility is unavailable. Recovery isolates every CAS-sensitive command in a fresh Orchard child scope so a normal optimistic-concurrency ownership loss cannot poison the remaining batch. A bounded minute recovery task uses due-time and expired-lease indexes and attempts to query the provider by command id; however, because current providers (DialPad, Asterisk) cannot query by command key, recovery safely defaults to `Paused` rather than risking blind redial. Fresh-tenant activation proves the Voice feature resolves its command manager, state machine, processor, index provider, migration, and recovery task with only its declared dependency closure. Explicit deferral: atomic domain-state-plus-outbox commits and eliminating inline network dispatch remain in the next separate R3 bullet.
  - [x] Canonical provider identity, portable uniqueness, sequence/high-water ordering, and stable outbox handler ids: provider modules now contribute canonical identities through the implementation-free `IProviderIdentityProvider` contract (Asterisk maps the `Default Asterisk` alias to `Asterisk`; DialPad declares its canonical name) and Contact Center Core resolves them through `IProviderIdentityResolver` before it builds any inbox, event, or call key, so aliases no longer mutate stored provider identity and Core never references provider assemblies. Provider voice events carry an optional monotonic `SequenceNumber`; `CallSession` persists `HighWaterSequence`, and ingestion rejects stale or equal-order deliveries by sequence when present and by a lifecycle-rank timestamp guard otherwise, so equal-timestamp events can no longer regress a more-advanced call while providers that only supply timestamps/idempotency keys stay safe. Portable, non-null claim-key columns plus canonicalizing preflight/repair upgrade migrations now database-enforce one inbox message per canonical provider delivery, one call session per canonical provider-call identity, and one interaction event per provider-scoped idempotency key (canonical provider plus raw delivery id, so identical raw ids from different providers never collide); the migrations resolve legacy aliases through `IProviderIdentityResolver` before duplicate preflight and unique-index creation, the index providers canonicalize `ProviderName` on reindex, runtime lookups resolve to canonical values, and the migrations fail actionably on irreconcilable duplicates rather than deleting business data. `CallSessionStore` enforces optimistic concurrency so concurrent provider-event ingestion is compare-and-set safe: a losing `ConcurrencyException` is treated as ownership loss that never reuses the canceled session and reprocesses the delivery in a fresh scope. The outbox now checkpoints handlers by an explicit stable versioned `HandlerId` (independent of CLR name/assembly version/registration order) with a deploy-safe legacy `{AssemblyQualifiedName}:{index}` alias/repair path, and construction fails fast on a null, blank, or duplicate `HandlerId`, so a rolling deployment, reorder, or duplicate id never replays or skips a handler. Real-SQLite migration/concurrency tests, inverted characterizations for alias mutation, equal-timestamp regression, and reorder replay, and fresh-tenant activation resolution of `IProviderIdentityResolver`, `IProviderVoiceEventService`, and `IProviderWebhookInbox` cover the change. Explicit deferral: poison-message isolation and per-handler idempotency-contract documentation remain in the next separate R3 bullet.
  - [x] Poison-message isolation and explicit replay-safety contracts: domain-event outbox and provider-webhook inbox retry passes isolate every due message in a fresh Orchard child scope, continue past failed scopes, and preserve pending work for later retry. Event handlers run in nested isolated scopes so a failed effect and its replay marker roll back together without canceling the message session. Every `IContactCenterEventHandler` and `IProviderWebhookInboxHandler` declares `ContactCenterHandlerReplaySafety`; construction rejects `Unspecified`, blank identities, and duplicate identities. Metrics and Workflows handlers use a durable unique `(HandlerId, EventId)` processed-event marker, while daily metric rows add optimistic concurrency and portable `(DateKey, EventType)` uniqueness so replay and concurrent distinct events cannot double-count or lose increments. Focused tests invert poison-blocking behavior, preserve stable checkpoints, validate contracts, and prove replay suppression. Versioned projection checkpoints, rebuild operations, and drift detection remain an R6 gate.
  - [x] Atomic domain-state-plus-outbox persistence and idempotent cross-context sagas: mutating inbound, outbound, reservation-timeout, answer, reject, and voicemail paths now persist typed provider-command intent instead of invoking a provider or offer transport inline. Activity, interaction, provider command, immutable event, and event-outbox state commit in one tenant transaction; rollback and duplicate-idempotency persistence tests prove no partial durable boundary escapes. Typed Dial, Answer, Reject, and SendToVoicemail executors validate payloads and current policy, execute only after commit, and settle projections through owner/fence-validated fresh scopes. Definitive rejection and voicemail failures durably reoffer still-live work, while ambiguous outcomes remain isolated for reconciliation. Domain-event outbox and provider-webhook inbox claims are committed before external handlers run, and terminal/retry settlement reloads the durable row in a new scope and verifies the owner and fencing token so an expired worker cannot settle or delete work reclaimed by another node. Feature activation no longer performs provider/database reconciliation inline, and lazy lifecycle/reoffer resolution removes optional-feature and event/outbox/router DI cycles. The strict Contact Center matrix passes 715 tests, and all eight fresh-tenant feature profiles pass with exact typed-executor cardinality.
  - [x] Active-work feature lifecycle: a tenant-shell singleton admission manager closes new work before descriptor mutation and tracks disposable leases for admitted work. The coordinator quiesces every matching participant before draining any and surfaces bounded drain failures; because Orchard logs non-fatal feature-event exceptions and continues descriptor mutation, admission remains closed through teardown rather than being reopened, and durable owner/fence recovery is the correctness boundary for work that outlives the drain. Base outbox dispatch, Dialer callbacks, Automated Dialer pacing, Voice endpoints/routing/reconciliation/provider-command recovery, Contact Center Real-Time connections, Asterisk and DialPad Contact Center adapters, and Asterisk media sessions are gated by their owning feature. Real-Time quiesce aborts active Contact Center hub connections; media sessions retain their lease until cleanup succeeds; a provider adapter that rejects before provider contact returns the command from `Sent` to delayed `Pending`; durable inbox/outbox rows retain owner/fence recovery; and unavailable expected outbox handlers neither count as delivered nor consume the dead-letter attempt budget. The lifecycle timeout is validated under `CrestApps_ContactCenter:FeatureLifecycle:DrainTimeoutSeconds` with a 30-second default and 1-300 second range. Focused tests cover admission rejection, drain waiting/timeout, participant ordering, connection abortion, provider/endpoint/dialer gates, fenced media cleanup, command deferral, and expected-handler retention. The tenant-shell-local gate composes with existing relational fencing; multi-node crash and rolling-deployment certification remains in R8.
- [x] **R4 — Executable provider capabilities and high availability:** recording/monitoring contracts, provider stream ownership, monotonic events, command cancellation, media transport hardening.
  ...
  - [x] Voice provider metadata is split from executable call-control, queue/assignment, transfer/conference, recording, monitoring, and media contracts. Recording, monitoring, and Contact Center transfer require the advertised capability plus the matching executable provider contract, apply state only after provider-confirmed success, reject unknown outcomes, and expose only executable supervisor controls.
  - [x] Duplicate provider streams are harmless in the supported Redis-locked multi-node topology. Provider event ingestion serializes each canonical provider-call stream with a tenant-scoped hashed lock, commits the YesSql state/outbox transaction before releasing the lock, rejects lifecycle-rank regression regardless of receipt time, and rejects unsequenced events after a provider sequence high-water is established. Focused two-node tests prove one update/publication.
  - [x] PBX mutations are independent of SignalR/request cancellation and bounded by the validated `CrestApps_Telephony:Commands:Timeout` server deadline. The boundary returns when providers ignore or swallow cancellation, links to host shutdown, settles sent durable commands as `OutcomeUnknown` on timeout, and persists provider-confirmed interaction, recording, monitoring, transfer, and event state with a non-request token. Voice-only and Recording-only fresh tenants resolve the executor through declared feature dependencies.
  - [x] GA-Core media fails closed. The unused `BidirectionalMedia` capability flag is removed; Contact Center Voice Media and Asterisk Contact Center Media are dependency-only, hidden from direct feature selection, absent from every approved profile, and unresolved in fresh GA tenants. Asterisk External Media remains a non-GA server-side RTP/UDP development foundation and must not be represented as browser WebRTC. Secure network-boundary, loss/reorder/jitter, capacity, failover, and node-affinity certification is explicitly deferred to R9.
- [x] **R5 — CRM attribution and regulated outbound:** ambiguous-contact workflow, terminal unroutable records, calendar-based calling windows, the abandonment/safe-harbor/AMD compliance policy, and the automated-mode feature gates are complete. Automated Power/Progressive dialing stays behind the non-GA `ContactCenter.Dialer.Automated` feature (guarded by a support-matrix test) until pacing certification.
...
- [ ] **R7 — Operations, privacy, and upgrades:** OpenTelemetry, health/readiness, SignalR backplane, retention/erasure, backup/restore, expand-contract migrations, failure runbooks. Re-opened by PLAN-3: the Engineering Review Board found the operations posture unproven, so R7 is tracked to completion in `PLAN-3-PRODUCTION-SINGLE-NODE-DISTRIBUTED.md` (waves W3, W4, W10).
...
- [~] Phase 14 — Advanced capabilities (**AI assist seam shipped 2026-07-01:** `IContactCenterAssistProvider` + `IContactCenterAssistService` orchestrate optional summarization + disposition-suggestion providers by order, decoupled from any specific AI provider. **AI voice media foundation shipped 2026-07-13:** the optional `IContactCenterVoiceMediaProvider`/`IContactCenterVoiceMediaSession` contracts describe negotiated PCM/G.711 formats and asynchronous incoming/outgoing frames; executable media registration is authoritative; and Asterisk supplies a non-GA ARI External Media RTP/UDP development adapter with G.711 mu-law framing, bridge attachment, and cleanup. **Transport validation added 2026-07-13:** scripted ARI lifecycle tests cover bridge reuse/creation, External Media parameters, endpoint resolution, owned-resource cleanup, cancelled-open cleanup, and cleanup-failure exception preservation; loopback UDP tests cover RTP sender filtering, malformed-packet rejection, receive/send framing, sequence/timestamp continuity, and stop semantics; and an automated outbound reminder batch verifies call-insight and campaign-summary report aggregation. **Live Asterisk bridge simulation shipped 2026-07-13:** the standalone dashboard originates two configurable Stasis endpoints, waits for both channels, connects them through a real mixing bridge, reports the created resources, and cleans up partial failures. The Aspire dialplan provides two synthetic tone-producing Local endpoints, and the same UI accepts registered PJSIP endpoints for two-device testing. A live container run confirmed four Local channel legs, two connected logical calls, and one shared mixing bridge. DialPad remains unavailable for AI media. R4 removed the unused public media capability flag and hid the dependency-only media features from GA; secure boundary, loss/reorder/jitter, capacity, failover, and node-affinity certification is deferred to R9. Remaining: transport-neutral AI conversation engine, no-human-reservation AI dialer path, media-session recovery/capacity, concrete AI provider (summaries/disposition/sentiment), virtual-agent handoff, and AI routing recommendations.)
...
- 2026-07-16: Started PLAN-2 Part 2 browser media implementation for Asterisk. The soft phone now has a page-local SIP.js browser-media adapter that fetches `SoftPhoneRegistrationConfig`, registers over WSS, attaches microphone tracks to the SIP peer connection, renders remote audio, and follows authoritative mute/hold/hangup state without routing agent RTP through .NET. A provider-agnostic registration-config contributor contract and Asterisk contributor issue tenant/session/expiry-bound browser SIP credentials; the Asterisk lifecycle explicitly chooses PJSIP Realtime over static pre-provisioning because ARI cannot create endpoint/auth/AOR objects and static pools cannot bind each browser credential to tenant, session, and expiry. The Realtime store writes endpoint/auth/AOR rows through configured ADO.NET storage, revocation removes those rows and best-effort terminates matching browser SIP channels, Asterisk browser audio now advertises only when WebRTC plus Realtime settings are executable, Contact Center delivery is server-side ACD, and reference `pjsip`/coturn templates document `direct_media=no`, DTLS-SRTP, ICE, and coturn REST credentials. Integration reconciled the shared boundary and layering with Part 1, added a scheduled per-tenant credential cleanup background task plus orphaned-credential reclamation, and the strict solution build and full test suite both pass green.

- 2026-07-16: Audited R1 for any non-infrastructure-gated remainder and pinned the forward design for non-voice webhook ingress to minimize future breaking changes. Verified in source that the two highest-severity audit candidates are false positives: `ContactCenterHub` joins only tenant-qualified groups via `GetQueueGroup`/`GetSupervisorsGroup` (which wrap `TenantSignalRGroupName.ForGroup(_tenantName, …)`), and every dynamic value written through `innerHTML` in the agent-workspace, supervisor-dashboard, and soft-phone scripts is escaped through `escapeHtml(...)` (only numeric counts are interpolated raw). The one genuine observation is that the non-voice Omnichannel webhooks (Twilio SMS, Twilio EventGrid, Azure EventGrid) are authenticated (Twilio HMAC signature; EventGrid auth + body cap) but do not use the durable ingress-control stack that the voice endpoints have. A full promotion of that stack now is not safe: the durable inbox is intentionally coupled to Contact Center (`IContactCenterScopeExecutor`, `IProviderIdentityResolver`, the `ProviderWebhookInboxMessage` YesSql index + module migration), so relocating it would move a shipped database migration and destabilize the completed, tested voice path for a non-voice (R9) surface. Recorded the target channel-neutral shared-abstraction design as a tracked R9 item so the eventual move is a planned expand-migrate-contract refactor rather than a late-discovered breaking change. R1 code is complete for its GA-Core voice scope; R1 stays `[~]` only for infrastructure-gated exit evidence (production-backplane two-tenant isolation, multi-database execution, real socket disconnect after commit).

 Added a code-level, single-source-of-truth `ContactCenterDataGovernanceCatalog` (in Core) classifying all fifteen persisted Contact Center data categories — interaction event log, interaction, call session, callback request, agent session/profile, event outbox, provider webhook inbox, provider command, queue item, activity reservation, event metric, projection checkpoint, processed-event ledger, and routing/dialing configuration — each with a `ContactCenterDataSensitivity` (NonPersonal/Personal/SensitivePersonal), a `ContainsRecordingReference` flag, a retention basis, and a `ContactCenterErasureStrategy` (NotApplicable/RetentionExpiry/Anonymize/CascadeWithInteraction/ExternalStore), plus a `TryGet` lookup. Grounded the classification in the real model (Interaction and CallSession carry only an opaque `RecordingReference`/`RecordingState`; Interaction holds `CustomerAddress`; CallSession holds from/to addresses). Documented the authoritative per-entity governance table, the erasure-strategy semantics, the recordings-are-external access-audit and delegated-erasure policy, and the SQL backup/restore + projection-rebuild guidance under Contact Center production support, cross-linked to the failure runbooks. Added eight catalog-integrity unit tests (unique keys, required text, personal↔sensitivity consistency, personal categories define a concrete erasure strategy, non-personal never anonymize, recording-bearing categories are always personal, known-key/unknown-key lookup). Strict build (0/0), the ten catalog tests, and the docs build pass. R7 is complete.
...
- 2026-07-17: Continued R7 by documenting the supported multi-node real-time backplane for Contact Center. The wiring already exists as the optional `CrestApps.OrchardCore.SignalR.Redis` feature (Redis `AddStackExchangeRedis` backplane on a dedicated connection, channel namespaced by `InstancePrefix` + immutable shell name), and the Contact Center hub is backplane-agnostic — hosted via `HubRouteManager.MapHub<ContactCenterHub>` and addressing tenant-qualified `TenantSignalRGroupName` groups — so the same code path serves single-node and multi-node deployments. Added an authoritative "Multi-node real-time backplane" section to Contact Center production support that names the supported topology (`CrestApps.OrchardCore.SignalR.Redis` for message distribution plus `OrchardCore.Redis.Lock` for distributed critical sections), states that a backplane without distributed locking and multi-region active-active are unsupported, requires a deployment-unique `InstancePrefix`, and cross-references the SignalR module backplane documentation. Docs build passes.
...
- 2026-07-16: Completed R6 by making the daily event-metrics projection rebuildable, drift-checkable, and checkpointed. Added a durable `ContactCenterProjectionCheckpoint` (per-projection replay cursor `LastOccurredUtc`/`LastEventId`, logic `Version`, and `RebuiltUtc`) with its index, provider, migration (one row per handler, unique on `HandlerId`), and `IContactCenterProjectionCheckpointStore`. The event log store gained `ListOrderedPageAsync` (deterministic `OccurredUtc`-then-`ItemId` offset paging for a full single-pass replay) and the metric store gained `ListAllAsync`. The new `IContactCenterMetricsProjectionMaintenanceService.RebuildAsync` recomputes every `(day, event type)` count from the source-of-truth event log and reconciles the stored projection — creating missing metrics, correcting wrong counts, and deleting orphaned rows — then advances the checkpoint; `DetectDriftAsync` performs the same recomputation read-only and returns every discrepancy without mutating state. The rebuild is deterministic: it counts by `OccurredUtc.Date` and skips events with no type or no occurrence time (the live path's wall-clock fallback for missing timestamps cannot be reproduced), which is documented in code. The projection handler id is now the shared `ContactCenterConstants.MetricsProjectionHandlerId` (same literal value, so existing dedup markers stay valid). Added real-SQLite tests proving rebuild recomputes/corrects/removes metrics and advances the checkpoint, drift reports a stored-vs-log mismatch, and drift is empty after a rebuild. Strict build, targeted ContactCenter tests (778), and ten fresh-tenant activation scenarios pass.

- 2026-07-16: Continued R6 by bounding and leasing the remaining background sweeps. Callback promotion now reads a bounded due batch (`CallbackService.MaxBatchSize`, 100) whose store query also excludes callbacks held by an unexpired lease, and each due callback is claimed with an owner token, an incremented fence token, and a five-minute lease before its outbound activity is created; the lease is cleared on successful promotion and a concurrency loss skips the callback. This keeps promotion durable and idempotent even if the background task's distributed lock expires mid-run and an overlapping pass starts. Provider-call and telephony reconciliation sweeps no longer load every active interaction: both `IInteractionStore.ListActiveWithProviderCallIdAsync` overloads and both `ITelephonyInteractionStore.ListActiveAsync` overloads now take a bounded `maxCount` (200) and return the oldest active interactions first so bounded passes make forward progress on the most likely-stale calls. The outbox and retention batches were already bounded (`ContactCenterOutbox.MaxBatchSize`, retention delete loop) and are unchanged. Added a `LeaseExpiresUtc` column and migration to the callback index, real-SQLite tests proving the bounded lease-aware due query, and service tests proving the claim/lease lifecycle and unexpired-lease skip. Strict build, targeted ContactCenter and Telephony tests (931), and ten fresh-tenant activation scenarios pass.

- 2026-07-16: Continued R6 by replacing broad queue loads and per-queue N+1 counts with aggregate SQL and bounded top-1 queries. `IQueueItemStore`/`IQueueItemManager` gained `CountWaitingAsync` (a single `CountAsync` aggregate), `CountWaitingOlderThanAsync` (an indexed count of items enqueued before a threshold, used for SLA-breach tallies), and `FindLongestWaitingAsync` (a top-1 `OrderBy(EnqueuedUtc)` lookup for oldest-wait). The reporting service, real-time event handler, supervisor dashboard endpoint, and agent workspace endpoint now compute waiting counts, longest wait, and SLA breaches through these bounded queries instead of loading every waiting item (and, in the reporting path, per-item `LoadAsync`) into memory. The SLA-aged `SelectNext` routing path is deliberately left unbounded because SLA aging can promote old low-priority items unboundedly, so bounding its candidate set by base priority would be a routing-correctness bug. Added real-SQLite aggregate-store tests covering count, older-than count, and longest-waiting selection; updated the real-time handler tests to the aggregate API. Strict build, targeted ContactCenter tests (771), and ten fresh-tenant activation scenarios pass.

- 2026-07-16: Started R6 by normalizing agent queue membership into a query-aligned index. Each agent profile now projects one `AgentQueueMembershipIndex` row per queue it is both entitled to and signed in to (the intersection of live sign-in and the manager-owned allow-list), keyed by a lower-cased queue identifier for portable case-insensitive matching and carrying presence and configured capacity. `IAgentProfileStore.ListAvailableForQueueAsync` now selects candidate agents with a single indexed lookup by queue and `Available` presence instead of loading every available agent in the tenant and filtering entitlement in memory, while keeping the fail-closed entitlement rule authoritative. Added real-SQLite store and index tests proving entitlement/presence filtering, case-insensitive queue matching, and one normalized row per entitled queue. Strict build, targeted ContactCenter tests (768), and ten fresh-tenant activation scenarios pass.
- 2026-07-16: Completed R5 by keeping automated dialing modes disabled until certified. The GA-Core Asterisk and DialPad tenant profiles enable only the base `ContactCenter.Dialer` feature (Manual/Preview); a new support-matrix test fails if any runtime profile ever enables `ContactCenter.Dialer.Automated` before pacing certification. With this, R5's CRM attribution and regulated-outbound gates are complete: automated Power/Progressive dialing remains behind the non-GA Automated Dialer feature. Strict build, ten fresh-tenant activation scenarios, and the Docusaurus build pass.
- 2026-07-16: Completed the R5 automated-mode feature gate. The Automated Dialer feature already owned the Power/Progressive strategies, scheduled pacing, and automated batch source and hard-depends on Compliance, but the dialer-profile editor still offered Power/Progressive unconditionally. The editor now offers those modes only when the Automated Dialer feature is enabled (a currently-saved automated mode is still shown), and saving a Power/Progressive profile is rejected server-side when the feature is disabled, so an automated profile can never silently fail to pace. Added a public, tested `DialerModeExtensions` classifier as the single source of truth for automated pacing modes, replacing the duplicated inline checks in the abandonment policy service and display driver. Strict build, targeted ContactCenter tests, and the Docusaurus build pass.
- 2026-07-16: Completed the R5 regulated-outbound compliance gate. Replaced fixed calling-window hours with reusable business-hours calendars plus per-region overrides (ISO 3166-1 alpha-2) evaluated in the contact's own time zone, failing closed when a required calendar is missing or disabled. Added a fail-closed rolling abandonment-rate policy (`IDialerAbandonmentPolicyService`) that gates automated pacing modes on `MaxAbandonmentRatePercent` once `AbandonmentSampleFloor` live answers accumulate and suppresses when statistics cannot be proven; automated profiles enforcing the cap require safe-harbor messaging with an announcement. The rolling window binds and validates under `CrestApps_ContactCenter:Compliance:AbandonmentRollingWindowMinutes` (30-minute default, 1-1440 range). Provider AMD outcomes map to a provider-neutral `AnswerClassification` persisted on the call session and interaction technical metadata under the stable `amd_answer_classification` key. Strict build, all 2,020 unit tests, nine fresh-tenant activation scenarios, and the Docusaurus build pass.
- 2026-07-15: Completed the second R5 inbound-routing gate. Entry-point plans and effective queues are now qualified before queue assignment, closed voicemail/reject decisions never fall back to a generic queue, and missing or disabled target queues fail closed. The router retains one provider-correlated CRM activity and interaction for audit/idempotency, immediately makes the activity terminal and non-routable, and atomically persists exactly one durable voicemail or rejection command before scheduling post-commit dispatch. The interaction remains non-ended while provider truth is pending, then the existing fenced command state machine records confirmation, definitive failure, or an unknown outcome without requeueing. Every result carries a stable reason code on the routing response, activity, command request, and interaction metadata; no queue item, reservation, or agent offer is created. Focused tests cover closed voicemail/reject, generic no-queue, missing or disabled entry-point queues, stable reason evidence, durable command registration, post-commit dispatch, duplicate webhook idempotency, and the no-enqueue/no-offer invariant. Strict build, all 2,004 unit tests, nine fresh-tenant activation scenarios, asset rebuild, and Docusaurus production build pass.
- 2026-07-14: Completed the first R5 CRM-attribution gate. Inbound contact lookup now deduplicates and orders all matches, persists `Unresolved`, `Resolved`, or `Ambiguous` attribution on the CRM-owned `OmnichannelActivity`, and never silently chooses the first of multiple contacts. Ambiguous calls continue through contact-independent queueing and agent offers, but the source-neutral disposition service rejects unresolved ambiguity and the resource-authorized activity-completion form requires an explicit candidate selection, recording the resolving user and UTC time before disposition. Unresolved, ambiguous, and legacy unknown inbound activities defer all contact-bound Subject Actions, while resolved follow-up work retains attribution audit metadata. Focused tests cover zero/single/unordered duplicate matches, candidate invariants, central completion enforcement, explicit UI wiring, endpoint authorization, validation retry preservation, legacy fail-closed behavior, action suppression, and resolved-contact regressions. Strict build, all 2,000 unit tests, nine fresh-tenant activation scenarios, asset rebuild, and Docusaurus production build pass.
- 2026-07-14: Completed R4 by failing GA-Core media closed and explicitly deferring transport certification to R9. Removed the unused `ContactCenterVoiceProviderCapabilities.BidirectionalMedia` flag, made Contact Center Voice Media and Asterisk Contact Center Media dependency-only features hidden from direct selection, and pinned both approved tenant profiles to no media resolver/provider registration. The Asterisk RTP/UDP adapter remains a development foundation only; secure private-network boundary, packet loss/reordering/jitter, capacity, failover, and node-affinity evidence is required before any production profile can include it.
- 2026-07-14: Completed the R4 PBX command-cancellation and bounded-deadline gate. Telephony hub mutations, Contact Center recording/monitoring/transfer, and durable provider-command dispatch now execute with a validated tenant server deadline instead of SignalR or request cancellation; host shutdown is linked, non-cooperative and cancellation-swallowing providers cannot extend the caller-visible boundary, and sent-command timeout persists `OutcomeUnknown` with `provider_dispatch_timeout`. Provider-confirmed local state and event persistence completes with `CancellationToken.None`, while Telephony maps post-provider interaction-persistence failure to an unknown result. Added focused deadline, disconnect, post-confirmation persistence, recording/monitoring/transfer timeout, options-validation, and Voice/Recording feature-isolation coverage. Strict build, 1,980 unit tests, nine fresh-tenant activation scenarios, asset rebuild, and Docusaurus production build pass.
- 2026-07-14: Completed the R4 duplicate-provider-stream and monotonic-truth gate without introducing a fragile long-lived socket lease. Provider voice ingestion now locks the tenant-scoped canonical provider-call identity before reading state, commits all tracked YesSql changes before releasing the lock, and uses a hashed key that does not expose provider call identifiers. Lifecycle rank cannot regress even when a delayed event has no provider timestamp, and an established provider sequence high-water rejects later unsequenced deliveries. Added red/green tests for timestampless regression, mixed sequence domains, lock timeout, commit-before-release on stale and duplicate canonicalization paths, protected lock keys, and two simulated nodes publishing one event. Strict build, 1,963 unit tests, all eight fresh-tenant activation profiles, asset rebuild, and Docusaurus production build pass.
- 2026-07-14: Completed the R3 atomic domain-state-plus-outbox boundary. Mutating call, routing, timeout, and dialer paths now persist typed provider-command intent and immutable events before any provider or transport effect; Dial, Answer, Reject, and SendToVoicemail execute after commit through durable fenced commands; definitive timeout-action failures reoffer live work; and outbox, webhook inbox, command, reconciliation, and compensation settlement reload and verify durable owner/fence state in fresh tenant scopes. Added real-SQLite atomic commit/rollback, duplicate-idempotency, stale-fence, typed-executor, failure/reoffer, and feature-isolation coverage. Orchard feature activation no longer performs provider/database reconciliation inline, and lazy lifecycle/reoffer resolution removes optional-feature DI cycles. Strict builds pass with zero warnings, the Contact Center matrix passes 715 tests, all eight fresh-tenant activation profiles pass, and the Docusaurus production build succeeds.
- 2026-07-14: Added the fail-closed Telephony browser-audio foundation without advancing the R3/R4 release sequence. `ITelephonyAudioProvider`, `TelephonyAudioCapabilities`, `TelephonyAudioMode`, and the resolver distinguish browser audio from external devices, automatically select single-mode providers, require provider configuration for dual-mode providers, and reject browser mode without an executable adapter name. The soft phone lazily requests microphone access before dial/answer, initializes a provider adapter with local and remote media seams, follows authoritative call state, and releases tracks on terminal/disconnect/unload. Built-in DialPad and Asterisk Telephony adapters remain explicitly external-device-only; provider-specific WebRTC certification remains an open R4 gate.
- 2026-07-14: Hardened and completed the Contact Center/Omnichannel administration UX pass. Voice Soft Phone resources now register before Footer-zone rendering to prevent duplicate Bootstrap Select markup beside Orchard admin filters; Soft Phone history defaults to 30 configurable calls; report and agent-entitlement user values use Orchard display-name resolution while campaign membership rows resolve catalog display text; Interaction Center configuration pages share a Management submenu; Omnichannel subjects are identified only by `OmnichannelSubjectPart`; workspace history, activity filters, phone fields, queue SLA wording, and feature categories were reorganized for clearer operator and administrator use.
...
- 2026-07-13: Started the AI voice implementation with an explicit executable media seam. `IContactCenterVoiceMediaProvider` and `IContactCenterVoiceMediaSession` keep provider transport and framing outside Contact Center orchestration, and resolution requires a matching base voice provider plus executable media registration. Selected ARI External Media over RTP/UDP as the first Asterisk development transport because it has broader version compatibility than newer WebSocket media transports; DialPad remains unavailable for AI media.
- 2026-07-13: Implemented the Asterisk ARI External Media RTP/UDP development adapter. It creates or reuses the live call bridge, attaches an `ulaw` external-media channel, resolves Asterisk's RTP return endpoint, parses and emits RTP packets, and removes media resources without hanging up the provider call. Session metadata supplies the Asterisk-reachable Orchard host and optional local bind address/port.
...
- 2026-07-15: **R3 canonical provider identity, portable uniqueness, ordering, and stable outbox handler ids completed.** Added the implementation-free `IProviderIdentityProvider` contract plus `ProviderIdentity` model in Contact Center Abstractions and the Core `IProviderIdentityResolver`/`ProviderIdentityResolver`, so provider modules contribute canonical identities without Core referencing provider assemblies. Asterisk now maps its `Default Asterisk` alias to the canonical `Asterisk` and DialPad declares its canonical name. `ProviderVoiceEventService` canonicalizes the provider before any interaction/call/event lookup or stamp, so an alias no longer mutates stored identity. Normalized provider events gained an optional monotonic `SequenceNumber`; `CallSession` persists `HighWaterSequence`, and ingestion rejects stale/equal-order deliveries by sequence when present and by a lifecycle-rank timestamp guard otherwise, so an equal-timestamp event can no longer regress a more-advanced call while timestamp/idempotency-only providers stay safe. Portable, non-null claim-key columns (`ProviderCallClaimKey`, `IdempotencyClaimKey`) and a canonical composite provider-delivery index now database-enforce one call session per provider-call identity, one interaction event per idempotency key, and one inbox message per canonical provider delivery, via preflight/repair upgrade migrations that fail actionably on irreconcilable duplicates rather than deleting business data. The outbox now checkpoints handlers by an explicit stable versioned `HandlerId` (independent of CLR name/assembly version/registration order) with a deploy-safe legacy `{AssemblyQualifiedName}:{index}` alias/repair path, and every concrete handler declares a unique id. Inverted the alias-mutation, equal-timestamp regression, and reorder-replay characterizations; added real-SQLite migration/constraint tests, the resolver unit tests, sequence/high-water tests, a legacy-checkpoint repair test, and fresh-tenant activation resolution of `IProviderIdentityResolver`, `IProviderVoiceEventService`, and `IProviderWebhookInbox`. Targeted Contact Center, Telephony, DialPad (765), and feature-activation (7) suites pass. Explicit deferral: poison-message isolation and per-handler idempotency-contract documentation remain the next separate R3 bullet.
- 2026-07-15: **R3 hardening of canonical identity, idempotency, high-water, and handler ids completed.** Closed four blockers found in the previous entry's implementation. (1) Alias repair now actually canonicalizes: the `CallSessionIndex` and `ProviderWebhookInboxMessageIndex` upgrade migrations resolve legacy `ProviderName` aliases through an injected `IProviderIdentityResolver` before duplicate preflight and unique-index creation (detecting `Default Asterisk`/`Asterisk` collisions and rewriting survivors to the canonical name), the `CallSessionIndexProvider` and `ProviderWebhookInboxMessageIndexProvider` canonicalize `ProviderName` while mapping so legacy documents cannot recreate alias index values on reindex, and runtime lookups resolve to canonical values so alias-stored rows stay reachable. (2) Provider voice-event idempotency keys are now scoped by canonical provider (`{provider}|{rawDeliveryId}`) before the duplicate lookup and publication, so identical raw delivery ids from Asterisk and DialPad can no longer collide; non-provider domain-event idempotency is unchanged. (3) `CallSessionStore` now enforces optimistic concurrency (`CheckConcurrency`); concurrent provider-event ingestion is compare-and-set safe, and a `ConcurrencyException` is treated as ownership loss — the durable inbox rethrows without reusing the canceled session and reprocesses the delivery in a fresh scope on the next pass (immediate webhook dispatch drains after all deliveries are durably accepted and stops on a concurrency loss). (4) The domain-event outbox now fails fast at construction on a null, blank, or duplicate `HandlerId`, so a duplicate id can never let one handler's checkpoint skip a different failed handler. Added deterministic shared-SQLite CAS tests (sequences N and N+1 cannot commit in reverse or lower the persisted high-water/state), alias-canonicalization migration tests (successful rewrite and collision failure), a provider-scoped idempotency non-collision test, inbox `ConcurrencyException` ownership-loss tests, and handler-id validation tests. Targeted Contact Center + DialPad + Asterisk (729) and feature-activation (7) suites pass. Explicit deferral: poison-message isolation and per-handler idempotency-contract documentation remain the next separate R3 bullet.
- 2026-07-15: **R3 poison-message isolation and handler replay-safety completed.** Outbox and provider-inbox retry passes now execute each due message in a fresh Orchard child scope and continue after isolated failures. Event handlers execute in nested child scopes so their durable effects and replay markers commit atomically on success and roll back together on failure. Added the required `ContactCenterHandlerReplaySafety` contract to event and inbox handlers with fail-fast validation for unspecified contracts and duplicate or blank identities. Added the durable `ContactCenterProcessedEvent` ledger and unique `(HandlerId, EventId)` index; metrics and Workflows handlers use it to suppress replayed non-idempotent effects. Daily metrics now also use optimistic concurrency and portable `(DateKey, EventType)` uniqueness for concurrent distinct events. Updated the public eventing, analytics, Workflows, control-matrix, and release documentation without claiming the still-open R6 projection rebuild/checkpoint/drift tooling.
