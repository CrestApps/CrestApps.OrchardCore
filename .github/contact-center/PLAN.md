...
- [ ] **R4 — Executable provider capabilities and high availability:** recording/monitoring contracts, provider stream ownership, monotonic events, command cancellation, media transport hardening.
  ...
  - [ ] Certify provider-specific browser media adapters through end-to-end audio tests before any provider advertises browser audio. The current DialPad REST integration and Asterisk ARI Telephony integration remain external-device-only; Asterisk External Media remains server-side RTP and must not be represented as browser WebRTC.
