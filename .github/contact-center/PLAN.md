...
- [~] **R1 — Security and tenant isolation:** tenant-qualified SignalR, queue/campaign entitlements, secret/PII redaction, simulator containment, XSS correction.
  ...
  - [ ] Centralized operational PII redaction and the remaining webhook ingress controls: rate/concurrency limits, freshness/replay policy, and durable inbox acknowledgement.
...
- 2026-07-13: Added the R0a legal feature activation and undeclared-service dependency architecture test (F001/F002/T001). `ContactCenterFeatureDependencyArchitectureTests` parses the Contact Center, SignalR, Telephony, and Omnichannel Managements manifests/`Startup.cs` files and asserts, against the new machine-readable ledger `.github/contact-center/feature-dependency-violations.v1.json`, that (1) every Contact Center feature's declared external manifest dependencies exactly match the ledger, (2) every concrete type a Contact Center `Startup` class registers outside a `[RequireFeatures]` guard resolves its constructor parameters only from its own feature's declared transitive closure or an explicitly accepted external dependency, (3) every ledger violation owns a control-matrix gate id, and (4) every Contact Center feature's exact transitive manifest closure matches the ledger and only reaches known features or accepted leaf dependencies. The ledger pins the three currently known P0 findings without refactoring production feature boundaries (deferred to R2): the base feature's declared manifest dependency on Omnichannel Managements (FDV001), Voice's declared manifest dependency on the concrete Telephony Soft Phone feature instead of a provider-agnostic abstraction (FDV002), and Queues' undeclared runtime requirement on SignalR's `HubRouteManager` through `ContactCenterSoftPhoneWidgetDisplayDriver` (FDV003). Any new, unrecorded undeclared dependency now fails the build.
