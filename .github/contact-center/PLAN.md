...
- [~] Phase 3 — Routing MVP (availability + priority + required-skills filtering + longest-idle assignment + auditable routing decision events shipped; sticky-agent and business-hours strategies pending)
...
- [~] Phase 5 — Outbound dialer MVP (`Dialer` feature: profiles, modes, power/progressive pacing, dialer batch sources, outbound calls routed through the Voice Contact Center Call Router, DialPad Contact Center Voice provider; **BLOCKER: Progressive/Predictive exposed but unsafe, and configured compliance (`RespectDoNotCall`/`RetryDelayMinutes`/calling windows/suppression) is not enforced** — see "Design review" P0 #4, #5; retry/callback/suppression pending)
- [~] Phase 6 — Wrap-up and disposition lifecycle (source-neutral `IActivityDispositionService` implemented and CRM activity completion routed through it so inbound and outbound disposition share the subject workflow; **BLOCKER: `ActivitiesController.Complete` only accepts `NotStated` so contact-center-state activities cannot be completed, and there is no wrap-up timer/required-disposition/capacity-release lifecycle** — see "Design review" P0 #8 and P1 #10)
- [ ] Phase 7 — Agent desktop and supervisor real-time UX
- [ ] Phase 8 — Inbound entry points, IVR and self-service
- [ ] Phase 9 — Recording and live monitoring
- [ ] Phase 10 — Outbound compliance hardening
...
- [ ] **G2 — Assignment safety (hardens Phases 2/3):** distributed-lock/single-writer assignment + optimistic concurrency on reservation; enforce `MaxConcurrentInteractions`; split a live `AgentSession` from `AgentProfile` with SignalR heartbeat + stale cleanup; drive offer timeout from the real-time layer. *(P0 #6, #7; P1 #13, #14)*
- [ ] **G3 — Wrap-up + completion unification (completes Phase 6):** `WrapUpSession`, required-disposition policies by queue/subject/campaign, capacity release, and a contact-center-aware completion path through `IActivityDispositionService`. *(P0 #8; P1 #10)*
- [ ] **G4 — Dialer safety (completes Phase 5, pulls forward Phase 10 essentials):** strategy-per-mode (`IDialerStrategy`), an `IDialerEligibilityService`/compliance gate (DNC, communication preferences, calling windows, retry cool-down, suppression audit), cap Power, and block Predictive until metrics + abandonment controls exist. *(P0 #4, #5)*
- [ ] **G5 — Agent desktop + supervisor real-time UX (Phase 7):** CRM-integrated agent cockpit, supervisor dashboards, queue monitor/wallboard, and scoped/audited live call-control intents. *(P1 #15, #16)*
- [ ] **G6 — Eventing/outbox + provider webhooks (hardens Phase 1, extends Phase 4):** outbox dispatch + retry/backoff, projection checkpoints, mandatory idempotency on provider events, rebuildable projections, and signed per-provider webhook adapters. *(P1 #17, #18)*
- [ ] **G7 — Routing depth (completes Phase 3):** `RoutingPolicy`, `QueueMembership`, skill proficiency, business-hours/holiday calendars, overflow, sticky agent, priority/SLA-aging strategies. *(P1 #11)*
...
- 2026-06-30: **G1 backend implemented (voice foundation hardening / media delivery).** Added the voice-provider delivery model (`VoiceProviderDeliveryModel.AgentDeviceNative`/`ServerSideAcd`), `ContactCenterConnectRequest`, `IContactCenterVoiceProvider.DeliveryModel` + `ConnectToAgentAsync`, and the `AgentConnect` capability; DialPad declares `AgentDeviceNative` and implements `ConnectToAgentAsync` as a native-device no-op. Added the `CallSession` aggregate (model, `ContactCenterCallState`, index, store, manager, migration) registered in the base feature, and `FindByProviderInteractionIdAsync` on the interaction store/manager. Added normalized provider-event ingestion (`ProviderVoiceEvent` + `IProviderVoiceEventService`) that matches events to the interaction/call session by provider call id, advances state/timestamps idempotently, bridges answered outbound calls on server-side ACD providers, and publishes domain events (`CallSessionCreated`/`CallSessionUpdated`/`CallConnected`/`CallEnded`/`OfferAccepted`/`OfferDeclined`). Added `IContactCenterCallCommandService`, the single authoritative server-side accept/decline command (accept reservation → connect media for `ServerSideAcd` → advance interaction + call session together; result carries `RequiresDeviceAnswer`), and rewired `VoiceController` to it — fixing the fire-and-forget accept (P0 #2) and the missing inbound media bridge (P0 #1). Added 5 unit tests (41 ContactCenter tests pass); full solution builds clean with `-warnaserror`. Docs (`contact-center/index.md`) and the `v2.0.0` changelog updated. **Remaining for G1:** soft-phone JS coordination (await accept, answer device only when `RequiresDeviceAnswer`) + asset rebuild; per-provider signed webhook adapters that post `ProviderVoiceEvent`; transfer/conference taxonomy. Next: G2 (assignment safety — distributed-lock assignment, capacity enforcement, live `AgentSession` + heartbeat, real-time offer timeout).
