...
- **W11.4 Feature-dependency audit.** `Voice.SoftPhone → RealTime` is declared but unused by its own startup; `EnabledByDependencyOnly` is applied inconsistently. Add a test that every declared dependency is used and every used service's feature is declared. This is the general fix for the class of bug W0.7 patched pointwise (orchard F-03).
...
- **W15.2 Additive-only migration enforcement test** — fail CI on any destructive column/table operation (the existing `UpdateFrom3Async` drop is the precedent).
...
- **W16.8** Add a test proving no Contact Center correctness path depends on Elasticsearch (the support matrix asserts this; nothing enforces it).
...
| `gate-feature-dependency-audit` | PR | declared dependencies used; used services declared |
...
| `gate-migration-additive-only` | PR | no destructive schema operations |
...
| `gate-no-elasticsearch-dependency` | PR | correctness paths run with Elasticsearch absent |
...
- [ ] W11.4 Feature-dependency audit gate live `gate:gate-feature-dependency-audit`
...
- [ ] W15.2 Additive-only migration enforcement `gate:gate-migration-additive-only`
...
- [ ] W16.8 No Elasticsearch in any correctness path `gate:gate-no-elasticsearch-dependency`
...
4. **W15.2's destructive-migration ban needs an explicit contract-step exception**, otherwise it forbids the very "contract" phase of the expand-migrate-contract policy W2 depends on. The gate must permit destructive steps only when accompanied by a declared, reviewed contract-phase marker.
...
14. **W0.4 — docs validation must run on code PRs**, not only doc-path PRs, or code changes silently invalidate documented claims.
