...
- [x] W11.4 Feature-dependency audit gate live `gate:gate-feature-dependency-audit` — `ContactCenterFeatureDependencyAuditTests` (3 tests) + `ContactCenterOptionalDependencyTests` + `CallControlAuthorizationBoundaryTests`. "Declared dependency is used" half is disclosed as not dynamically enforceable.
