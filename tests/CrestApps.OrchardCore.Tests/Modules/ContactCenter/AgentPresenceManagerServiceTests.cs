...
public sealed class AgentPresenceManagerServiceTests
{
    ...
    public async Task SignInAsync_CreatesAvailableProfile_AndJoinsQueues()
    {
        ...
        var service = new AgentPresenceManagerService(agentManager.Object, [], new NoAgentWorkStateHealingService(), new EnforcingAgentEntitlementPolicy(), publisher.Object, CreateDistributedLock().Object, clock, new Mock<ILogger<AgentPresenceManagerService>>().Object);
    ...
    [Fact]
    public async Task SignInAsync_WhenSignedIntoACampaign_EnrollsInTheCampaignsVirtualQueue()
    {
        // Arrange: an agent entitled to a campaign. The campaign's routing queue is derived from the campaign id
        // (never stored), so signing into the campaign should enroll the agent in that virtual queue with no
        // dialer-profile lookup at all.
        var agentManager = new Mock<IAgentProfileManager>();
        agentManager.Setup(m => m.FindByUserIdAsync("u1", It.IsAny<CancellationToken>()))
            .ReturnsAsync(() => new AgentProfile { ItemId = "a1", UserId = "u1", AllowedCampaignIds = ["camp-1"] });
        ...
        // Assert: signing into the campaign enrolled the agent in its virtual queue for routing, entitled it,
        // and recorded the campaign membership.
        var campaignQueueId = ContactCenterConstants.CampaignQueue.CreateId("camp-1");
        Assert.Contains("camp-1", profile.CampaignIds);
        Assert.Contains(campaignQueueId, profile.QueueIds);
        Assert.Contains(campaignQueueId, profile.AllowedQueueIds);
    }
    ...
    public async Task SignInAsync_WhenNoQueueOrCampaignIsEntitled_ThrowsAndDoesNotSignIn()
    {
        ...
        var clock = new FakeTimeProvider();
        clock.SetUtcNow(_now);
        var service = new AgentPresenceManagerService(agentManager.Object, [], new NoAgentWorkStateHealingService(), new EnforcingAgentEntitlementPolicy(), new Mock<IContactCenterEventPublisher>().Object, CreateDistributedLock().Object, clock, new Mock<ILogger<AgentPresenceManagerService>>().Object);
    ...
    public async Task UpdateMembershipsAsync_PreservesPresenceAndActiveReservation()
    {
        ...
        // Signing into campaign "c1" enrolls the agent in its derived virtual queue alongside the real queue "q2".
        Assert.Equal(["q2", ContactCenterConstants.CampaignQueue.CreateId("c1")], profile.QueueIds);
    ...
    /// <summary>
    /// Going offline because the connection lapsed is not the same as choosing to stop working: the agent keeps
    /// the queues and campaigns they were signed into, so coming back and setting Available is enough to be
    /// offered work again. Routing is unaffected meanwhile because it requires Available plus a live session.
    /// </summary>
    [Fact]
    public async Task MarkOfflineAsync_SetsOfflineAndKeepsMembership()
    {
        // Arrange
        var existing = new AgentProfile
        {
        ...
        Assert.Equal("session-expired", profile.PresenceReason);
    ...
    [Fact]
    public async Task MarkOfflineAsync_WhenAlreadyOffline_LeavesProfileUntouched()
    {
        // Arrange
        var existing = new AgentProfile
        {
        ...
        // Assert
        Assert.Equal("Signed out", profile.PresenceReason);
        agentManager.Verify(m => m.UpdateAsync(It.IsAny<AgentProfile>(), It.IsAny<System.Text.Json.Nodes.JsonNode>(), It.IsAny<CancellationToken>()), Times.Never);
        publisher.Verify(p => p.PublishAsync(It.IsAny<InteractionEvent>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    [Fact]
    public async Task MarkOfflineAsync_WhenNoProfile_ReturnsNull()
    {
        // Arrange
        var agentManager = new Mock<IAgentProfileManager>();
        agentManager.Setup(m => m.FindByUserIdAsync("u1", It.IsAny<CancellationToken>())).ReturnsAsync((AgentProfile)null);
    ...
    public async Task SignOutAsync_WhenLiveSessionExists_ClearsSessionMembership()
    {
        ...
        var clock = new FakeTimeProvider();
        clock.SetUtcNow(_now);
        var service = new AgentPresenceManagerService(agentManager.Object, [sessionManager.Object], new NoAgentWorkStateHealingService(), new EnforcingAgentEntitlementPolicy(), new Mock<IContactCenterEventPublisher>().Object, CreateDistributedLock().Object, clock, new Mock<ILogger<AgentPresenceManagerService>>().Object);
    ...
    public async Task SignInAsync_WhenExistingProfilePresent_HealsStaleWorkBeforeResettingAvailability()
    {
        ...
        var clock = new FakeTimeProvider();
        clock.SetUtcNow(_now);
        var service = new AgentPresenceManagerService(agentManager.Object, [], healer.Object, new EnforcingAgentEntitlementPolicy(), new Mock<IContactCenterEventPublisher>().Object, CreateDistributedLock().Object, clock, new Mock<ILogger<AgentPresenceManagerService>>().Object);
