...
public sealed class SmsQuietHoursTests
{
    ...
    private static MessagingQuietHoursGuard CreateGuard(
        bool isOpen,
        ...
        string contactTimeZoneId = null)
    {
        ...
        queuePolicyReader.Setup(reader => reader.ReadAsync("queue-1", It.IsAny<CancellationToken>()))
            .ReturnsAsync(new MessagingQueuePolicy(true, 0, calendarId));

        var timeZoneResolver = new Mock<IMessagingContactTimeZoneResolver>();
        timeZoneResolver.Setup(resolver => resolver.ResolveAsync(It.IsAny<MessagingConversation>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(contactTimeZoneId);

        var clock = new Mock<IClock>();
        clock.SetupGet(value => value.UtcNow).Returns(_now);

        return new MessagingQuietHoursGuard(gate.Object, queuePolicyReader.Object, timeZoneResolver.Object, clock.Object);
    }
}
