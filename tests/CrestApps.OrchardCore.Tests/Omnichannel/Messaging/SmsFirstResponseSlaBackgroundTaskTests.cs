...
public sealed class SmsFirstResponseSlaBackgroundTaskTests
{
    ...
    public async Task ARun_MakesOnePass_AndReturnsTheLoopAtOnce()
    {
        ...
        // Act
        var run = new MessagingFirstResponseSlaBackgroundTask().DoWorkAsync(serviceProvider, cancellation.Token);
        ...
        scheduler.Verify(
            value => value.Schedule(MessagingFirstResponseSlaSweep.DeadlineKey, _now + MessagingFirstResponseSlaSweep.MaximumInterval, It.IsAny<Func<IServiceProvider, CancellationToken, Task<DateTime?>>>()),
            Times.Once);
    }
    ...
    public async Task APass_HoldsTheNextPassForTheSoonestTargetStillAhead()
    {
        ...
        // Act
        var next = await serviceProvider.GetRequiredService<MessagingFirstResponseSlaSweep>().RunAsync(TestContext.Current.CancellationToken);
    ...
    public async Task APass_LooksAgainWithinTheOldCadence_WhenTheSoonestTargetIsFurtherOut()
    {
        ...
        // Assert
        Assert.Equal(_now + MessagingFirstResponseSlaSweep.MaximumInterval, next);
    }
    ...
    public async Task APass_AnnouncesNothing_WhileAnotherPassHoldsTheLock()
    {
        ...
        Assert.Equal(_now + MessagingFirstResponseSlaSweep.LockedRetryInterval, next);
    }

    private static (ServiceCollection Services, Mock<IMessagingFirstResponseSlaService> Sla, Mock<IContactCenterDeadlineScheduler> Scheduler, Mock<ISession> Session) CreateServices(
        DateTime? nextDueUtc,
        bool lockAcquired = true)
    {
        var sla = new Mock<IMessagingFirstResponseSlaService>();
