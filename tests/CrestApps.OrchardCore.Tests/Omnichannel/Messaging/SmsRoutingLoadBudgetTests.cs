...
public sealed class SmsRoutingLoadBudgetTests
{
    ...
    private sealed class RoutingHarness
    {
        ...
        private readonly Mock<IMessagingConversationStore> _conversationStore = new();
        private readonly Mock<IMessagingAvailabilityService> _availability = new();
        ...
        public RoutingHarness()
        {
            ...
            _availability.Setup(x => x.Get(It.IsAny<AgentProfile>()))
                .Returns(new MessagingAgentAvailability { Available = true, MaxConcurrent = 10_000 });
            ...
            Strategy = new LeastLoadedMessagingRoutingStrategy(
                agentManager.Object,
                _conversationStore.Object,
                _availability.Object,
                queueManager.Object,
                _interactionManager.Object,
                new OptionsWrapper<MessagingRoutedDistributionOptions>(new MessagingRoutedDistributionOptions()));
        }

        public LeastLoadedMessagingRoutingStrategy Strategy { get; }
