...
public class SmsInboundProcessorTests
{
    ...
    public async Task NewInbound_WithAgentRoute_CreatesAssignedConversation()
    {
        ...
        harness.Notifier.Verify(n => n.NewInboundMessageAsync(It.IsAny<MessagingInboundNotification>(), It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task NewInbound_AnnouncesItsChannelAndCustomer_SoTheWorkspaceCanBadgeThatChannelsTab()
    {
        var harness = new Harness(routing: null, contactContentItemId: "contact-9");
        MessagingInboundNotification announced = null;

        harness.Notifier
            .Setup(n => n.NewInboundMessageAsync(It.IsAny<MessagingInboundNotification>(), It.IsAny<CancellationToken>()))
            .Callback<MessagingInboundNotification, CancellationToken>((notification, _) => announced = notification)
            .Returns(Task.CompletedTask);

        var conversation = await harness.Processor.ProcessAsync(Harness.InboundMessage("hi"), TestContext.Current.CancellationToken);

        Assert.NotNull(announced);
        Assert.Equal("SMS", announced.Channel);
        Assert.Equal(conversation.GetCustomerKey(), announced.CustomerKey);
        Assert.Equal("SMS", conversation.Channel);
    }

    [Fact]
    public async Task Inbound_OnAChannelNoFeatureServes_IsNotTurnedIntoAConversation()
    {
        var harness = new Harness(routing: null);
        var message = Harness.InboundMessage("hello");
    ...
    public async Task ExistingConversation_WhileAutomatedActivityActive_YieldsToTheAiPath()
    {
        ...
        harness.ConversationStore.Verify(s => s.UpdateAsync(It.IsAny<MessagingConversation>(), It.IsAny<CancellationToken>()), Times.Never);
    ...
    public async Task Inbound_OptInKeyword_WhenTheContactIsNotOptedOut_SendsNothing(string body)
    {
        ...
        Assert.NotNull(conversation);
        Assert.NotEqual(ConversationStatus.Closed, conversation.Status);
    ...
    public async Task ProcessAsync_SerializesTheThread_OnTheAddressPairLock()
    {
        ...
        Assert.Contains("MessagingConversation:SMS:+15553334444:+15551112222", harness.DistributedLock.AcquiredKeys);
    }
    ...
    public async Task ProcessAsync_WhenAnotherNodeCreatedTheThreadFirst_ReReadsItInsteadOfCreatingASecond()
    {
        ...
        var winner = new MessagingConversation
        {
            Channel = "SMS",
            ItemId = "conv-winner",
            ServiceAddress = "+15553334444",
    ...
    private sealed class Harness
    {
        ...
        public Mock<IMessagingConversationStore> ConversationStore { get; } = new();

        public MessagingInboundProcessor Processor { get; }
            ...
            bool contactOptedOut = false)
        {
            ...
            {
                conversationStore.Setup(s => s.CreateAsync(It.IsAny<MessagingConversation>(), It.IsAny<CancellationToken>()))
                    .Returns(() => ValueTask.FromException(new InvalidOperationException("UNIQUE constraint failed: UQ_MessagingConversationIndex_Addresses")));
            }
            ...
            var routers = new IMessagingInboundRouter[]
            {
            ...
            var channels = MessagingTestChannels.Resolver(Dispatcher.Object);

            Processor = new MessagingInboundProcessor(
                channels,
