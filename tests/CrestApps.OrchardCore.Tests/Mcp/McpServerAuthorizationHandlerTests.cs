...
public sealed class McpServerAuthorizationHandlerTests
{
    // ───────────────────────────────────────────────────────────────
    ...
    [Fact]
    public void AccessMcpServerPermission_IsSecurityCritical()
    {
        Assert.True(McpServerPermissionsProvider.AccessMcpServer.IsSecurityCritical);
    }
    ...
    {
        var handler = CreateHandler(new McpServerOptions
        {
            AuthenticationType = McpServerAuthenticationType.None,
        });
    ...
    public async Task ApiKey_AuthenticatedWithCorrectScheme_Succeeds()
    {
        var handler = CreateHandler(new McpServerOptions
        {
            AuthenticationType = McpServerAuthenticationType.ApiKey,
        });
    ...
    public async Task DefaultFallback_AuthenticatedWithPermission_Succeeds()
    {
        var handler = CreateHandler(
            new McpServerOptions
            {
                AuthenticationType = (McpServerAuthenticationType)999,
                RequireAccessPermission = true,
            },
            authorizeResult: true);
        ...
        bool authorizeResult = false)
    {
        ...
        services.AddSingleton<Microsoft.Extensions.Options.IOptionsMonitor<McpServerOptions>>(
            new TestOptionsMonitor<McpServerOptions>(options));
    ...
    {
        var requirement = new McpServerAuthorizationRequirement();
