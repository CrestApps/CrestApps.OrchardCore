...
public sealed class SmsHandoffTranscriptTests
{
    ...
    public async Task InboundDuringAnAutomatedActivity_ThenHandoff_RecordsEachMessageOnce(bool processorRunsFirst)
    {
        ...
        harness.Activity = new OmnichannelActivity { ItemId = "activity-1", AISessionId = "session-1", Status = ActivityStatus.AwaitingCustomerAnswer };
    ...
    private sealed class Harness : IAsyncDisposable
    {
        ...
        public async Task<string> SeedConversationAsync(string content, string providerMessageId, DateTime createdUtc)
        {
            ...
            await new SmsConversationStore(session).CreateAsync(conversation, cancellationToken);
        ...
        public async Task<string> FindConversationIdAsync()
        {
            ...
            var conversation = await new SmsConversationStore(session)
                .FindByAddressesAsync(ServiceAddress, ContactAddress, TestContext.Current.CancellationToken);
        ...
        private SmsInboundProcessor CreateProcessor(ISession session)
        {
            ...
            contactResolver
                .Setup(resolver => resolver.ResolveContactContentItemIdAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
                .Returns(() => ValueTask.FromResult<string>(null));

            return new SmsInboundProcessor(
                CreateEndpointManager(),
                activityStore.Object,
                new SmsConversationStore(session),
        ...
        private SmsAgentHandoffService CreateHandoffService(ISession session)
        {
            var clock = CreateClock();
            ...
            queuePolicyReader
                ...
                .ReturnsAsync(new SmsQueuePolicy(true, 0, null));

            return new SmsAgentHandoffService(
                new SmsConversationStore(session),
                Mock.Of<ISmsRealTimeNotifier>(),
        ...
        private static SmsConversationRouter CreateRouter(IClock clock)
            => new(
                [
                    new ExistingConversationRouter(),
                    new NumberRouteRouter(),
                    new HandoffQueueRouter(Mock.Of<ISmsRoutingStrategy>(), clock),
                    new FallbackRouter(),
                ],
