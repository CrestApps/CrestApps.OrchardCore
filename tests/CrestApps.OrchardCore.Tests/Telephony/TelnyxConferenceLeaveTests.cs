...
public sealed class TelnyxConferenceLeaveTests
{
    ...
    // A dialed number leads a merge with an extension call, whichever the phone named first: its agent leg carries the agent in.
    [Fact]
    ...
    public async Task AMergeLedByAnExtensionCall_MovesTheColleagueIntoANewConference_WhichTheAgentLeavesWithoutEndingIt()
    {
        ...
        // Assert - a new conference is made from the colleague; the agent's extension leg leaves its own conference, which
        // it created, with actions/leave before joining, so that one expiring cannot take it down.
        Assert.True(result.Succeeded, result.Error);
        Assert.Equal(
            [
                ...
                $"GET /v2/calls/{ExtensionAgentLeg}",
                "POST /v2/conferences",
                $"GET /v2/conferences?filter[name]=ext-{ExtensionAgentLeg}&filter[status]=in_progress",
                "POST /v2/conferences/conference-ext/actions/leave",
                "POST /v2/conferences/conference-1/actions/join",
                "POST /v2/conferences/conference-1/actions/join",
            ],
            handler.Requests.Select(TelnyxMergeExtensionCallTests.Describe));
        Assert.Equal(ExtensionAgentLeg, TelnyxMergeExtensionCallTests.ReadString(handler.Requests[4].Body, "call_control_id"));
    ...
    public async Task WhenTelnyxRefusesToMoveTheColleague_TheMergeUsesTheExtensionCallsOwnConference()
    {
        ...
        // Assert - nothing moved, so the caller joins the extension call's own conference.
        Assert.True(result.Succeeded, result.Error);
        Assert.Equal(
            [
                $"GET /v2/calls/{CallerLeg}",
    ...
    public async Task EndingAConferenceThatIsNoLongerRunning_StillHangsUpTheCall()
    {
        ...
        Assert.True(result.Succeeded, result.Error);
        Assert.Equal(
            ["GET /v2/conferences?filter[name]=conf-agent-a&filter[status]=in_progress", "POST /v2/calls/agent-a/actions/hangup"],
            handler.Requests.Select(TelnyxMergeExtensionCallTests.Describe));
    }
