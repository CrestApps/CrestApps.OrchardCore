...
| | |
| --- | --- |
| **Feature Name** | Model Context Protocol (MCP) FTP Resource |
| **Feature ID** | `CrestApps.OrchardCore.AI.Mcp.Resources.Ftp` |

Provides FTP/FTPS resource support for the MCP Server, allowing remote files to be exposed as MCP resources.

## Overview

This module wires Orchard Core's FTP resource UI and deployment integration on top of the shared `CrestApps.Core.AI.Ftp` framework package. The shared handler uses the [FluentFTP](https://github.com/robinrodricks/FluentFTP) library to connect to FTP servers and retrieve file content.
...
- [MCP Server](../mcp/) - Core MCP Server functionality
- [MCP SFTP Resource](sftp) - SSH File Transfer Protocol support
