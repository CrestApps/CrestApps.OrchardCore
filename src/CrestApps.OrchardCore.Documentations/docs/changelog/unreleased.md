...
- Added a complete **Articles** admin feature to the MVC sample app demonstrating custom knowledge-base data sources: CRUD management with Markdown descriptions, YesSql-backed catalog storage with `ArticleIndex`, automatic search indexing via `ArticleIndexingHandler` (Elasticsearch and Azure AI Search), configurable pagination settings (default 25 items per page) shared across all admin lists, a reusable `_Pager` partial, and six seed articles covering AI and search indexing topics.
