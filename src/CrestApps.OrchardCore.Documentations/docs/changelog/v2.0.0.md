...
- **AI Profile and Profile Template editors reorganized** — Profile editors now group common settings into **General**, **Deployments**, **Interactions**, **Instructions**, and **Parameters** cards. The standalone connection dropdown was removed from AI Profiles and Profile-source AI Templates in favor of deployment selection, deployment labels now consistently use **Chat Deployment** and **Utility Deployment**, and the per-profile **Enable User Memory** toggle moved out of the separate **Memory** tab into **Interactions**.
- **Temporary recipe schema exporter for AgentSkills** — Added `tests/CrestApps.OrchardCore.RecipeSchemaExporter`, a removable sync utility that exports `recipe.schema.json`, `index.json`, and one `<step>.schema.json` file per `IRecipeStep` into the `orchardcore-recipes` skill references folder. The `orchardcore-recipes` skill now instructs AI models to consult those generated schema files before emitting recipe JSON, reducing invalid recipe payloads caused by guessed step names, property casing, or unsupported options.
- **Expanded `ReplaceContentDefinition` schema** — `ReplaceContentDefinitionRecipeStep` now reuses the same structured `ContentTypes` and `ContentParts` JSON schema builder as `ContentDefinitionRecipeStep`, so replacement recipes expose the same nested part and field contract to validation and AI tooling instead of using loose object arrays.
- **Completed Orchard Core recipe step coverage for exported schemas** — Added missing schema definitions and AgentSkills exports for Orchard Core's built-in user, external authentication, OpenID, and localization recipe steps, including `Users`, `custom-user-settings`, `AzureADSettings`, `MicrosoftAccountSettings`, `FacebookCoreSettings`, `FacebookLoginSettings`, `GitHubAuthenticationSettings`, `TwitterSettings`, `OpenIdApplication`, `OpenIdClientSettings`, `OpenIdScope`, `OpenIdServerSettings`, `OpenIdValidationSettings`, `Translations`, and `DynamicDataTranslations`.
- **Recipe schema exporter now resolves a relative sibling AgentSkills path** — The temporary exporter no longer assumes a machine-specific `C:\Code\CrestApps\...` layout. When no output path is supplied, it now finds the `CrestApps.OrchardCore` repo root via markers such as `global.json`, `NuGet.config`, or `CrestApps.OrchardCore.slnx`, then writes to the sibling `CrestApps.AgentSkills` repository path automatically.
- **Recipe schema exporter now documents itself and fails with fix-up guidance** — Added a `README.md` to `tests/CrestApps.OrchardCore.RecipeSchemaExporter` explaining what the exporter does and how to run it manually. When the default sibling `CrestApps.AgentSkills` repository cannot be found, the exporter now throws an actionable error that tells contributors to clone `https://github.com/CrestApps/CrestApps.AgentSkills.git` next to `CrestApps.OrchardCore` or provide an explicit output path.
...
- Reusable templates for creating AI Profiles with pre-configured system messages, model parameters, tool selections, and deployment settings.
...
- **Metadata-based storage** — The `AIProfileTemplate` model contains only generic fields (Name, DisplayText, Description, Category, IsListable, Source). All source-specific fields are stored as metadata objects in the template's `Properties` using `As<T>()`/`Put<T>()`:
  - `ProfileTemplateMetadata` — Profile type, deployments, orchestration settings, system message, model parameters, tool names, etc.
...
- **Markdown front matter** — Profile templates use the same parser as prompt templates. Profile-specific keys (`ProfileType`, `ChatDeploymentId`, `UtilityDeploymentId`, `Temperature`, `ToolNames`, etc.) are extracted from `AdditionalProperties`.
...
- AI Memory indexing and de-indexing now run from deferred AI memory catalog entry handlers instead of individual tools, so all create, update, and delete flows keep the configured master index synchronized.
...
- **Legacy profile deployment backfill** — The typed deployment migration now also backfills `AIProfile.ChatDeploymentId` for older profiles that still only carry the legacy `ConnectionName`. The migration resolves the default chat deployment for that connection in a deferred task so existing profiles keep working after connection-based runtime fallback is removed.
- **AI Profile connection cleanup** — AI Profiles and Profile-source AI Templates no longer carry a separate `ConnectionName` setting at runtime. The selected deployment now determines the provider connection, which removes duplicate configuration between profiles/templates and deployments. The legacy `ConnectionName` properties remain only for backward-compatible data loading, are marked obsolete, and are no longer used internally.
...
- **`Microsoft.Extensions.AI` text-to-speech abstraction adopted** — The solution now uses the framework-provided `ITextToSpeechClient`, `TextToSpeechOptions`, `TextToSpeechResponse`, and streaming update types from `Microsoft.Extensions.AI` instead of maintaining duplicate local copies of those abstractions.
- **Speech voice lookup moved out of `IAIClientFactory`** — Voice enumeration now resolves through a dedicated `ISpeechVoiceResolver` service. `IAIClientFactory` remains focused on returning AI clients, while provider-level voice discovery continues through `IAIClientProvider.GetSpeechVoicesAsync(...)`.
