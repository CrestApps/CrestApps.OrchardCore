...
- **Browser tool definition normalization** — Browser tool JSON schemas now follow the same inline `JsonSerializer.Deserialize<JsonElement>( """ ... """ )` formatting pattern as the rest of the AI Agent tools, and browser category labels are registered with literal localizer strings so Orchard Core extraction can localize them correctly.
