...
- **Markdown front matter** — Profile templates use the same parser as prompt templates. Profile-specific keys (`ProfileType`, `ChatDeploymentId`, `UtilityDeploymentId`, `Temperature`, `ToolNames`, etc.) are extracted from `AdditionalProperties`. Legacy `ConnectionName` metadata is still read for backward compatibility when older templates are loaded.
