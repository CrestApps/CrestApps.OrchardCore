...
- **WebSocket External Chat Relay** — New infrastructure for real-time bidirectional communication with third-party live-agent platforms via WebSocket connections. Unlike the webhook pattern (where the external system calls back into your application), a WebSocket relay maintains a persistent connection so events like typing indicators, agent-connected notifications, wait-time updates, and messages flow instantly without polling. Key abstractions: `IExternalChatRelay` (persistent connection interface), `IExternalChatRelayManager` (singleton lifecycle manager), `IExternalChatRelayEventHandler` (event-to-notification router). The default `DefaultExternalChatRelayEventHandler` automatically maps relay events to `IChatNotificationSender` calls. Supports signals (thumbs up/down, user feedback) sent back to the external system. See the [Response Handlers documentation](../ai/response-handlers.md#step-4-real-time-communication-via-websocket-alternative-to-webhook) for a complete implementation example.
