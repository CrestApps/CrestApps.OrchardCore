...
- **Dual-source templates** — Define templates via admin UI (database-stored, runtime-managed) or as `.md` files in `AITemplates/Profiles/` directories (file-based, read-only).
- **Template application** — When creating a new AI Profile, select a template from the dropdown and click **Apply** to pre-fill the form. All display drivers render the pre-populated values. Template properties are copied to both `profile.Properties` and `profile.Settings`, ensuring all profile drivers can read applied values.
- **Admin CRUD** — Full create, read, update, delete management for database-stored templates at **Artificial Intelligence → Profile Templates**.
- **Tabbed editor** — The template editor mirrors the AI Profile editor with the same tabs and sections (Capabilities, Documents, Data Processing & Metrics), so templates can configure all aspects of a profile.
- **External module drivers** — Every module that adds a `DisplayDriver<AIProfile>` also provides a `DisplayDriver<AIProfileTemplate>` so templates capture the full profile configuration:
...
- **Connection & Orchestrator dropdowns** — Connection name and orchestrator name use dropdown selectors populated from configured providers.
- **Profile type toggle** — The UI conditionally shows Chat-specific or TemplatePrompt-specific fields based on the selected profile type.
- **`AIProfileTemplateIndex`** — YesSql MapIndex for efficient querying by `Name`, `Category`, `ProfileType`, and `IsListable`.
- **`IAIProfileTemplateService`** — Unified service merging database and file-based templates. Database templates take precedence when names conflict.
- **`INamedCatalogManager<AIProfileTemplate>`** / **`INamedCatalog<AIProfileTemplate>`** — Standard CatalogItem lifecycle management following the `NamedDocumentCatalog` pattern. Templates are source-independent.
