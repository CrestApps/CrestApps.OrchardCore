...
- **Chat UI Notifications** — New extensible notification bubble system that allows C# code to send transient UI feedback to the chat interface via SignalR — no JavaScript required. Built-in notifications include typing indicators ("Mike is typing…"), transfer status with estimated wait times and cancel buttons, and conversation/session ended indicators. The `IChatNotificationSender` interface provides `SendAsync`, `UpdateAsync`, and `RemoveAsync` methods. Extension methods (`ShowTypingAsync`, `ShowTransferAsync`, `ShowConversationEndedAsync`, etc.) simplify common scenarios. Notification bubbles support action buttons that trigger server-side `IChatNotificationActionHandler` callbacks. Built-in action handlers: `cancel-transfer` (resets handler to AI) and `end-session` (closes the session). Custom notification types with custom actions and styling are fully supported. See the [Chat UI Notifications documentation](../ai/chat-notifications.md) for details.
