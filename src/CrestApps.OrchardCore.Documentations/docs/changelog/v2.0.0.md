...
- **Real-Time Streaming**: Audio is streamed in chunks via SignalR to the server, where it is transcribed in near real-time using `ISpeechToTextClient`. Transcript text is sent back to the UI as the user speaks.
