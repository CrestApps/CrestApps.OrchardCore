...
- **Agent Selection UI**: AI Profiles, AI Templates, and Chat Interactions now include an **Agents** section under the Capabilities tab, displaying available agents as checkboxes. Agents are listed separately from tools for clarity.
- **System Agents**: Agents can be marked as system agents (`AgentMetadata.IsSystemAgent`), which are always included in completion requests regardless of user selection — similar to system tools.
...
- **Built-in Agent Templates**: Three agent templates are included out of the box: **Planner Agent** (structured task planning), **Research Agent** (information gathering and synthesis), and **Executor Agent** (step-by-step plan execution).
- **Template Support**: `ModuleAIProfileTemplateProvider` and `AppDataAIProfileTemplateProvider` now support `AgentNames` and `ProfileDescription` in template front matter metadata.
