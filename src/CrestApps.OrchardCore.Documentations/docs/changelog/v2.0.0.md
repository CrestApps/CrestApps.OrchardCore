...
- **Speech-to-Text (Voice Input)** — AI Chat and Chat Interactions now support speech-to-text input. Users can click a microphone button to record speech, which is streamed to the server via SignalR and transcribed using the configured `ISpeechToTextClient` (e.g., OpenAI Whisper, Azure OpenAI Whisper). The transcribed text appears in the input field for review before sending. For AI Chat, enable via the **Enable speech-to-text** checkbox on AI Profiles or AI Profile Templates. For Chat Interactions, enable via the site-level setting under **Settings → Artificial Intelligence → Chat Interactions**. Both require a **Default Speech-to-Text Deployment** to be configured in default deployment settings.
...
- **Microphone Input**: AI Chat profiles, Chat Interactions, and the frontend chat widget now support voice input via a microphone button. When enabled, users can speak their prompts instead of typing.
- **Real-Time Streaming**: Audio is streamed in chunks (~1 second intervals) via SignalR to the server, where it is transcribed in near real-time using `ISpeechToTextClient`. Transcript text is sent back to the UI as the user speaks — words appear progressively, providing immediate feedback. Errors (e.g., authentication failures) are reported immediately and recording stops automatically.
- **Profile/Template Setting**: A new **Enable Speech to Text** checkbox on AI Profiles and AI Profile Templates (Profile source) controls whether the microphone button appears. The checkbox is only visible when a default Speech-to-Text deployment is configured in site settings.
- **Chat Interaction Setting**: A site-level setting under **Settings → AI → Chat Interaction Speech Settings** enables speech-to-text for all Chat Interaction UIs.
- **Deployment Requirement**: The microphone button only appears when both the profile/interaction setting is enabled **and** a valid `DefaultSpeechToTextDeploymentId` is configured in the Default AI Deployment Settings.
