...
namespace CrestApps.Core.ContactCenter.Hubs;

/// <summary>
/// The real-time Contact Center, independent of the host it runs in.
/// </summary>
/// <remarks>
/// <para>
/// Agents connect to receive presence, offer and queue updates, and to sign in, change presence and
/// change memberships without reloading the page; supervisors connect to watch queues. Every
/// invocation runs in its own unit of work, and every invocation that changes a session re-asks
/// whether the caller may take agent work, because a grant can be withdrawn while a connection is
/// still up.
/// </para>
/// <para>
/// Everything host-specific is asked as a question rather than answered here: whether the caller may
/// do a thing, which tenant the connection belongs to, and how to open a unit of work. A host binds
/// those three; this class names no web framework of its own beyond SignalR.
/// </para>
/// </remarks>
public abstract class ContactCenterHubBase : Hub<IContactCenterHubClient>
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ContactCenterHubBase"/> class.
    /// </summary>
    /// <param name="logger">The logger.</param>
    /// <param name="scopeExecutor">The executor that isolates each hub operation in its own unit of work.</param>
    ...
    protected ContactCenterHubBase(
        ILogger logger,
    ...
    public override async Task OnConnectedAsync()
    {
        ...
        {
            if (await AuthorizeAsync(services, ContactCenterOperations.TakeAgentWork))
            {
                authorized = true;
            ...
            if (await AuthorizeAsync(services, ContactCenterOperations.MonitorContactCenter))
            {
                authorized = true;
        ...
        await Groups.AddToGroupAsync(
            Context.ConnectionId,
            TenantSignalRGroupName.ForUser(_tenantAccessor.TenantName, userId),
            HubConnectionWork.MustComplete);
    ...
    /// </summary>
    /// <returns>A task that completes when the heartbeat is recorded.</returns>
    ...
    /// <returns>A task that completes when the subscription has been decided.</returns>
    ...
    public async Task<AgentDesktopSnapshot> SignIn(IList<string> queueIds, IList<string> campaignIds)
    {
        ...
        await _scopeExecutor.ExecuteAsync<ContactCenterHubScopeContext>(async services =>
        {
            await EnsureAuthorizedAsync(services, ContactCenterOperations.TakeAgentWork);

            var previousSnapshot = await services.SessionService.BuildSnapshotAsync(userId, HubConnectionWork.MustComplete);
            var normalizedQueueIds = AgentMembershipIds.Normalize(queueIds);
            var normalizedCampaignIds = AgentMembershipIds.Normalize(campaignIds);
        ...
        OperationAuthorizationRequirement operation)
    {
        ...
        var result = await services.AuthorizationService.AuthorizeAsync(httpContext.User, (object)null, operation);
    ...
    private async Task EnsureAuthorizedAsync(
        ...
        OperationAuthorizationRequirement operation)
    {
        if (!await AuthorizeAsync(services, operation))
        {
            throw new HubException($"The current user is not authorized for '{operation.Name}'.");
        }
    }
    ...
    {
        return TenantSignalRGroupName.ForGroup(_tenantAccessor.TenantName, QueueGroup(queueId));
    }

    private string GetSupervisorsGroup()
    {
        return TenantSignalRGroupName.ForGroup(_tenantAccessor.TenantName, SupervisorsGroup);
    }
}
