...
public sealed class DefaultInteractionEventUpcastService : IInteractionEventUpcastService
{
    ...
    /// property of the algorithm, so a caller can drive a chain of several steps without the shipped constant
    /// having to move first.
    /// </summary>
    /// <param name="upcasters">The registered upcasters.</param>
    /// <param name="currentVersion">The schema version events are converted up to.</param>
    public DefaultInteractionEventUpcastService(IEnumerable<IInteractionEventUpcaster> upcasters, int currentVersion)
    {
