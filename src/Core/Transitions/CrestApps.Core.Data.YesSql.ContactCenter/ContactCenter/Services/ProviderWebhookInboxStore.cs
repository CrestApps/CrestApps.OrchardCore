...
public sealed class ProviderWebhookInboxStore : ConcurrentDocumentCatalog<ProviderWebhookInboxMessage, ProviderWebhookInboxMessageIndex>, IProviderWebhookInboxStore
{
    ...
    public async Task<IReadOnlyCollection<ProviderWebhookInboxMessage>> GetDueAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var take = maxCount <= 0 ? ContactCenterStorage.WebhookInboxBatchSize : maxCount;
    ...
    public async Task<IReadOnlyCollection<ProviderWebhookInboxMessage>> GetProcessedBeforeAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var take = maxCount <= 0 ? ContactCenterStorage.WebhookInboxTombstoneCleanupBatchSize : maxCount;
