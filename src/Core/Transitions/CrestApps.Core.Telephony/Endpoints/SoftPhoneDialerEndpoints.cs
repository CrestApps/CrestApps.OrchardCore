...
namespace CrestApps.Core.Telephony.Endpoints;
...
public static class SoftPhoneDialerEndpoints
{
    ...
    {
        if (!(await authorizationService.AuthorizeAsync(httpContext.User, (object)null, TelephonyOperations.UseSoftPhone)).Succeeded)
        {
        ...
        await notifier.RequestDialAsync(userId, new TelephonyDialRequest { Number = number });
