...
internal sealed class CallSessionIndexMigrationsSchemaMigration : ISchemaMigration
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="CallSessionIndexMigrationsSchemaMigration"/> class.
    /// </summary>
    ...
    public CallSessionIndexMigrationsSchemaMigration(
        IStore store,
    ...
    /// </remarks>
    public string Name => "CallSessionIndexMigrations";
    ...
    public async Task<int> CreateAsync(ISchemaBuilder builder)
    {
        ...
        await builder.AlterIndexTableAsync<CallSessionIndex>(table => table
    ...
    private async Task EnsureNoDuplicateClaimKeysAsync(ISchemaBuilder builder)
    {
        var quotedTableName = ContactCenterMigrationSql.GetQuotedTableName(builder, _store, typeof(CallSessionIndex));
        var claimColumn = builder.Dialect.QuoteForColumnName("ProviderCallClaimKey");
    ...
    private static async Task EnsureItemIdentifiersPresentAsync(ISchemaBuilder builder, string quotedTableName)
    {
    ...
    /// </remarks>
    private async Task CanonicalizeAndBackfillClaimKeysAsync(ISchemaBuilder builder, string quotedTableName)
    {
        var itemIdColumn = builder.Dialect.QuoteForColumnName("ItemId");
        var providerNameColumn = builder.Dialect.QuoteForColumnName("ProviderName");
        var providerCallColumn = builder.Dialect.QuoteForColumnName("ProviderCallId");
        var claimColumn = builder.Dialect.QuoteForColumnName("ProviderCallClaimKey");

        await CanonicalizeProviderNamesAsync(builder, quotedTableName, providerNameColumn);

        var claimKeyExpression = BuildClaimKeyExpression(builder, providerNameColumn, providerCallColumn);

        await EnsureNoDuplicateClaimsAsync(builder, quotedTableName, providerCallColumn, claimKeyExpression);
    ...
    private async Task CanonicalizeProviderNamesAsync(ISchemaBuilder builder, string quotedTableName, string providerNameColumn)
    {
    ...
    private static string BuildClaimKeyExpression(ISchemaBuilder builder, string providerNameColumn, string providerCallColumn)
    {
    ...
    private static async Task EnsureNoDuplicateClaimsAsync(ISchemaBuilder builder,
    ...
    private async Task<int> UpdateFrom1Async(ISchemaBuilder builder)
    {
                    ...
                    await EnsureItemIdentifiersPresentAsync(builder, quotedTableName);
                    ...
                    await CanonicalizeAndBackfillClaimKeysAsync(builder, quotedTableName);
    ...
    /// <param name="builder">The schema builder.</param>
    /// <returns>The version the schema is now at.</returns>
    private async Task<int> UpdateFrom3Async(ISchemaBuilder builder)
    {
                    ...
                    await EnsureNoDuplicateClaimKeysAsync(builder);
