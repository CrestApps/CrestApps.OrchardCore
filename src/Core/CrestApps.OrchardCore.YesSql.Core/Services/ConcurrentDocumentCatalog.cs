...
/// as individual YesSql documents with a corresponding index, with optional document-version
...
public class ConcurrentDocumentCatalog<T, TIndex> : ICatalog<T>
    where T : CatalogItem
    where TIndex : CatalogItemIndex
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ConcurrentDocumentCatalog{T, TIndex}"/> class.
    /// </summary>
    /// <param name="session">The YesSql session for database access.</param>
    public ConcurrentDocumentCatalog(ISession session)
    {
        Session = session;
    }
