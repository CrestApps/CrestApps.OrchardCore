...
/// <remarks>
/// <para>
/// The browser sends call control here - dial, hang up, hold, transfer - and this routes it to the
/// configured provider through <see cref="ITelephonyService"/>. Every invocation runs in its own unit
/// of work, and every invocation re-asks whether the caller may use the soft phone at all, because a
/// grant can be withdrawn while a connection is still up.
/// </para>
/// <para>
/// Everything host-specific is asked as a question rather than answered here: whether the caller may
/// use the soft phone, which tenant the connection belongs to, and how to open a unit of work. A host
/// binds those three; this class names no web framework of its own beyond SignalR.
/// </para>
/// </remarks>
public abstract partial class TelephonyHubBase : Hub<ITelephonyClient>
{
    private readonly ILogger _logger;
    private readonly IScopedWorkExecutor _scopeExecutor;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="TelephonyHubBase"/> class.
    /// </summary>
    ...
    public override async Task OnConnectedAsync()
    {
        ...
        {
            authorized = await AuthorizeAsync(services);
        });
    ...
    public async Task<TelephonyClientCredentials> GetCredentials()
    {
        ...
        await _scopeExecutor.ExecuteAsync(async services =>
        {
            ...
            var service = services.GetRequiredService<ITelephonyService>();
    ...
    public async Task<TelephonyConnectionStatus> GetConnectionStatus()
    {
        ...
        await _scopeExecutor.ExecuteAsync(async services =>
        {
            ...
            var authenticationService = services.GetService<ITelephonyAuthenticationService>();
    ...
    public async Task RevokeSupersededCredential(string credentialId)
    {
        ...
        await _scopeExecutor.ExecuteAsync(async services =>
        {
            ...
            foreach (var revoker in services.GetServices<ISoftPhoneCredentialRevoker>())
            {
    ...
    public async Task ReportCredentialRegistered(string credentialId)
    {
        ...
        await _scopeExecutor.ExecuteAsync(async services =>
        {
            ...
            foreach (var registrar in services.GetServices<ISoftPhoneCredentialRegistrar>())
            {
    ...
    public async Task<TelephonyCallLookupResult> GetActiveCall()
    {
        ...
        await _scopeExecutor.ExecuteAsync(async services =>
        {
            ...
            var synchronizationService = services.GetRequiredService<ITelephonyInteractionSynchronizationService>();
        ...
        Func<IServiceProvider, CancellationToken, Task<TelephonyResult>> preflight = null)
    {
        ...
        await _scopeExecutor.ExecuteAsync(async services =>
        {
            ...
            {
                var refusal = await preflight(services, CancellationToken.None);
            ...
            var service = services.GetRequiredService<ITelephonyService>();
            var commandExecutor = services.GetRequiredService<ITelephonyCommandExecutor>();
    ...
    public async Task RecordBrowserCall(string callId, string to, string from)
    {
        ...
        await _scopeExecutor.ExecuteAsync(async services =>
        {
            ...
            await RecordInteractionAsync(services, "Dial", call, Context.ConnectionAborted);
        });
    }
    ...
    private async Task<bool> AuthorizeAsync(IServiceProvider services)
    {
        ...
        var result = await authorizationService.AuthorizeAsync(httpContext.User, (object)null, TelephonyOperations.UseSoftPhone);
