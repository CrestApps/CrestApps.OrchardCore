...
/// through it, so a workspace whose numbers span multiple carriers routes each send to the correct provider.
/// </summary>
public sealed class SmsDispatcher : ISmsDispatcher
{
    ...
    public async Task<MessageDispatchResult> SendAsync(SmsMessage message, CancellationToken cancellationToken = default)
    {
        ...
        {
            return MessageDispatchResult.Failed("The sending number (From) is required to resolve a provider.");
        }
        ...
        else
        {
            ...
            dispatch = result.Succeeded
                ? MessageDispatchResult.Success()
                : MessageDispatchResult.Failed((result.Errors ?? []).Select(error => error.Message).ToArray());
        }
    ...
    // is what stops the workspace, broadcasts and the outbox from trying again.
    private async Task RecordOptOutAsync(string recipient, CancellationToken cancellationToken)
    {
        var contactContentItemId = string.IsNullOrWhiteSpace(recipient)
            ? null
            : await _contactResolver.ResolveContactContentItemIdAsync(OmnichannelConstants.Channels.Sms, recipient.GetCleanedPhoneNumber(), cancellationToken);
