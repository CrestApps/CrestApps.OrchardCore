...
public sealed class SmsDispatcher : ISmsDispatcher
{
    ...
    /// <param name="siteService">The site service used to read the workspace and built-in SMS settings.</param>
    ...
    public async Task<MessageDispatchResult> SendAsync(SmsMessage message, CancellationToken cancellationToken = default)
    {
        ...
        {
            return await dispatchProvider.DispatchAsync(message, cancellationToken);
        }
