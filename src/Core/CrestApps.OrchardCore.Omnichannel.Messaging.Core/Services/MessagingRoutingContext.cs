...
/// </summary>
public sealed class MessagingRoutingContext
{
    /// <summary>
    ...
    /// </summary>
    public MessagingRoutingTrigger Trigger { get; init; } = MessagingRoutingTrigger.Inbound;

    /// <summary>
    /// Gets the channel the conversation runs on. The router fills it in from the conversation when the caller
    /// did not.
    /// </summary>
    public IMessagingChannel Channel { get; set; }

    /// <summary>
    /// Gets a value indicating whether automation must stay silent for this pass, because the channel has already
    /// given the message the one answer it is owed (a carrier keyword confirmation, for example).
    /// </summary>
    public bool SuppressAutomatedReplies { get; init; }
    ...
    /// </summary>
    public IMessagingInboundRouter ClaimedBy { get; set; }
}
