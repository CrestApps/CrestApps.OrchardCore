...
/// <summary>
/// The messaging-workspace implementation of <see cref="IOmnichannelHandoffService"/>, for every enabled
/// messaging channel. When an automated conversation on one of them escalates, it creates (or re-opens) the human
/// <see cref="MessagingConversation"/> owned by the target queue, hydrates it with the prior automated transcript
/// so the agent inherits the full context, and announces the hand-off so the inbox lights up. From that point the
/// workspace's inbound routing keeps every reply in the human thread.
/// </summary>
public sealed class MessagingAgentHandoffService : IOmnichannelHandoffService
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="MessagingAgentHandoffService"/> class.
    /// </summary>
    public MessagingAgentHandoffService(
        ...
        ISession session,
        IClock clock,
        ILogger<MessagingAgentHandoffService> logger)
    {
    ...
    public async Task<OmnichannelHandoffResult> RequestHandoffAsync(OmnichannelHandoffRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var channel = _channelResolver.Get(activity.Channel);

        if (channel is null)
        {
            return OmnichannelHandoffResult.Failure("The handoff channel is not an enabled messaging channel.");
        }

        var serviceAddress = channel.NormalizeAddress(request.ServiceAddress);
        var contactAddress = channel.NormalizeAddress(request.ContactAddress);
        ...
        await _router.RouteAsync(
            ...
            {
                Trigger = MessagingRoutingTrigger.Handoff,
                Channel = channel,
                Endpoint = endpoint,
        ...
        MessagingConversationRollup.ApplyInbound(conversation, lastContent, now, now, importedCount);
        ...
        if (_logger.IsEnabled(LogLevel.Information))
        {
            _logger.LogInformation(
                "Handed off automated {Channel} Activity {ActivityId} to queue {QueueId} as conversation {ConversationId}.",
                channel.Name,
        ...
        return OmnichannelHandoffResult.Success("Handed off to the messaging queue.", conversation.ItemId);
    }
