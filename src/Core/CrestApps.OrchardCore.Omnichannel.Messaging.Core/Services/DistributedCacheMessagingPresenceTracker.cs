...
/// <summary>
/// Keeps workspace presence in the distributed cache. A heartbeat is a fact that expires, not a record worth a
/// document write every few seconds per agent, and the cache is shared across nodes wherever the deployment
/// has configured it to be, so an agent whose workspace is open on one node is present to the router on every
/// other.
/// </summary>
public sealed class DistributedCacheMessagingPresenceTracker : IMessagingPresenceTracker
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="DistributedCacheMessagingPresenceTracker"/> class.
    /// </summary>
    ...
    public DistributedCacheMessagingPresenceTracker(
    ...
    private string GetKey(string agentId)
        => $"MessagingPresence:{_tenantName}:{agentId}";
}
