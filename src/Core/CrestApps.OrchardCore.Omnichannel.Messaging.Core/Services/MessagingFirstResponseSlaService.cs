...
/// <summary>
/// Default <see cref="IMessagingFirstResponseSlaService"/>. The deadline lives on the conversation so the same record
/// the inbox renders is the one the sweep reads, and a breach is announced exactly once.
/// </summary>
public sealed class MessagingFirstResponseSlaService : IMessagingFirstResponseSlaService
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="MessagingFirstResponseSlaService"/> class.
    /// </summary>
    public MessagingFirstResponseSlaService(
        ...
        IClock clock,
        ILogger<MessagingFirstResponseSlaService> logger)
    {
    ...
    /// <inheritdoc/>
    public async Task ApplyFirstResponseTargetAsync(MessagingConversation conversation, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<int> EscalateOverdueAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var conversation in overdue)
        {
            ...
            await _notifier.FirstResponseBreachedAsync(
                new MessagingFirstResponseBreachNotification
                {
        ...
        {
            _logger.LogInformation("Announced {Count} first-response breach(es).", escalated);
        }
