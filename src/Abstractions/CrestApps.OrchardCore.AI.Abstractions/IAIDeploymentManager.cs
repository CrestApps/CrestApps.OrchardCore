...
public interface IAIDeploymentManager
{
    ...
    /// </returns>
    Task<bool> DeleteAsync(AIDeployment profile);
    ...
    /// <returns>
    /// A task that represents the asynchronous operation. The result is the <see cref="AIDeployment"/> corresponding
    /// to the specified ID, or <c>null</c> if no deployment with the specified ID is found.
    /// </returns>
    Task<AIDeployment> FindByIdAsync(string id);
    ...
    /// <returns>
    /// A task that represents the asynchronous operation. The result is a <see cref="AIDeploymentResult"/> containing
    /// the paginated list of model deployments and any relevant metadata (such as total count, etc.).
    /// </returns>
    Task<AIDeploymentResult> PageQueriesAsync(int page, int pageSize, QueryContext context);
    ...
    /// </returns>
    Task<IEnumerable<AIDeployment>> GetAllAsync();
    ...
    /// </returns>
    Task SaveAsync(AIDeployment profile);
    ...
    /// A task that represents the asynchronous operation. This method does not return any value.
    /// </returns>
    Task UpdateAsync(AIDeployment profile, JsonNode data = null);
    ...
    /// <returns>
    /// A task that represents the asynchronous operation. The result is a <see cref="AIDeploymentValidateResult"/>
