...
public interface IModelDeploymentManager
{
    ...
    /// </returns>
    Task<bool> DeleteAsync(ModelDeployment profile);
    ...
    /// <returns>
    /// A task that represents the asynchronous operation. The result is the <see cref="ModelDeployment"/> corresponding
    /// to the specified ID, or <c>null</c> if no deployment with the specified ID is found.
    /// </returns>
    Task<ModelDeployment> FindByIdAsync(string id);
    ...
    /// <returns>
    /// A task that represents the asynchronous operation. The result is the newly created <see cref="ModelDeployment"/>.
    /// </returns>
    Task<ModelDeployment> NewAsync(string source, JsonNode data = null);
    ...
    /// <returns>
    /// A task that represents the asynchronous operation. The result is a <see cref="ModelDeploymentResult"/> containing
    /// the paginated list of model deployments and any relevant metadata (such as total count, etc.).
    /// </returns>
    Task<ModelDeploymentResult> PageQueriesAsync(int page, int pageSize, QueryContext context);
    ...
    /// </returns>
    Task<IEnumerable<ModelDeployment>> GetAllAsync();
    ...
    /// </returns>
    Task SaveAsync(ModelDeployment profile);
    ...
    /// A task that represents the asynchronous operation. This method does not return any value.
    /// </returns>
    Task UpdateAsync(ModelDeployment profile, JsonNode data = null);
    ...
    /// <returns>
    /// A task that represents the asynchronous operation. The result is a <see cref="ModelDeploymentValidateResult"/>
    /// containing the validation results (e.g., success or failure and any associated errors).
    /// </returns>
    Task<ModelDeploymentValidateResult> ValidateAsync(ModelDeployment profile);
}
