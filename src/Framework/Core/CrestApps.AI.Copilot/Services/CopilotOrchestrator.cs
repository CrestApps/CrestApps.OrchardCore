...
namespace CrestApps.AI.Copilot.Services;
...
public sealed class CopilotOrchestrator : IOrchestrator
{
    ...
    private const string TokenProtectorPurpose = "CrestApps.AI.Copilot.GitHubTokens";
    ...
    /// </summary>
    private async Task<CopilotClientOptions> BuildClientOptionsAsync(
        OrchestrationContext context,
        CopilotSessionMetadata metadata,
        CopilotOptions settings)
    {
        ...
        return settings.AuthenticationType switch
        {
            ...
            CopilotAuthenticationType.ApiKey =>
                ...
                (!string.Equals(settings.ProviderType, "azure", StringComparison.OrdinalIgnoreCase) ||
                    (!string.IsNullOrWhiteSpace(settings.AzureApiVersion) && !string.IsNullOrWhiteSpace(settings.ApiKey))
                ),
            _ => false,
        };
    }
