...
When multiple legacy deployment rows target the same provider model name, the migration no longer collapses them onto a read-only configuration deployment. Instead, it merges compatible types onto a writable deployment record. Legacy interactive deployments are normalized so `Chat` and `Utility` support travel together, and if the conflicting deployment name is already reserved by a read-only or unrelated record, the migration creates a database-backed deployment with a unique `-migrated` suffix while preserving the original provider model name in `ModelName`.

The same startup migration pass now also backfills `DefaultAIDeploymentSettings` for upgraded tenants. If a legacy default deployment name cannot be matched exactly, the migration falls back to the first deployment that supports the requested type so chat and utility defaults are always populated after import.
