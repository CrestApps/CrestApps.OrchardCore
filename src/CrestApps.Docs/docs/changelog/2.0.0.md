...
Several recipe steps now compose their schema from extensible, per-provider contributions, so a custom module can describe its own shapes:

- The `Layers` step describes every rule condition and operator. See [Rule Condition Schemas](../modules/rule-condition-schemas.md).
- The `Sitemaps` step describes every sitemap source. See [Sitemap Source Schemas](../modules/sitemap-source-schemas.md).
- The `deployment` step describes every deployment step payload. See [Deployment Step Schemas](../modules/deployment-step-schemas.md).
- The `AdminMenu` step describes every admin menu node. See [Admin Menu Node Schemas](../modules/admin-menu-node-schemas.md).
- The `Queries` step describes every query source. See [Query Source Schemas](../modules/query-source-schemas.md).
- The `UrlRewriting` step describes every rewrite rule source. See [URL Rewrite Rule Schemas](../modules/url-rewrite-rule-schemas.md).
- The `Placements` step describes every placement node filter. See [Placement Node Filter Schemas](../modules/placement-node-filter-schemas.md).

Wherever a property accepts a known tenant value, the schema now lists live example values drawn from the current tenant, such as content types, content parts, cultures, roles, and index profile names. These examples are non-restrictive suggestions, so custom values stay valid.
...
Dependabot now waits 14 days after a NuGet package, npm package, or GitHub Action release before opening an update PR. This adds a review buffer for newly published supply-chain artifacts while preserving the existing weekly update cadence.
