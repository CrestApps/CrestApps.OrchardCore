...
Version `2.0.0` is a major CrestApps.OrchardCore release for **.NET 10** and **Orchard Core 3.0.x**. It introduces a much broader set of modules for AI, content operations, omnichannel communication, phone-number intelligence, telephony, reporting, recipes, and administration.
...
Applications using CrestApps.OrchardCore `2.0.0` should run on .NET 10 and Orchard Core 3.0.x. Update the application target framework, package references, and package feeds before starting upgraded tenants.

### AI configuration review

If you used the `1.x` AI modules, review your provider connections, deployments, default deployments, profiles, profile templates, prompt templates, and chat settings after upgrade. The `2.0.0` AI experience is organized around reusable provider connections and deployment records instead of older provider-specific selectors.
