...
The shared date-range picker now seeds empty custom ranges with today's local date from `00:00:00` through `23:59:59`. Empty **On or before** pickers default to today at `23:59:59`, and empty **On or after** pickers default to today at `00:00:00`. Report pages now use that same one-day default period instead of the previous last-30-days range.
