...
Version `2.1.0` is a service release for the .NET 10 and Orchard Core 3.0 line. It keeps the Orchard Core package range at `>= 3.0.0` and `< 3.1.0`, and updates the CrestApps.Core package line to stable `1.1.0`.

## At a glance

| Area | 2.1.0 focus |
| --- | --- |
| Platform | .NET 10, Orchard Core `>= 3.0.0` and `< 3.1.0`, CrestApps.Core `1.1.0`, ModelContextProtocol `2.0.0`, and YesSql `6.0.0`. |
...
Applications using CrestApps.OrchardCore `2.1.0` should stay on Orchard Core `>= 3.0.0` and `< 3.1.0`. This release uses the stable CrestApps.Core `1.1.0` packages, not preview packages.

The stable CrestApps.Core `1.1.0` package line also moves the MCP SDK packages to ModelContextProtocol `2.0.0`, the YesSql packages to `6.0.0`, and Microsoft.Data.Sqlite to `10.0.10`. Optional CrestApps.Core provider and MCP resource adapter packages that do not have stable `1.1.0` packages available remain pinned to stable `1.0.0` packages; no CrestApps.Core preview packages are used.
