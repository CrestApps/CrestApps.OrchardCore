...
Another servicing fix corrects the Orchard Core configuration section registrations for AI deployments, connections, and providers so the integration now reads the intended `OrchardCore:CrestApps:AI:*` paths from `appsettings.json`. The older misspelled `OrcahrdCore:*` paths are still registered as compatibility fallbacks for existing environments.
