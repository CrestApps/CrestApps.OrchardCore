...
The Copilot editor experience also now shows Copilot model cost multipliers directly in the model selectors, adds a per-session **Effort level** override for profiles, profile templates, and chat interactions, and backfills the required AI Profile past-messages parameter with the default value when no explicit value has been saved yet.

Orchestrator servicing also adds a new `CrestApps.OrchardCore.AI.Chat.Claude` module that wires the shared `CrestApps.Core.AI.Claude` package into Orchard Core, including tenant-level Claude settings, Anthropic model discovery, availability checks, and Claude model/effort controls across AI Profiles, Chat Interactions, and Profile-source templates.
...
Another branch-migration servicing fix now imports legacy `DictionaryDocument<AIProfile>`, `DictionaryDocument<AIProviderConnection>`, `DictionaryDocument<AIDeployment>`, and `DictionaryDocument<AIDataSource>` records into the current persisted AI stores through deferred startup migrations. This keeps AI profiles, provider connections, typed deployments, and data sources visible in the admin UI after moving a tenant database forward from older `main`-branch builds that still wrote those records into the default document table. The same servicing path now also replays legacy profile documents through the current profile manager and rewrites the stored AI profile JSON in `AI_Document` to the current property layout, removing legacy nested `Properties` payloads and older metadata aliases from storage instead of carrying that compatibility logic in the runtime handler path. Together, those fixes preserve profile metadata and settings such as system prompts, token limits, analytics, tools, data extraction, post-session processing, and document attachments. It also remaps the renamed Azure connection metadata payload (`AzureOpenAIConnectionMetadata` → `AzureConnectionMetadata`) so migrated Azure endpoints, authentication mode, managed identity, and API keys survive the import.

AI Chat and Chat Interactions also now pass request-relative SignalR hub paths to the browser instead of absolute hub URLs derived from the tenant `BaseUrl`. This avoids negotiation failures when a tenant has a configured public `BaseUrl` but the current admin session is being served from a different origin such as localhost during local development.

Recent Core-alignment servicing also updates Orchard Core to the renamed shared AI document and MCP support packages (`CrestApps.Core.AI.Documents.*` and `CrestApps.Core.AI.Mcp.*`), which keeps restore/build working against the newer CrestApps.Core package set.
...
The Orchard Core AI cleanup also removes an unregistered external chat relay notification pipeline that was no longer wired into runtime DI, drops a duplicate Azure AI Inference client-provider registration, preserves legacy profile backfill behavior when deployment matching needs to fall back through `ConnectionNameAlias`, and clears mismatched restored chat sessions before session-document uploads so the attachment endpoint stays aligned with the current profile.
...
Recent chat UI servicing also repositions per-message actions to the bottom-right of assistant responses, keeps the play/pause icon synchronized with active text-to-speech playback, ensures a newly started playback stops any other active message playback, and hides per-message playback buttons while Conversation mode is active.

The Orchard Core admin and frontend chat widgets also now restore their persisted layout before the widget becomes visible, which removes the initial flash where the toggle or panel briefly appeared at the default position before snapping to the saved one.

Recent repository consistency servicing also aligns the checked-in Copilot guidance around shared coding preferences. The repository instructions now consistently prefer `IClock` over direct `DateTime.UtcNow` usage, call out JavaScript indentation expectations for `<script>` blocks embedded in `.cshtml` views, and tighten solution-wide layout analysis so routine formatting cleanup follows the documented blank-line conventions more closely. The same cleanup pass also fills in missing XML summaries on interface and domain-model methods, adds `<param>` documentation to interface methods that take parameters, normalizes property-to-doc spacing and multi-parameter constructor layout, removes a few leftover fully qualified namespace usages and decorative region/comment patterns, and makes the densest unit tests easier to scan with clearer Arrange/Act/Assert separation.

The session-document widgets also now render their attachment bar above the input with a smaller supported-formats note, which keeps the input width intact and reduces visual noise in both the admin and frontend widget layouts.

AI Documents also now includes an optional `CrestApps.OrchardCore.AI.Documents.Azure` storage module that swaps the default document file store for Orchard Core's Azure `BlobFileStore`, supports shell-tokenized base paths, and can create or clean up the Azure blob container through tenant lifecycle events.

The documentation now also calls out that local file-system storage is the default for uploaded AI documents and adds a dedicated Azure Blob Storage page with full `OrchardCore:CrestApps:AI:AzureDocuments` configuration guidance.
...
- **Update:** Orchard Core 3 also obsoletes its `As<T>()` helpers on `ISite`, `ContentItem`, and related entities. Current code paths now use `TryGet<T>(out ...)` for conditional reads and `GetOrCreate<T>()` when a default instance is required.

### Warning Cleanup: Orchard Core `As<T>()` Replacements

- **Fixed:** The solution now builds without warnings after replacing obsolete Orchard Core `As<T>()` calls in runtime code, Razor views, and tests with `TryGet<T>(out ...)` and `GetOrCreate<T>()`.
- **Fixed:** Search provider modules now declare their central package versions explicitly for `OrchardCore.Search.AzureAI.Core` and `OrchardCore.Search.Elasticsearch.Core`, keeping warning validation and restore behavior aligned across the full solution.
...
- **Updated** — The `@crestapps/ai-chat-ui` npm package has been upgraded to `1.0.0-preview.12` across AI Chat, Chat Interactions, and Resources. The module package manifests, lockfiles, CDN URLs, and SRI integrity hashes now all point at the published `1.0.0-preview.12` package.
- **Fixed** — The Orchard Core admin widget and frontend AI chat widget templates now match the current Core MVC widget layout for session documents. Attached files render in a dedicated bar above the input instead of shrinking the input horizontally, and the remove-file button remains visible for each attachment.

### Tenant-Scoped AI Documents Storage

- **Changed** — AI Documents now post-configures `DocumentFileSystemFileStoreOptions` so uploaded files are always stored under `wwwroot\<tenant-name>\AIDocuments`. This overrides the core package default path and keeps document files isolated per Orchard Core tenant while remaining web-root accessible for the module's document workflows.
