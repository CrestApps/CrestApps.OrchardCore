...
| Project | Description |
|---------|-------------|
| `CrestApps.AI.Chat.Copilot` | GitHub Copilot chat orchestration (`CopilotOrchestrator`, `GitHubOAuthService`, `CopilotOptions`, `ICopilotCredentialStore`). Uses `IOptions<CopilotOptions>` and `TimeProvider` instead of OrchardCore's `ISiteService` and `IClock`. |
| `CrestApps.AI.DataSources.AzureAI` | Azure AI Search data source content management (`AzureAISearchDataSourceContentManager`, `AzureAIODataFilterTranslator`, `DataSourceAzureAISearchDocumentReader`). Uses `SearchIndexClient` directly with no OrchardCore dependency. |
| `CrestApps.AI.DataSources.Elasticsearch` | Elasticsearch data source content management (`ElasticsearchDataSourceContentManager`, `ElasticsearchODataFilterTranslator`, `DataSourceElasticsearchDocumentReader`). |
...
- `datasource-context-header.md` → `CrestApps.AI/AITemplates/Prompts/`
- `tabular-batch-processing.md` → `CrestApps.AI.Chat/AITemplates/Prompts/`
