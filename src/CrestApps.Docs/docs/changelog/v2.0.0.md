...
Another branch-migration servicing fix now imports legacy `DictionaryDocument<AIProviderConnection>`, `DictionaryDocument<AIDeployment>`, and `DictionaryDocument<AIDataSource>` records into the current persisted AI stores through deferred startup migrations. This keeps provider connections, typed deployments, and data sources visible in the admin UI after moving a tenant database forward from older `main`-branch builds that still wrote those records into the default document table. The same servicing path now also remaps the renamed Azure connection metadata payload (`AzureOpenAIConnectionMetadata` → `AzureConnectionMetadata`) so migrated Azure endpoints, authentication mode, managed identity, and API keys survive the import.
