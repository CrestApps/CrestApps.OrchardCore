...
| `CrestApps.AI.AzureAI` | Azure AI Search provider integration (`AzureAISearchDataSourceContentManager`, `AzureAIODataFilterTranslator`, `DataSourceAzureAISearchDocumentReader`). Uses `SearchIndexClient` directly with no OrchardCore dependency. |
| `CrestApps.AI.Elasticsearch` | Elasticsearch provider integration (`ElasticsearchDataSourceContentManager`, `ElasticsearchODataFilterTranslator`, `DataSourceElasticsearchDocumentReader`). |
