...
| Project | Description |
|---------|-------------|
| `CrestApps.AI.Copilot` | GitHub Copilot chat orchestration (`CopilotOrchestrator`, `GitHubOAuthService`, `CopilotOptions`, `ICopilotCredentialStore`). Uses `IOptions<CopilotOptions>` and `TimeProvider` instead of OrchardCore's `ISiteService` and `IClock`. |
| `CrestApps.Azure.AISearch` | Azure AI Search provider integration (`AzureAISearchDataSourceContentManager`, `AzureAIODataFilterTranslator`, `DataSourceAzureAISearchDocumentReader`). Uses `SearchIndexClient` directly with no OrchardCore dependency. |
| `CrestApps.Elasticsearch` | Elasticsearch provider integration (`ElasticsearchDataSourceContentManager`, `ElasticsearchODataFilterTranslator`, `DataSourceElasticsearchDocumentReader`). |
| `CrestApps.AI.Resources` | Shared frontend JavaScript resources (`ai-chat.js`, `chat-interaction.js`) for AI chat UI, usable by any framework. |

### Handlers Moved to Framework Level

- `ChatInteractionCompletionContextBuilderHandler` → `CrestApps.AI.Chat`
- `ChatInteractionEntryHandler` → `CrestApps.AI.Chat`
- `DataSourceAICompletionContextBuilderHandler` → `CrestApps.AI`

### AI Prompt Templates Moved to Framework Level

- `datasource-context-header.md` → `CrestApps.AI/Templates/Prompts/`
- `tabular-batch-processing.md` → `CrestApps.AI.Chat/Templates/Prompts/`
...
- Updated MVC project to use shared `ai-chat.js` from `CrestApps.AI.Resources`.

**Breaking:** DataSource service namespaces changed from `CrestApps.OrchardCore.AI.DataSources.*` to `CrestApps.AI.DataSources.*`. Update `using` directives in any code referencing `AzureAISearchDataSourceContentManager`, `ElasticsearchDataSourceContentManager`, or related types.
