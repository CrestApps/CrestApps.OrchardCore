...
The Copilot editor experience also now shows Copilot model cost multipliers directly in the model selectors, adds a per-session **Effort level** override for profiles and chat interactions, and backfills the required AI Profile past-messages parameter with the default value when no explicit value has been saved yet.
