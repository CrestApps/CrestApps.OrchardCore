...
- upgrades also rerun the `PhoneNumberInfoPart.Number` backfill so legacy `Number.Text` values inside the `ContactMethods` bag are reparsed into `PhoneField` storage (`PhoneNumber`, `CountryCode`, and `NationalNumber`) when an earlier deferred migration was interrupted
