...
| Operator identity & queue membership | [Contact Center Agents & Work Distribution](../contact-center/agents-queues-dialer) | Operators are Contact Center **agent profiles**; the "my conversations" view is scoped by the agent's queue memberships. |
...
- **Contact Center Work Distribution** (`Queues`) — provides agent profiles, operator/queue entitlement management, and queue membership. This is where operators are onboarded.
...
It does **not** require Contact Center **Voice**, and it does not use the Contact Center real-time hub (it ships its own).

## Enable the feature

1. Go to **Tools → Features** in Orchard Core.
2. Enable **SMS Workspace**. Its dependencies (Channel Endpoints, Work Distribution, SMS, SignalR) are enabled automatically.
...
4. **Onboard operators.** Create Contact Center **agent profiles** for the people who will staff the inbox (Contact Center agent entitlements), and grant them the workspace permissions.
