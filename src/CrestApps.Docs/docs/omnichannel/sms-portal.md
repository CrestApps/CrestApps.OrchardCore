---
sidebar_label: SMS Portal
sidebar_position: 4
title: SMS Portal (Human Two-Way SMS)
description: A human-operated two-way SMS inbox, conversation portal, and broadcast console for Orchard Core.
---

| | |
| --- | --- |
| **Feature Name** | SMS Portal |
| **Feature ID** | `CrestApps.OrchardCore.Omnichannel.Sms.Portal` |
| **Optional add-on** | `CrestApps.OrchardCore.Omnichannel.Sms.Portal.RoutedDistribution` |
| **Category** | Contact Center |
| **Admin menu** | **SMS Portal** |

The **SMS Portal** is a **human-operated** two-way SMS console: an inbox, threaded conversations, a composer, canned-response templates, and 1:1 broadcasts, staffed by real operators.

It is the human counterpart to [SMS Automation](sms), which lets an **AI agent** carry an SMS conversation on its own. Both build on the same Omnichannel foundation and can run side by side on the same numbers — an automated activity handles a conversation until it hands off, after which the human portal owns the thread.

## Overview

The portal turns a set of SMS numbers into a shared team inbox:
...
The portal deliberately **reuses existing building blocks** instead of duplicating them:
...
Enabling **SMS Portal** pulls in only what it actually uses:
...
- **Contact Center Agent Services** (`CrestApps.OrchardCore.ContactCenter.AgentServices`) — a small **dependency-only** feature that provides just the shared agent-profile directory (store, manager, index). It carries **no** administration screens, so enabling the portal does **not** pull in the Contact Center Agents or Work Distribution admin (agent states, entitlements, business hours, queues, skills). Operator profiles are provisioned automatically on first portal access.
- **Orchard Core SMS** (`OrchardCore.Sms`) — the provider abstraction and the site SMS settings (default provider).
- **Orchard Core SignalR** — transport for the portal's own real-time hub.
...
If you also enable the full Contact Center features, the portal integrates with them automatically — the same operator profiles gain queues, entitlements, and presence, and queue-target SMS routing becomes meaningful.

## Enable the feature

1. Go to **Tools → Features** in Orchard Core.
2. Enable **SMS Portal**. Its dependencies (Channel Endpoints, Agent Services, SMS, SignalR) are enabled automatically.

## Setup

1. **Configure an SMS provider.** Enable and configure at least one provider — for example [Telnyx SMS](../telephony/telnyx#telnyx-sms) or Twilio — and pick the tenant **default provider** at **Settings → SMS** (`/Admin/Settings/sms`). The portal does **not** define its own default-provider setting; it uses Orchard Core's.
...
4. **Grant operator permissions.** Assign the portal permissions to the roles that will staff the inbox. A bare operator (agent) profile is created automatically the first time a permitted user opens the portal — no separate onboarding step is required. (If you also run the full Contact Center, manage richer entitlements there.)
...
6. **Open the portal** from **SMS Portal** in the admin menu and start handling **Conversations**.
