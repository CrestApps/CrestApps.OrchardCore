...
- `POST /bridges` plus `POST /bridges/{id}/addChannel` to merge two calls; the provider clears the prior hold marker, stamps the owned bridge id on both channels, and deletes the mixing bridge after the last tracked participant hangs up
