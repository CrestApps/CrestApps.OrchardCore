...
Pressing **Enter** while the number field is focused starts the call. While the selected call is connected, the field remains visible and disabled with the connected phone number formatted for display. After the selected call is placed on hold, the field and keypad become available for a second outbound call. The **Active calls** list shows every provider-authoritative in-progress interaction by phone number and state; selecting a row changes which call the individual hold, resume, mute, transfer, and hang-up controls operate on.

When at least two calls are active, select exactly two checkboxes and choose **Conference selected calls**. The shared provider contract currently merges two call identifiers per request, matching the Asterisk and DialPad adapters; adding more than two participants requires subsequent provider-supported conference operations. **Disconnect all calls** sends hang-up to every active call, while the regular hang-up button ends only the selected call. The current provider-neutral contract does not model a separate agent media leg or a leave-conference operation, so ending the selected call must not be described as leaving a conference while the other participants continue.
