...
Pressing **Enter** while the number field is focused starts the call. While a call is active, the field remains visible and disabled with the connected phone number formatted for display; the server-backed active-call restoration keeps that number visible after a page reload or SignalR reconnect.
