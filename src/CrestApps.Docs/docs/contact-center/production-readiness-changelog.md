...
## Review pass — verification of the implementation

A second, independent review of everything above, done after all 43 items were reported complete. The whole tree was built the way CI builds it (Release, warnings as errors, analyzers on), the unit, activation and JavaScript suites were run, and each item was read against its acceptance criteria rather than its change-log entry. Most of it held. What follows is what did not, and what was done about it. Every fix below has tests.

### Cross-queue selection dropped every campaign agent's campaign work

`AgentWorkSelector` (A1) looked each of an agent's queues up in the queue catalog and skipped any it could not find. Outbound campaign work is routed under a **virtual per-campaign queue that is never stored**, so the catalog never finds it, and every campaign an agent was signed into silently vanished from the selection. Preview-dial agents got no campaign offers at all. The selector's own test masked this with a queue manager stub that returned a queue for any identifier.

- The selector now resolves a campaign queue through `CampaignRoutingQueue.Create`, the same way the offer path does, and skips only queues that are genuinely missing or disabled.
- `SelectNextForAgentAsync` takes an **exclusion set**. `QueuedVoiceWorkOfferService` had ended its pass the moment the selector chose a paced campaign queue (which the dialer owns) or a queue that offered nobody, so an inbound caller waiting behind either was never reached. The pass now excludes that queue and re-selects, bounded by the agent's membership count.
- `QueueItemStore.GetHeadWaitingByQueueAsync` read every waiting item's **document** to pick one head per queue. It now reads index rows for the reduction and loads only the head documents, so the cost of the agent sync no longer grows with the backlog.
- Tests: `AgentWorkSelectorTests` (campaign queue selected, excluded queue skipped, disabled queue skipped), `QueuedVoiceWorkOfferServiceTests` (paced campaign ahead of an inbound queue still offers the inbound; a queue with nothing takeable yields to the next); the offer-service harness's catalog stub now returns nothing for a campaign identifier, as the real one does.

### Queue limits were stored and never enforced

A6 added `MaxWaitSeconds`/`MaxWaitAction` and `MaxQueueSize`/`QueueFullAction` to the queue, with a six-value action enum. Nothing read any of them: a queue "limited" to twenty callers took the twenty-first, and a caller past the maximum wait kept waiting. Four of the six actions (Callback, Transfer, HangUpWithMessage, and the automatic form of Callback) had no implementation behind them at all, so an operator could save a setting that did nothing.

- **`QueueMaxWaitAction` is now `None`, `Voicemail`, `Overflow`** — the actions the platform carries out. An option that saves and does nothing is worse than an absent one; the removed values can return with an implementation.
- **`IQueueLimitService`** (`QueueLimitService`) owns both limits. `AdmitAsync` decides whether a new caller may wait: under the size limit they are admitted; at it, `Voicemail` sends them there, and `Overflow` walks the overflow chain to the **first queue that exists, is enabled and has room under its own limit**, so a full queue never hands its overflow to another full one. When no overflow queue can take them, the caller is admitted over the limit and a warning is logged: a size limit spreads load, it does not hang up on people because of a configuration gap. `EnforceMaxWaitAsync` runs from `QueueTreatmentBackgroundTask` after the overflow tiers (so a caller whose hop and limit fall due together is handed on, not voiced) and measures the wait from when the caller **entered this queue**, the same clock the overflow thresholds use.
- **`InboundVoiceCallProcessor`** asks for admission before it creates the interaction, so an overflowed caller's interaction and queue item name the queue they actually wait in; a queue-full voicemail is terminalized with reason `queue_full_voicemail` through the same path as a closed entry point.
- **Voicemail needs a live call to move, which only the Voice feature has.** `IWaitingCallVoicemailSink` is declared in Core with a declining default registered by the Queues feature; Voice replaces it with `InboundVoiceWaitingCallVoicemailSink`, which hands the item to the processor's new `SendWaitingToVoicemailAsync`. That method is the generalisation of the direct-hold timeout: only a still-waiting **voice** item is moved, so a queued SMS thread on the same queue is left alone. The sink resolves the processor through `Lazy<T>` because the dependency genuinely goes both ways (processor → limit service → sink → processor).
- Tests: `QueueLimitServiceTests` (no limit reads nothing; room admits; full+voicemail; full+overflow skips a full and a disabled hop; no hop with room admits over the limit; max wait measured from queue entry; overflow takes the first unvisited hop; a declining sink leaves the caller waiting), two `InboundVoiceServiceTests` (queue-full voicemail is terminalized and never enqueued; queue-full overflow enqueues in the overflow queue and stamps it on the interaction), and the harness now injects the limit service.

### SMS routed distribution could not start on a portal-only tenant

C8 made an agent's routed-SMS availability depend on a live **voice** session heartbeat, and injected `IAgentSessionManager` into `SmsAgentAvailabilityService`. That manager is registered by the Contact Center **Agents** feature; the SMS Portal depends only on Agent Services. On a tenant running the portal without voice, the portal's admin controller could not be constructed. The semantics were also wrong for the portal: a pushed conversation is only visible in the portal, so an agent signed into the soft phone with the inbox closed would have been assigned threads they could not see.

- Presence for routed SMS is now **the portal itself checking in**. `ISmsAgentPresenceTracker` (`DistributedCacheSmsAgentPresenceTracker`) keeps a per-tenant, per-agent entry in the distributed cache that expires after the shared `AgentAvailabilityOptions.HeartbeatTimeout`; a heartbeat is a fact that expires, not a document write per agent every thirty seconds, and the cache is shared across nodes wherever the deployment has configured it to be.
- `SmsPortalHub` touches presence on connect and exposes `Heartbeat()`; the inbox and conversation pages call it on connect, every thirty seconds while open, and again after a reconnect, and stop when the connection closes.
- `SmsAgentAvailabilityService.IsAvailableAsync` is the volunteered flag **and** an open portal, and no longer depends on anything outside the portal's own feature closure.
- Tests: `DistributedCacheSmsAgentPresenceTrackerTests` (absent until touched; present until the timeout lapses; kept alive by check-ins; keyed by tenant), `SmsAgentAvailabilityServiceTests` re-pointed at the tracker.

### The auto-reply texted contacts who had opted out

C9's `AutoReplyRouter` sent the endpoint's acknowledgement to any inbound message that was not itself a carrier keyword. A contact who had opted out earlier and texted again got an automated reply, which the agent-facing send path already refuses. The router now reads the contact's `DoNotSms` flag and stays silent for them. Tests: `SmsAutoReplyRouterTests` (opted-out contact gets nothing; a known contact who has not opted out still gets the reply).

### Configuration that could only be set through a recipe

A2, A6 and A7 added skill proficiencies, queue membership preferences, overflow chains, queue limits, in-queue treatment and IVR flows to the model, and the change log recorded the editors as deferred. An operator could not set any of it without writing JSON by hand, which for a queue limit or an agent's skill level is not a reasonable ask.

- **Queue editor.** New sections for skill requirements (skill, minimum proficiency, required or preferred, relax-after seconds), the overflow chain (queue and after-seconds hops), limits (maximum callers waiting with the queue-full action, maximum wait with its action, first-response target), and what callers hear (welcome message, hold music, announcement interval, position and estimated-wait announcements with the average handle time and estimate bounds, callback key and offer delay). Rows are added and removed by a small vanilla-JS repeater (`_ContactCenterRepeaterScript.cshtml`) binding through MVC's `.Index` convention, so no client framework was added. The driver normalizes rows (drops blank ones, dedupes, clamps proficiency, orders hops); the rules that a callback key must be a telephone key and that an overflow action needs somewhere to overflow to live in `ActivityQueueHandler`, so a recipe import and the editor reject the same queues (the validation-ownership guard forbids a driver from declaring them).
- **Agent entitlements.** Skills are edited as proficiency rows (the plain tag list is derived from them, so everything that asks "does the agent have this skill" stays right), and every allowed queue gets a priority and offer-delay row that follows the allowed-queues selection. `IAgentPresenceManager.UpdateEntitlementsAsync` takes an `AgentEntitlements` object; `AgentManagedConfiguration` gained `SkillProficiencies` and `QueueMemberships`, and the recipe step and deployment source round-trip both (a recipe that ships only the tag list leaves existing proficiencies alone). `AgentEntitlementUtilities` normalizes both lists.
- **Entry point IVR.** The menu tree is edited as JSON with a placeholder that documents the shape and a list of the queue and agent identifiers an action may route to. `IvrFlowJsonModelBinder` parses the field and reports malformed JSON against it, keeping the text as typed. Whether the flow is **runnable** is checked by the new `IvrFlowValidator` from the entry point handler, so the editor and a recipe import reject the same flows: a missing or unknown root, duplicate or blank menu identifiers, a silent menu, a menu with no keys, a key that is not a single telephone key, two options on one key, an option with no action, an action that needs a destination without one, a sub-menu that does not exist, and a retry count that allows no attempt.
- Tests: `IvrFlowValidatorTests` (every rule above plus a runnable flow), `AgentEntitlementNormalizationTests` (both normalizers; entitlements store proficiencies and preferences and derive the tags; a tag-only configuration leaves proficiencies alone; proficiencies union into the tags).

### Verified and left as implemented

Read against their acceptance criteria and found correct: C1 (fail-closed conversation authorization), C2 (group-targeted notifications), C3/E1 (the routed-distribution split and the absence of the Managements reference), C4 (durable SMS inbox, per-thread lock, unique index), C5 (paged inbox), C6 (SMS outbox), D1/D2 (dial and transfer destination policies), D5 (one Telnyx client with per-command retry), D7 (distributed WebSocket registry and orphan reconciler), B3 (handoff lock and context), A3/A4/A8 (routing strategies and caller priority), A10 (predictive pacing behind the abandonment cap), E2 (null-object defaults), and the CI wiring for the activation and JavaScript suites.

### Realtime voice for automated calls

**Why.** An automated phone call ran as a loop: wait for the person to stop talking, send the recording for transcription, wait for the transcript, run the completion, wait for the answer, synthesize it, play it. Every one of those is a network round trip, and they are strictly sequential, so each turn cost seconds of dead air — and a person who started talking again mid-answer was not heard, because nothing was listening while the assistant spoke. That is what makes an automated call sound like a recording rather than a conversation.

`main` brought `IRealtimeOrchestrator`: a live speech-to-speech session that still applies the profile's system message, tools and data sources, but streams audio both directions at once. This puts the Contact Center's automated calls on it.

**How it is chosen.** Nothing new to configure in the contact center. A profile whose chat mode is Realtime carries a realtime deployment, and the answered-call path checks for exactly that: with one, the call is held as a realtime session; without one, it runs the loop it always did. Both write the same transcript to the same chat session, so the conclusion, the summary, the disposition and the subject write-back are untouched.

**The audio.** A phone call is 8 kHz, usually companded to μ-law because that is what the telephone network has always used; a realtime model sends and expects 16-bit PCM at 24 kHz. `RealtimeAudioConverter` translates both directions. This is the part where a mistake does not fail loudly — it produces a caller who hears a chipmunk, a growl, or static — so it is a pure function with its own tests: μ-law round-trips, silence staying silent (a wrong bias turns it into a hum for the whole call), duration preserved in both directions (duration is what a listener hears as pitch), the shape of a ramp surviving, and PCM not being companded when the provider did not ask for it.

The decisions worth stating:

- **Interruption is on.** Being talked through is the most common complaint about automated calls, and the one thing that makes a caller certain they are talking to a machine.
- **The call leaves the awaiting-answer state before the session starts.** A realtime session holds the call for its whole duration, and an activity left awaiting an answer for that long gets failed by the no-response expiry pass while somebody is still on it.
- **No live media means no realtime — and the caller still gets a conversation.** Speech-to-speech needs a bidirectional path to the caller's leg, which the Contact Center **Voice Media** feature provides. Automated Voice does not depend on it, so the capability is optional: the default implementation reports that no session ran and the turn-based loop takes the call. The same is true when a session fails to start. Silence is never the fallback.
- **Both pumps end together.** Whichever side finishes first — the caller hung up, or the session closed — cancels the other, and both are awaited before the media session is disposed, so neither is left writing to a socket that has gone.

**Tests added**: `RealtimeAudioConverterTests` (10) and `RealtimeVoiceConversationRunnerTests` (11) — caller audio reaching the model converted, the assistant's voice reaching the caller in the line's format, both transcripts recorded (the assistant's deltas joined into sentences rather than stored a word at a time), the deployment and voice the session is asked for, interruption enabled, the media session always stopped, and both fallbacks. `VoiceAgentConversationLoopTests` gained 5 for the selection, including that a profile with no realtime deployment keeps exactly the behaviour it had.

**A real DI cycle, found by the activation suite.** Admitting a call asks the queue-limit service whether the queue is full; a full queue sends the caller to voicemail; that path is the inbound processor again. The graph could not be constructed at all on some feature combinations. Broken with `Lazy<IInboundVoiceCallProcessor>` at the voicemail sink — the one hop where the processor is not already on the stack — which is the pattern this codebase already uses for genuine cycles.
