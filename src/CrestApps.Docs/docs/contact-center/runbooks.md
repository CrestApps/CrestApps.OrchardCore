...
- **Metrics** from the `CrestApps.OrchardCore.ContactCenter` meter: `contactcenter.outbox.redelivered` and `contactcenter.outbox.dead_lettered` (tagged by `reason`). On tenants that enable Voice, the `CrestApps.OrchardCore.Asterisk` meter adds `asterisk.realtime.ingestion.saturated` (tagged by `provider`), which counts real-time buffer saturation episodes on the Asterisk listener — a sustained rise means the dispatcher is falling behind the provider event stream.
