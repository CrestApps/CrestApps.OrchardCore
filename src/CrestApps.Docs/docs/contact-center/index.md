...
The **Contact Center Real-Time** feature (`CrestApps.OrchardCore.ContactCenter.RealTime`, depends on **Queues**, **Availability**, and the **SignalR** module) adds:
...
Providers and PBX webhooks feed call-state changes in as normalized `ProviderVoiceEvent` instances through `IProviderVoiceEventService`. The service matches the event to the interaction and call session by provider call identifier, advances their state and timestamps, bridges the agent for answered outbound calls on `ServerSideAcd` providers, and publishes the corresponding Contact Center domain events. Each event's idempotency key is scoped by its canonical provider (`{provider}|{deliveryId}`) before the duplicate check and publication, so an already-seen delivery is ignored while identical raw delivery ids emitted by different providers never collide; duplicate or out-of-order webhook deliveries are therefore safe.
...
A `ProviderVoiceEvent` may carry an optional monotonic `SequenceNumber`. When a provider supplies it, the call session persists it as a `HighWaterSequence` and ingestion rejects any delivery at or below the high-water mark. Providers that only supply timestamps or idempotency keys are still safe: ingestion falls back to timestamp ordering and additionally refuses an equal-timestamp delivery that would regress the call to an earlier lifecycle stage (for example a late `Ringing` arriving after `Connected`). Call session writes use optimistic concurrency, so two workers ingesting events for the same call cannot both commit — the loser observes a `ConcurrencyException`, which is treated as ownership loss (the canceled session is never reused and the delivery is reprocessed in a fresh scope), guaranteeing sequences `N` and `N+1` can never commit in reverse and lower the persisted high-water or state. One call session per **canonical provider-call identity** is database-enforced through a portable, non-null claim column, and a canonicalizing upgrade migration resolves legacy provider aliases, preflights and repairs legacy rows, and fails with actionable guidance on irreconcilable duplicates rather than deleting call history.
