...
| `contactcenter-queue-backlog` | Dependency | Contact Center Work Distribution | A live gauge, not a verdict: reports `queued_interactions`, the number of interactions waiting for an agent across every queue, in the check's `Data`. Registered by the Work Distribution feature, which owns the queue item store, so it is absent on tenants without Work Distribution. Stays healthy at any count. | — | The store is unreadable. |
...
| `contactcenter-shared-endpoint` | Dependency | Contact Center | Alerting-only. Present only when `OrchardCore.HealthChecks` is enabled. Reports **degraded** when that module's shared aggregate endpoint still claims a liveness route (for example the default `/health/live`) and the route has not been acknowledged, and healthy otherwise. Never gates readiness, because failing readiness here would drain the node — the very restart it warns against. | — | Never fails; degraded is its worst verdict. |
...
Because documentation cannot stop a deployment chart from wiring that route, the Contact Center module reports a degraded `contactcenter-shared-endpoint` dependency check — and logs a `Critical` entry on tenant activation — when `OrchardCore.HealthChecks` is enabled and its route still claims liveness. The check is alerting-only and never gates readiness, so the tenant stays reachable and can be corrected from `/admin`. Resolve it by moving the shared endpoint off a liveness name:
...
- **A leg that sent nothing is Poor, whatever its MOS.** The MOS only describes the audio the agent hears. The soft phone's summary rates Poor, with the likely cause **No audio sent**, when its sender carried a track but sent no packets while audio was arriving. It also rates Poor when the soft phone saw no audio leave a connected call for 5 seconds while the agent was neither muted nor holding (`OutboundAudioStalled`). A call carries silence as well as speech, so a working microphone never sends nothing. The soft phone shows the agent "The caller may not hear you. Check your microphone." as soon as it sees this. It reports a `no-outbound-audio` client diagnostic and puts a fresh microphone capture on the call.
- **The provider** (Telnyx) reports statistics for every leg it carries, including the customer's, on the hangup. That leg is rated on the provider's **MOS alone**, using the same MOS thresholds. When the provider received no audio packets at all, it has no MOS worth rating, so the leg keeps no MOS. On a customer's leg that rates Good. On an **agent's leg that the provider sent audio to**, it rates Poor with the likely cause **No audio sent**: the caller could not hear the agent.
...
### Routing configuration cache

Some tables are cheap to read but read constantly. Enabled queues, skills, business-hours calendars, and entry points are small and change rarely, yet they sit on the hot inbound-routing path — an entry point is resolved once per inbound call, and queues and skills are read again while the call is placed — so a fresh query per read let routing throughput fall as configuration grew even though a query plan for any one of them was never the problem. These four collections are served from a shared per-tenant cache (`IContactCenterConfigurationCache`): the first read after a change loads an enabled snapshot, and every routing decision afterward reads it without touching the database.

Three properties make the cache safe to trust on the routing path. **A change invalidates only the collection it touched.** Each of the four types already has a catalog entry handler that runs on every write, whether an administrator edits the entry or a recipe imports it; a generic invalidation handler registered alongside it signals that one collection's change token on create, update, and delete. **The invalidation lands after the write commits.** The signal is raised through a deferred task that runs once the writing transaction has committed, so a concurrent read cannot repopulate the cache with a value the database has not yet made durable. **A write during a load is not lost.** The change token is captured before the snapshot is loaded, so a write that commits while the load is in flight marks the just-stored snapshot stale and forces the next read to reload rather than serve it.

The cache is shell-scoped state guarded by an `ISignal` change token, not a process-local `IMemoryCache` — which the Contact Center architecture guard forbids in tenant-sensitive code. Snapshots are per-tenant by construction because the service is a per-tenant singleton, and because invalidation flows through `ISignal` it is honored across every process instance sharing the tenant's signal backplane, so a configuration change made on one node clears the snapshot on the others rather than leaving them to route on stale configuration until a restart.
