...
The first R4 capability change splits `IContactCenterVoiceProvider` metadata from executable call-control, queue/assignment, transfer, conference, recording, monitoring, and media contracts. Asterisk and DialPad now advertise only dialer dialing on their base adapters, outbound routing and server-side bridge staging require the call-control contract, and separately feature-gated media is discovered from its executable registration rather than a capability owned by the base adapter. The C001/C002 behavior remains intentionally characterized until the next R4 change makes recording and monitoring success depend on provider execution and hides unsupported controls.
