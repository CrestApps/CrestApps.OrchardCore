...
The D002 provider-command tests prove reservation acceptance and stable command-intent persistence complete before provider execution, failed acceptance never invokes the provider, command-intent commit failure compensates in a fresh Orchard scope, and definitive provider failures release accepted work. Pending crash recovery revalidates current outbound compliance and fails closed without provider execution when eligibility cannot be proven. Lost provider responses and ambiguous HTTP 408, 429, and 5xx responses persist `OutcomeUnknown`, require exclusively leased reconciliation before retry, and use fenced idempotent compensation. Unknown and terminal confirmation settlement commit atomically with interaction and activity projections, and CAS-losing commands are isolated in fresh Orchard scopes so the remaining recovery batch continues safely.
