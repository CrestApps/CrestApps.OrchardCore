...
The F001, F002, and T001 in-process proof, `ContactCenterFeatureDependencyArchitectureTests`, statically parses the Contact Center, SignalR, Telephony, and Omnichannel Managements manifests plus every Contact Center `StartupBase` class and checks them against the machine-readable ledger `.github/contact-center/feature-dependency-violations.v1.json`. The R2 headless-base remediation removed FDV001: `CrestApps.OrchardCore.ContactCenter` now depends only on headless Omnichannel infrastructure, while the independently selectable `CrestApps.OrchardCore.ContactCenter.Admin` feature owns the Omnichannel Managements dependency. For recognized generic registrations the proof resolves constructor dependencies against both manifest and `[RequireFeatures]` closures, fails on a new unrecorded mismatch, and continues to pin the two remaining findings: Voice's declared coupling to the concrete Telephony Soft Phone feature (FDV002), and Queues' undeclared runtime requirement on SignalR's `HubRouteManager` (FDV003). This is a static dependency-closure characterization only; factory/non-generic registrations and the runtime proof that every legal feature combination actually enables, migrates, and resolves every service on a live Orchard tenant remain `planned:contact-center-feature-activation-matrix` dependencies.
