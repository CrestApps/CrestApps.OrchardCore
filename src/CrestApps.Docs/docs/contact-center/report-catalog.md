...
| Default filters | Every report has tenant-local **From** and **To** date/time controls. Each report declares its supported dimensions, and the filter editor renders only those controls. Interaction reports commonly add queue group, queue, agent, channel, and direction. Workforce reports add agent. Contact Center campaign/subject reports add campaign group, campaign, channel, source, and activity status. Omnichannel reports add campaign group, campaign, channel, source, and activity status. The same filter values are applied to browser results and exports. |
...
| Visualizations | The shared renderer supports KPI cards, tables, horizontal bars, and responsive Chart.js line, bar, stacked-bar, and doughnut charts. The **Executive performance dashboard** currently provides the interactive Chart.js visualizations. Other chart types described in the catalog are planned unless their report output explicitly includes a chart section. Heat-map, gauge, funnel, Sankey, and timeline renderers remain recommended extensions where noted below. |
| Aggregate rows | Every Contact Center aggregate table appends a semantic grand-total row recalculated from the filtered raw counts and durations. Layered tables add semantic subtotals: Queue Usage identifies queue-group subtotals, Campaign Summary identifies campaign-group subtotals, and Daily Agent Timecard identifies each UTC day's subtotal. Interaction detail, presence audit, and exception-detail tables remain raw lists and do not receive artificial totals. The browser renderer uses distinct background colors for subtotal and grand-total rows. |
| Queue-group membership | Queue groups are reporting/catalog metadata and use current-membership semantics. Reports resolve an interaction's stored queue identifier against the queue's current `QueueGroupId`; moving a queue changes historical group attribution without changing the interaction or any routing/SLA/entitlement behavior. Deleting a group makes its assigned queues ungrouped. |
...
| Outcome | Every interaction has exactly one outcome, decided the same way by every report (see [Interaction outcomes](#interaction-outcomes) below): answered, abandoned, voicemail, failed, not connected, or in progress. |
| Answered | Interactions whose outcome is answered: an agent took the call. `Inbound answered` restricts the numerator to inbound interactions. A caller the platform answered only to play the queue or to record a voicemail is not answered. |
...
| Abandoned | Inbound interactions whose outcome is abandoned: the caller hung up while waiting in a queue or while an agent was being offered the call, before anybody answered. A caller who hangs up while ringing an agent is an abandon, never a failure. All abandons are included because the platform has no short-abandon threshold setting. Transfers, consults, and conferences after answer cannot be abandoned. |
| Voicemail | Inbound interactions the platform sent to voicemail instead of connecting them: an unanswered offer, the queue's maximum wait, an entry point outside business hours, or a full queue. They are neither answered nor abandoned, are shown in their own column, and are left out of the abandonment rate's numerator and the service level. |
| Failed | Interactions that failed for a technical reason: the provider reported an error, or the call could not be placed or bridged. An answered call that later failed is answered. |
| Abandonment rate | `Abandoned / Inbound offered`. Voicemail calls count toward inbound offered but not toward abandoned. Percentages are recalculated from counts at every aggregation level. |
| Wait before abandon | From the moment the caller joined the queue to the moment they hung up, as the routing engine measured it when it recorded the abandon. |
| Wait before voicemail | From the moment the caller joined the queue to the moment routing took them out of it to send them to voicemail. The greeting and the message are not part of the wait. |
...
| Service level | Per queue: `Inbound interactions answered within the queue SLA threshold / (Inbound answered + abandoned)`. Failed and voicemail interactions are excluded. All abandons remain in the denominator because no short-abandon threshold is stored. The original interaction is counted once after overflow or requeue. A queue without a positive threshold displays no service-level percentage. |
...
An interaction's status says where its communication session stopped; its outcome says what happened to the contact. The two differ: a caller who gave up and a caller sent to voicemail both leave an ended session. Every report takes the outcome from one classifier, `InteractionOutcomeClassifier`, which reads the interaction and the call events the routing engine records against it, and applies these rules in order:

1. A recorded `CallAbandoned` event makes the interaction **abandoned**. Routing records it only for a caller who hung up while waiting, in a queue or while ringing an agent, before any agent answered, so it holds even when the platform had answered the caller itself to play the queue.
2. A call the platform sent to voicemail is **voicemail**: a `CallSentToVoicemail` event, or the voicemail flag set on the interaction when it was sent. The platform answers the leg to record the message, and that answer does not make it an answered call.
3. Any other call with an answer time is **answered**.
4. A call that has not ended is **in progress**.
5. A call whose session failed is **failed**.
6. Any other inbound call that ended unanswered is **abandoned** (a caller who hung up before reaching a queue has no abandon on record); an outbound one is **not connected**.

A caller who hangs up before anybody answers cancels the call, and the interaction's status is `Ended`, not `Failed`. `Failed` is kept for a provider that could not carry the call, a dial that could not be placed, or an agent leg that could not be bridged.

Because the outcome is derived from the event log rather than stored, history reports correctly without a data migration. Interactions written before a caller's cancel stopped settling as `Failed` are still reported as abandoned, because the abandon was recorded alongside them.
...
4. Narrow the population with the displayed dimensions. Interaction reports commonly offer queue group, queue, agent, channel, and direction; workforce reports offer agent; CRM reports offer campaign, channel, source, and status.
...
| 11 | Interaction outcome summary | Shows volume and response metrics by interaction outcome (answered, abandoned, voicemail, failed, not connected, in progress). Helps operations teams isolate failures and unfinished work. | Historical; one status per row. | Status, channel, direction, provider; status/day; descending interactions. | Outcome, interactions, answered, abandoned, failed, answer rate, abandonment rate, ASA, AHT. | Stacked bar + table; outcome → provider/queue → interaction detail. |
...
| # | Report | Purpose and business value | Type and granularity | Filters; grouping; sorting | Columns/KPIs | Visualization and drill-down |
| --- | --- | --- | --- | --- | --- | --- |
| 15 | Interaction detail | Provides the auditable source row behind summary reports. Analysts and supervisors use it to reconcile totals and investigate individual contacts. | Historical drill-down; one interaction per row. | Interaction, queue, agent, channel, direction, provider, status; optional grouping by day; newest first. | Started UTC, interaction id, channel, direction, status, queue, agent, provider, wait, connected duration, wrap-up, transfer count. | Table/timeline; interaction → event lifecycle → CRM activity → recording. |
...
| 30 | Campaign performance | Gives CRM campaign owners completed-versus-pending progress across activity inventory. Supports campaign pacing and remediation. | Historical; campaign rows plus campaign-group aggregate rows. | Campaign group, campaign, source, channel, status; campaign or group/day; descending total. | Campaign or campaign-group display name, total, completed, pending, in progress, failed, cancelled, completion rate. | Stacked bars + table; campaign group → campaign → status → activity. |
...
| 39 | Long interaction detail | Supports cost, coaching, and exception review for sessions lasting at least 15 connected minutes. | Historical detail; one interaction per row. | Standard interaction filters; newest first. | Standard interaction detail columns. | Table; interaction → recording. |
...
1. Include inbound answered, inbound abandoned (a caller who hung up while queued and one who hung up while ringing an agent), inbound sent to voicemail (an unanswered offer and a maximum-wait overflow), inbound failed, and outbound answered/failed interactions.
