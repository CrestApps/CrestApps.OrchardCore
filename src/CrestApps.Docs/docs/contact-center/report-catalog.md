...
Campaign and disposition dimensions render their configured display names. If a referenced catalog entry has been deleted, the report shows **Unknown campaign** or **Unknown disposition** instead of exposing the stored identifier. User dimensions render the configured user display name rather than the account username or internal user identifier; deleted or unavailable users are shown as **Unknown user** or **Unknown agent**.
