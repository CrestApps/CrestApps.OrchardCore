...
/// <summary>
/// Durable, per-tenant record that tracks the secure ingestion of one completed conversation recording from
/// Asterisk into the encrypted media store. Persisted in the tenant's own YesSql store, the job is the single
/// source of truth for ingest progress, so a transient download or upload failure is retried with back-off and
/// eventually dead-lettered rather than silently losing the recording (failed-upload recovery).
/// </summary>
public sealed class AsteriskRecordingIngestJob
{
    /// <summary>
    ...
    /// <summary>
    /// Gets or sets the deterministic Asterisk recording name that addresses the stored file and doubles as the
    /// media store storage key. It uniquely identifies the job, so enqueueing is idempotent per recording.
    /// </summary>
    public string RecordingName { get; set; }

    /// <summary>
    /// Gets or sets the media format the recording is stored in (for example, <c>wav</c>).
    /// </summary>
