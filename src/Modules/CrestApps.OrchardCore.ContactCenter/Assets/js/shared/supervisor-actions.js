...
(function (root) {
    ...
    function agentActionsHtml(agent, state, labels, openMenu) {
        ...
        if (actions.menu.length) {
            ...
            html += '<span class="cc-agent__more">' +
                '<button type="button" class="btn btn-sm btn-outline-secondary" data-cc-more="' + id + '" aria-haspopup="menu"' +
                ' aria-expanded="' + (openMenu ? 'true' : 'false') + '" aria-controls="' + menuId + '">' +
                escapeHtml(text(labels, 'more', 'More')) + ' <span aria-hidden="true">&#9662;</span></button>' +
