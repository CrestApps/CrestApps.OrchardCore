...
internal sealed class EnterpriseInteractionReportProvider : IReport
{
        ...
        Dictionary<string, ActivityQueue> queues)
    {
        ...
        var rows = interactions
            .Where(interaction => interaction.AnsweredUtc.HasValue)
            .GroupBy(interaction => ResolveQueueName(interaction.QueueId, queues), StringComparer.OrdinalIgnoreCase)
            .Select(group =>
            {
        ...
        AgentPerformanceMode mode)
    {
        ...
        var rows = interactions
            .Where(interaction => !string.IsNullOrEmpty(interaction.AgentId))
            .GroupBy(interaction => interaction.AgentId, StringComparer.Ordinal)
            .Select(group =>
            {
                var metrics = Aggregate(group);
    ...
    {
        var eligible = interactions.Where(interaction => interaction.AnsweredUtc.HasValue);
        var columns = new[]
        {
            new ReportColumn(S["Channel"].Value),
            new ReportColumn(S["Answered"].Value, ReportColumnAlign.End),
    ...
    {
        var legs = interactions.SelectMany(interaction => interaction.CallLegs);
        var columns = new[]
        {
