...
/// <c>[data-phone-dial]</c> placeholder and opens the messaging workspace on the field's number, on the SMS channel, so an operator can start
...
{
    private const string EvaluatedRequestKey = "CrestApps.Messaging.Sms.PhoneFieldButtonEvaluated";
    ...
    private void RegisterButton()
    {
        ...
        // OnDisplaying is synchronous. The workspace permission is evaluated by the standard permission handler,
        ...
        var authorizeTask = services.GetRequiredService<IAuthorizationService>()
            .AuthorizeAsync(httpContext.User, MessagingPermissions.UseMessagingWorkspace);
        ...
        var startUrl = services.GetRequiredService<LinkGenerator>()
            .GetPathByName(httpContext, "MessagingStart", values: null);
        ...
        return $$"""
            ...
            (function () {
                ...
                function enhance(placeholder) {
                    ...
                    button.type = 'button';
                    button.className = 'btn btn-sm btn-outline-primary messaging-sms-btn ms-1';
                    ...
                    button.addEventListener('click', function () {
                        ...
                        window.open(startUrl + sep + 'channel=SMS&address=' + encodeURIComponent(number), '_blank', 'noopener');
                    });
