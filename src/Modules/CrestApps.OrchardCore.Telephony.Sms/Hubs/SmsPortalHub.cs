...
public sealed class SmsPortalHub : Hub<ISmsPortalHubClient>
{
    ...
    public override async Task OnConnectedAsync()
    {
        ...
        if (httpContext?.User is null ||
            !await _authorizationService.AuthorizeAsync(httpContext.User, TelephonySmsPermissions.UseSmsPortal))
        {
            Context.Abort();
        ...
        if (!string.IsNullOrEmpty(userId))
        {
            ...
            if (await _authorizationService.AuthorizeAsync(httpContext.User, TelephonySmsPermissions.ViewAllConversations))
            {
