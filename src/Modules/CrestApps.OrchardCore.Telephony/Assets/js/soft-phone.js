    ...
    function createSoftPhone(rootElement, options) {
        ...
        function recoverLocalAudioTrackMidCall() {
            ...
            return navigator.mediaDevices.getUserMedia(buildAudioConstraints()).then(function (stream) {
                ...
                var replace = browserAudioSession && typeof browserAudioSession.replaceLocalAudioTrack === 'function'
                    ? Promise.resolve(browserAudioSession.replaceLocalAudioTrack(fresh))
                    : Promise.reject(new Error('The media session cannot replace its outgoing track.'));
                ...
                return replace.then(function () {
                    ...
                    reportDiagnostic('info', 'microphone-recovered',
                        'The captured microphone was replaced under the live call.', fresh.label || '');
        ...
        function onInputDeviceChange() {
            ...
            showError(strings.microphoneChangeOnNextCall ||
                'The microphone will change on your next call. This call keeps the one it started with.');
        }
