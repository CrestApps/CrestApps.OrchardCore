...
(function () {
  ...
  function createTelnyxSession(telnyx, context, registrationConfig) {
    ...
    var holdAudio = typeof createHoldAudioController === 'function' ? createHoldAudioController({
  ...
  function createSoftPhone(rootElement, options) {
    ...
    function render() {
      ...
      if (dom.number && currentCall && (active || stateName === 'OnHold')) {
        ...
        if (peerNumber && shouldShowCallNumber({
          stateName: stateName,
      ...
      show(dom.transfer, liveMedia && transferModes(capabilities).length > 0 && (!currentIsConference || selectedConferenceCallIds.length === 1));
      if (dom.number) {
      ...
      [dom.dial, dom.hangup, dom.hold, dom.resume, dom.mute, dom.unmute, dom.transfer, dom.hangupAll].forEach(function (button) {
        if (button) {
          button.disabled = !!activeCommand;
        }
      });
    ...
    function transfer() {
      ...
      transferPanel.open({
        callLabel: formatPhoneNumber(getPeerNumber(currentCall))
      });
    }
