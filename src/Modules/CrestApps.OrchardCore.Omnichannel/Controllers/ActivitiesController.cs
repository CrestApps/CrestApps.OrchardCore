...
public sealed class ActivitiesController : Controller
{
        ...
        [FromServices] IShapeFactory shapeFactory)
    {
        ...
        var subjectIds = activities.Select(x => x.SubjectId)
            .Where(x => !string.IsNullOrEmpty(x))
            .Distinct()
            .ToArray();
        ...
        foreach (var activity in activities)
        {
            ...
            var subject = contentItems.FirstOrDefault(x => x.ContentItemId == activity.SubjectId);
    ...
    public async Task<IActionResult> Process(string id)
    {
        ...
        var contentItems = await _session.Query<ContentItem, ContentItemIndex>(index => (index.ContentItemId == activity.ContactContentItemId && index.Latest) || (index.ContentItemId == activity.SubjectId && index.Published)).ListAsync();

        var user = await _session.Query<User, UserIndex>(index => index.UserId == activity.AssignedToId).FirstOrDefaultAsync();

        var contact = contentItems.FirstOrDefault(x => x.ContentItemId == activity.ContactContentItemId);

        var container = new OmnichannelActivityContainer(activity, contact, user, contentItems.FirstOrDefault(x => x.ContentItemId == activity.SubjectId));
