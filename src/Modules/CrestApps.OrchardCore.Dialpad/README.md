...
Enter a Dialpad Admin API key in **Webhook registration API key**, save the settings, then use **Register webhook** to create the company-level call-event webhook and subscription automatically. The server generates the signing secret, sends it to Dialpad through `POST /api/v2/webhooks`, creates the subscription through `POST /api/v2/subscriptions/call`, and stores the secret encrypted without showing it in the browser. Dialpad event delivery is required for synchronized soft-phone call state and inbound routing; REST polling is only a fallback when provider events are delayed or unavailable.
