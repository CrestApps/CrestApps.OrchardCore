...
internal sealed class AIDocumentLegacyTypeNameMigrations : DataMigration
{
    ...
    private static async Task RewriteLegacyTypeNamesAsync(IServiceProvider scope)
    {
        ...
        var logger = scope.GetRequiredService<ILogger<AIDocumentLegacyTypeNameMigrations>>();
        var yesSqlStoreOptions = scope.GetRequiredService<IOptions<YesSqlStoreOptions>>().Value;

        if (string.IsNullOrWhiteSpace(yesSqlStoreOptions.AIDocsCollectionName))
        {
            return;
        }

        var dialect = store.Configuration.SqlDialect;
        var documentTableName = store.Configuration.TableNameConvention.GetDocumentTable(yesSqlStoreOptions.AIDocsCollectionName);
    ...
    private static string RewriteLegacyTypeName(string typeName)
    {
        ...
        foreach (var (legacyNamespacePrefix, legacyAssemblyName, currentNamespacePrefix) in _legacyTypeNameReplacements)
        {
            if (!typeName.StartsWith(legacyNamespacePrefix, StringComparison.Ordinal) ||
                !typeName.Contains($", {legacyAssemblyName}", StringComparison.Ordinal))
            {
                continue;
            }
