...
/// <c>[data-phone-dial]</c> placeholder and opens the SMS Portal on the field's number, so an operator can start
...
{
    private const string EvaluatedRequestKey = "CrestApps.SmsPortal.PhoneFieldButtonEvaluated";
    ...
    private void RegisterButton()
    {
        ...
        // OnDisplaying is synchronous. The SMS Portal permission is evaluated by the standard permission handler,
        ...
        var authorizeTask = services.GetRequiredService<IAuthorizationService>()
            .AuthorizeAsync(httpContext.User, SmsPortalPermissions.UseSmsPortal);
        ...
        return $$"""
            ...
            (function () {
                ...
                function enhance(placeholder) {
                    if (placeholder.__smsPortalEnhanced) { return; }
                    placeholder.__smsPortalEnhanced = true;

                    var button = document.createElement('button');
                    button.type = 'button';
                    button.className = 'btn btn-sm btn-outline-primary sms-portal-sms-btn ms-1';
