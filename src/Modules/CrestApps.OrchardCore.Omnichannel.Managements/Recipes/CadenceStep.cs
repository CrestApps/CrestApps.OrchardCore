...
/// <summary>
/// Imports Omnichannel re-engagement cadences carried by a recipe step, creating entries that do not exist and
/// updating those that do.
/// </summary>
internal sealed class CadenceStep : NamedRecipeStepHandler
{
    ...
    public CadenceStep(
        ...
        IStringLocalizer<CadenceStep> stringLocalizer)
        : base(OmnichannelDeploymentSteps.Cadence)
    {
        _manager = manager;
        S = stringLocalizer;
    }
    ...
    {
        var model = context.Step.ToObject<CadenceStepModel>();
        var tokens = model.Cadences?.OfType<JsonObject>() ?? [];
        ...
        foreach (var token in tokens)
        {
            ...
            var id = token[nameof(Cadence.ItemId)]?.GetValue<string>();
    ...
    private sealed class CadenceStepModel
    {
        public JsonArray Cadences { get; set; }
    }
}
