    ...
    where TTool : AITool
{
    ...
    protected static int GetTimeout(AIFunctionArguments arguments, int fallbackValue = BrowserAutomationConstants.DefaultTimeoutMs)
    {
        var timeout = arguments.TryGetFirst<int>("timeoutMs", out var parsedTimeout)
            ? parsedTimeout
            : fallbackValue;

        return Math.Clamp(timeout, 1_000, BrowserAutomationConstants.MaxTimeoutMs);
    }

    protected static int GetMaxItems(AIFunctionArguments arguments, int fallbackValue = BrowserAutomationConstants.DefaultMaxItems)
    {
        var maxItems = arguments.TryGetFirst<int>("maxItems", out var parsedMaxItems)
            ? parsedMaxItems
            : fallbackValue;

        return Math.Clamp(maxItems, 1, BrowserAutomationConstants.MaxCollectionItems);
    }

    protected static int GetMaxTextLength(AIFunctionArguments arguments, int fallbackValue = BrowserAutomationConstants.DefaultMaxTextLength)
    {
    ...
    protected static string GetSessionId(AIFunctionArguments arguments)
        => GetRequiredString(arguments, "sessionId");
